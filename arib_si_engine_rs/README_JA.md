...
このライブラリの ARIB 文字列 decoder は、字幕以外の SI/EPG 文字列を対象にする。字幕は TIS 側の字幕 path で `libaribcaption` を使う前提であり、このライブラリは字幕用 decoder の完全実装を claim しない。`arib_si_engine_rs` は libaribcaption wrapper を所有しない。

## 公開 API 境界

通常の JNI getter が返す service/transport/PMT snapshot は publishable snapshot である。raw snapshot は診断・test の内部確認用に限定する。publish できない service の理由は publishability 診断 getter で確認する。
