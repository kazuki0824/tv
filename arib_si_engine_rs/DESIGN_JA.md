...
NIT transport loopの`descriptor_tag=0x43`衛星分配記述子は、先頭4 byteの有効な8桁BCD周波数を10 kHz単位からHzへ変換し、該当ONID/TSIDの`DiscoveredTransport.satellite_frequency_hz`として保持する。BCD nibble不正または記述子長不足では周波数を生成しない。値は放送由来のdownlink周波数であり、IF変換、BS/CS110分類、scan候補化はTISが所有する。NIT version更新で該当transport metadataが失効した場合は旧周波数も同じscopeで破棄し、別versionへ持ち越さない。
