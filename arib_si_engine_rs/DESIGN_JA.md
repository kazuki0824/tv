...
ARIB SI/EPG文字デコードの受け入れ判定は、単体テストのみで通過してよい。実波 TS ファイル テストデータは r51 必須条件にせず、descriptor byte array / section builder による Rust 単体テスト を正式な受け入れ条件にする。対象は SDT サービス名、EIT short_event、extended_event fragment、長形式イベント項目、component、audio_component、series、unsupported escape、truncated text、replacement 診断である。
