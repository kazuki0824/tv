...
impl EitInstances {
    pub fn ingest(&mut self, section: &[u8]) {
        let Some(header) = parse_section_header(section) else { return; };
        if !(0x4e..=0x6f).contains(&header.table_id) || section.len() < 18
            || header.total_length != section.len() || !section_crc_valid_with_header(section, &header) {
            return;
        }
        let (Some(version), Some(number), Some(last), Some(current)) =
            (header.version, header.section_number, header.last_section_number, header.current_next_indicator) else { return; };
        let key = TableKey(header.table_id, u16::from_be_bytes([section[10], section[11]]),
            u16::from_be_bytes([section[8], section[9]]), u16::from_be_bytes([section[3], section[4]]), current);
        let instance = self.instances.entry(key).or_insert_with(|| Instance { version, last, ..Instance::default() });
        if instance.version != version { *instance = Instance { version, last, ..Instance::default() }; }
        if instance.last != last || number > last { instance.inconsistent = true; }
        if header.table_id >= 0x50 {
            let start = number & 0xf8;
            let end = section[12];
            if end < number || end > last || end & 0xf8 != start
                || instance.segment_ends.get(&start).is_some_and(|old| *old != end) {
        ...
        while cursor + 12 <= body_end {
            let length = (((section[cursor + 10] & 15) as usize) << 8) | section[cursor + 11] as usize;
            cursor += 12 + length;
        }
        instance.safe.remove(&number);
        if cursor == body_end && events.iter().all(|event| event.diagnostics.iter()
    ...
    pub fn states(&self) -> Vec<EitInstanceState> {
        self.instances.iter().map(|(key, value)| {
            let required: BTreeSet<u8> = if key.0 < 0x50 { (0..=value.last).collect() } else {
                (0..=value.last).step_by(8).flat_map(|start| {
            ...
            EitInstanceState {
                table_id: key.0, original_network_id: key.1, transport_stream_id: key.2, service_id: key.3,
                version: value.version, current_next_indicator: key.4, last_section_number: value.last,
                complete: !value.inconsistent && missing.is_empty(), inconsistent: value.inconsistent,
                received_sections: received.into_iter().collect(), missing_sections: missing,
    ...
    pub fn events(&self) -> Vec<EitEvent> {
        self.instances.iter().filter(|(key, _)| key.4)
            .flat_map(|(_, value)| value.sections.values().flatten().cloned()).collect()
    }
}
...
mod tests {
    ...
    fn section(version: u8, current: bool, number: u8, last: u8) -> Vec<u8> {
        let mut bytes = vec![0x4e, 0xf0, 15, 0, 1, 0xc0 | (version << 1) | u8::from(current),
            number, last, 0, 17, 0, 34, last, 0x4e];
    ...
    fn versions_next_table_and_inconsistency_remain_distinct() {
        ...
        let current = store.states().into_iter().find(|state| state.current_next_indicator).unwrap();
