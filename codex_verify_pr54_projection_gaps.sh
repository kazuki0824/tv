...
python3 "$STAGING_DIR/codex_apply_pr54_projection_gaps.py"
# Normalize only generated EOF whitespace if needed; no semantic edits.
python3 - <<'PY'
from pathlib import Path
for p in [Path('arib_si_engine_rs/src/descriptors.rs'), Path('arib_si_engine_rs/src/provider_data.rs')]:
    text=p.read_text()
...
assert '複数 series_id がある場合は `COLUMN_MULTI_SERIES_ID`' in doc
assert 'event-${event.eventId}' not in Path('tis/src/com/maleicacid/tvinput/aribsi/EventModelMapper.kt').read_text()
print('XML/projection scope/series exclusion OK')
PY
...
git commit -m 'fix(tis): align multilingual TvProvider projection'
