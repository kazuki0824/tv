...
mutex汚染復旧 は 閉鎖側失敗 として固定する。`PoisonError::into_inner()` で通常復旧して処理継続してはならない。
...
- ロック名を ログまたは診断情報に出す
- 汚染回数を増やす
- 対象オブジェクト / サブシステムを 劣化、閉鎖済み、失敗 のいずれかへ遷移させる
...
```text
- 通常停止、エラー停止、`panic` 停止 を区別する
- エラーを診断情報に記録する
- 影響を受けたオブジェクトを 失敗 / 劣化 / 閉鎖済み にする
- 次の 公開APIで エラーを返す
```

`std::thread::spawn(move || { ... })` 内で `unwrap()` を使って ワーカーを落とすことは禁止する。`catch_unwind` を使う場合も、捕捉後に通常継続せず、対象サブシステムを 閉鎖側失敗 にする。
...
4. mutex汚染を 閉鎖側失敗 にしているか
5. ワーカーエラーが診断情報とオブジェクト状態に反映されるか
