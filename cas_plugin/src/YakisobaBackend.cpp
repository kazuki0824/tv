...
Result YakisobaBackend::processEcm(const std::shared_ptr<KeyRegistry::Slot>& slot, View payload) {
    ...
    if (groupIndex(payload[1]) < 0 || !protocolSupported(payload[0])) return Result::Unsupported;
    Secret<16> keys;
    result = decodeResult(bcas_decodeECM(payload.data, payload.size, keys.bytes.data(), nullptr));
    if (result != Result::Ok) return result;
    return KeyRegistry::instance().update(slot, keys, payload[1]);
}
...
Result YakisobaBackend::applyMessage(const EmmMessage& message) {
    ...
    const auto& persisted = state_.groups[groupIndexValue];
    const auto number = be16(plain + 9);
    if (persisted.updated && number <= persisted.number) {
        const bool duplicate = number == persisted.number && persisted.lastMessage.size() == payload.size &&
            std::equal(persisted.lastMessage.begin(), persisted.lastMessage.end(), payload.data);
    ...
    for (size_t p = 13; p < payload.size - 4;) {
        ...
        if (tag != 0x10) return Result::Unsupported;
        ...
        // Register() は即時更新で巻戻し手段がないため、受理条件だけを更新前に確認する。
        Secret<8> previous;
    ...
    if (updates.empty()) return Result::Unsupported;

    Bytes committedMessage(payload.data, payload.data + payload.size);
    YakisobaPersistentState next = state_;
    auto& nextGroup = next.groups[groupIndexValue];
    nextGroup.updated = true;
    nextGroup.number = number;
    nextGroup.lastMessage = committedMessage;
    for (const auto& update : updates) {
        auto& saved = nextGroup.keys[update.id % kYakisobaKeyBuckets];
        saved.present = true;
        saved.id = update.id;
