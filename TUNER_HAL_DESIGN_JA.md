...
| 実装アンカー | 物理 module / file / type は `tuner_hal2/DESIGN_JA.md` の同名規範実装アンカー行を単一正本として参照し、本書では複製しない |
...
サービス全体の利用停止状態はサービス所有者に一元化し、一度確定した`ServiceCritical`を再初期化で解除しない。サービスの状態ロックが汚染された場合も、利用停止と汚染検出回数を同じ観測値として読み出せること。停止状態の記録には当該ロックの正常取得を要求せず、診断参照から通常状態を変更する権限は与えない。ロック汚染時の実装規約は`GLOBAL_CODE_CONVENTION.md`の「mutex汚染復旧」と`tuner_hal2/CODE_CONVENTION.md`の「サービス状態ロックの実装入口」を参照する。
...
DVR通知の回収ワーカーにも、0-S-3Bの`WorkerRuntime`および`WorkerFailureClassifier`の論理契約を適用する。対応する実装箇所は`tuner_hal2/DESIGN_JA.md`の「共通の失敗伝達とワーカー管理の実装位置」を参照する。

`WorkerRuntime`に従属するsupervisorでworker開始が進行中の場合、開始中のstop/reset要求はその開始試行の取消しとして保持し、取消し後の再開始要求は同じ開始試行と混同せず後続要求として保持する。開始成功時だけactiveまたは回収対象へ移し、開始失敗時は開始中状態を回収する。開始中のstopは通常のactive worker回収完了として扱わず、開始保留中であることを型付き結果で区別する。supervisor固有の状態は`WorkerRuntime`から独立したgeneric lifecycle正本または第二generation/retry ownerにしてはならない。
...
採用px4 driverのISDB-T `PTX_SET_CHANNEL` はRF/channel設定後にdriver内で有限のlock確認を行い、その観測期間内にlockしなかった場合もchannel設定自体は適用済みのまま `-EAGAIN` を返す。`PTX_SET_CHANNEL` の `EAGAIN` はISDB-Tだけ「channel適用済み・lock未確定」のtyped pending結果として扱い、`IFrontend.tune()` のcaller-visible failureへ写像しない。HALはbackend submit開始直前に当該candidateのabsolute `tuneTerminalDeadlineMs` deadlineを確定し、`PTX_SET_CHANNEL` 内部のlock確認時間も同じbudgetへ含める。`EAGAIN` 後は当該sessionをcapture未開始のまま維持し、残りbudgetだけをfresh `PTX_GET_LOCK_STATUS` に使用する。deadlineまでlockしなければ `PTX_START_STREAMING` / `PTX_STOP_STREAMING` を発行せず `NO_SIGNAL` で終端し、lockを確認した場合だけ `PTX_START_STREAMING` を正確に1回実行して `LOCKED` 経路へ進む。その他errnoおよびISDB-Sの `PTX_SET_CHANNEL EAGAIN` はbackend failureとして扱う。
...
PX4の選局では、`PTX_START_STREAMING`をbackend submit中に先行実行しない。lock確定後、bound demuxが存在する場合だけ同一backend sessionからlive readerを派生させ、live pump workerが開始待ち状態へ到達したことを確認してから`PTX_START_STREAMING`を実行する。bound demuxがない正常なfrontend単独選局ではlive pumpを必須化せず、tune / `LOCKED`経路を維持しつつ`PTX_START_STREAMING`も保留する。後からdemuxがfrontendへ結合された場合は、current lockを確認した同じworker内でlive readerとprepared pumpを先に準備し、`PTX_START_STREAMING`を正確に1回実行してからpumpをactivateする。START成功後はprepared pumpの開始待ちを直ちに解除し、その後にTS配送を継続する。Linux DVBにはこのprepared-pump順序を適用しない。これによりPX4のHAL自身が`START -> callback -> reader生成 -> worker生成 -> read`の欠落窓を作らない。STARTまたは開始待ち解除に失敗した場合は、同一sessionとlive pumpの既存cleanup入口で一回だけ回収し、初期TS用の第二queue、第二generation、第二stream boundary ownerを追加しない。

PX4の`PTX_START_STREAMING`とDemux-Frontend relation変更の競合は`DemuxFrontendSourceTxn`のrelation ownerで直列化する。START実行前にcurrent bound demuxとstream generationを再検証し、同じrelation ownerに従属する一回性のSTART-in-flight reservationを取得する。reservation取得からSTART成功後のprepared pump activate完了、またはSTART不成立・取消し時の非activate確定とprepared pump停止回収完了まで、当該frontendを含むrelation mutationをcommitしない。この区間と`IDemux.setFrontendDataSource()`が競合した場合は待機、polling、専用deadlineを追加せず`UNAVAILABLE`を返し、既存relationとstream generationを変更しない。Frontend/Demux close等のcleanup要求は同じ競合を既存cleanup retryへ接続し、START専用relation epoch、mutation generation、rollback leaseを追加しない。START成功後のactivate直前を取消しとの線形化点とし、それ以前に確定した取消しではpumpをactivateせず、開始済みcaptureとprepared pumpを既存session/worker cleanupへ渡す。それ以後の取消しはactivate済みworkerの通常取消しとして扱う。

採用px4_drvの`PTX_START_STREAMING` ioctl内部に残る`set_capture(true)`からringbuffer write-readyまでの窓はdriver内部の実装境界であり、HALのreader/pump準備順序だけでは解消済みと扱わない。HALはSTART前のconsumer準備とSTART後の即時activateまでを保証し、driver内部のwrite-ready成立条件をHAL側の第二状態や待機で推測・代替しない。
...
STD-B25デコード能力とSTD-B25各条項へのproduct-level適合宣言を分離する。Part 1 §4.9の適合判断は`開発規則.md`を唯一の正本とし、本書では再宣言しない。`StdB25DecodeCapability`、1鍵組の保証、実鍵組数または実PID数はHALの限定decode能力を表すだけであり、製品全体の規格適合宣言へ読み替えない。
...
製品profileで公開demuxのいずれにもSTD-B25デコード能力を有効にしない構成では`openDescrambler()`を`UNAVAILABLE`とし、VTS製品設定へdescrambling flowを含めない。一部のdemux経路だけで能力を有効にする構成では、未結合objectの生成後、対象外demuxへの`setDemuxSource()`を`UNAVAILABLE`とする。限定能力の公開を製品全体のSTD-B25適合宣言へ読み替えず、適合判断は`開発規則.md`を参照する。鍵素材はslot数だけを台帳化し、公開AIDLまたは診断へ出さない。

VTS/lab config のdescrambling flowをどのreleaseで有効化するかは `開発規則.md`、HAL capabilityと`VtsEnvironmentProfile`の成立条件は本書の`VTS profile / capability 対応契約`を正とする。release名ごとのCAS完成度を本書へ複製しない。正本release scopeでdescramblingを有効化する場合は、MediaCas session由来tokenと有効な内部鍵状態の対応、対象demuxの`StdB25DecodeCapability`、使用するVTS artifact / variant / input / CAS system / filter / PID / queue / memory等の入力と必要資源が起動前に確定・予約できなければならない。descrambler能力を宣言したprofileからflowを隠して検査を回避してはならない。Tuner HAL は section payload delivery、`IDescrambler`、`setKeyToken()`、`addPid()` / `removePid()`、token lookup境界と診断を契約対象とし、CA意味解析・MediaCas制御・実token取得等の上位責務は各正本文書へ委ねる。
...
| B25/B1製品結合試験 | `開発規則.md`のCA system IDに対応するMaleicacid plugin | `LIVE + MULTI2`のsessionで実ECM処理を成功させ、同じsession IDからcurrent Ksを参照して実TSを復号する |
| B25/B1 Tuner VTS descrambling profile | 上記Maleicacid pluginと、呼出し契約が一致する選択済みVTS artifact | session IDの呼出し元への返却、対応mode、対象sessionへのECM投入、`setKeyToken()`結果の検査、対象PIDの復号確認までを成立させる。CA system名やXMLの変更だけでこの条件を満たしたとは扱わない |
...
能力選択の診断では失敗理由と資源を返却した順序を、FMQ配送の診断では書込み失敗・短い書込み・起床失敗の種別を識別できること。配送失敗後の取消しも失敗した場合は、元の配送失敗と取消し失敗の両方を観測可能とする。ワーカー生成失敗では取得できたOSエラー情報を診断材料として残す。これらの診断情報によって公開AIDLの戻り値分類を変更しない。型付き情報の伝達と複合エラーの実装規約は`tuner_hal2/CODE_CONVENTION.md`の§1・§12・§13を参照する。

サービス診断snapshotは診断領域ごとに取得結果を保持し、一つの診断領域の取得失敗を理由に、取得に成功した他領域の記録を破棄しない。取得失敗した領域は正常な空結果へ読み替えず、対象領域を識別できる型付き取得失敗として、取得済みの他領域の記録と同じsnapshotに保持する。
...
コールバック保管・死亡通知、代替診断・破棄時診断、DVR確定後通知・通知回収・コールバック整合性診断、回収処理の所有者格納、ワーカー取消し理由・管理状態、DVRキュー世代の各ロックは、汚染を通常の内部不整合やキュー記述子不正と区別する。汚染検出時は保護データへ通常アクセスせず、対象ロックの種別、同じロックの寿命内の検出回数、回数の飽和を型付き失敗として既存の呼出し結果・診断へ伝達する。キュー識別子が存在する場合は同じ識別子も保持する。

汚染の状態と検出回数は当該ロックに従属し、別の状態所有者や永続台帳を作らない。取得、条件変数待機後の再取得、記録、消去のいずれも汚染を成功へ読み替えず、失敗した消去では汚染回数を初期化しない。ワーカーの終了分類ではロック汚染として扱い、DVRの主処理失敗と取消し失敗が重なった場合は両方を保持する。公開状態の異常時遷移・再試行可否は既存の各所有者の契約に従い、診断回数をその判定入力にしない。

診断取得は既存のサービス診断経路へ接続する。診断保持先自体が汚染した場合は、空の正常結果ではなく対象と検出回数を含む取得失敗を返す。汚染したデータから記録を復元することは要求しない。
...
- ARIB STD-B10 5.13-E1 Part 2 5.2.4〜5.2.17・Part 3 5.1.1〜5.1.3を表ごとのsection上限1021/4093の根拠とし、STD-B32 3.11-E1 Fascicle 3 Chapter 3 3.1をPES構文、Fascicle 1 Chapter 5 5.1.1・Attachment 2 Chapter 5 5.1・Attachment 5 Chapter 5 5.1.1を製品対象video PESのPTS明示、Fascicle 2 Chapter 5.2.2をMPEG-2 AAC LC ADTSのsampling frequencyと1 raw-data-block/frameというexact frame duration、Fascicle 2 Attachment Chapter 2 2.1をaudioでは特定境界の先頭frameにPTSを要求するだけで全PESへの明示保証ではないことの証拠本文とする。Fascicle 3がoptional PES headerを委ねるITU-T H.222.0 2.4.3.7は、audio PTSが当該PES内で開始する最初のaudio access unitへ対応することの根拠とする。B32を4093の独立した上限根拠として使用しない。B25は公式英訳6.7-E1全文を精読基準とする。Part 1 §4.9のproduct-level適合判断は `開発規則.md` を参照し、本書はSTD-B25デコード能力の実鍵組数、実PID数、pool共有単位、枯渇時のHAL結果だけを製品profileの事実として定義する。AOSPに公開欄は追加せず、session間で共有する同じ内部台帳で受付と解放を強制する。
- 対象ドライバーと上流Linuxの証跡は、AOSP契約とは独立した根拠として扱う。

### ARIB規範本文との静的照合

STD-B25 6.7-E1 Part 1 §4.10（本文147頁）は当該CASに12 PID以上の同時処理を要求し、§5.1はBS・広帯域CS・地上デジタル等の固定受信に第2〜4章を適用する。実PID数の台帳だけで適合が成立するわけではなく、CASを含む受信機全体との照合を要する。§4.9に関するproduct-level判断を§4.10の免除へ流用しない。どのreleaseで実CAS適合を検証対象にするかは `開発規則.md` を正とし、本書は検証時に物理tuner/backend・共有pool・同時処理PIDの実証と適用原文を対応付ける条件だけを持つ。12 PIDの証拠は6.7-E1に限定し、現行日本語7.0との差分と実容量の確認は未完である。
