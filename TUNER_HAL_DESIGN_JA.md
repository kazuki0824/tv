...
採用px4 driverのISDB-T `PTX_SET_CHANNEL` はRF/channel設定後にdriver内で有限のlock確認を行い、その観測期間内にlockしなかった場合もchannel設定自体は適用済みのまま `-EAGAIN` を返す。`PTX_SET_CHANNEL` の `EAGAIN` はISDB-Tだけ「channel適用済み・lock未確定」のtyped pending結果として扱い、`IFrontend.tune()` のcaller-visible failureへ写像しない。HALはbackend submit開始直前に当該candidateのabsolute `tuneTerminalDeadlineMs` deadlineを確定し、`PTX_SET_CHANNEL` 内部のlock確認時間も同じbudgetへ含める。`EAGAIN` 後は当該sessionを維持してstreamingを開始し、残りbudgetだけをfresh `PTX_GET_LOCK_STATUS` に使用して `LOCKED` または `NO_SIGNAL` を終端させる。その他errnoおよびISDB-Sの `PTX_SET_CHANNEL EAGAIN` はbackend failureとして扱う。
