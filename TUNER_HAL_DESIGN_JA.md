...
採用px4 driverのISDB-T `PTX_SET_CHANNEL` はRF/channel設定後にdriver内で有限のlock確認を行い、その観測期間内にlockしなかった場合もchannel設定自体は適用済みのまま `-EAGAIN` を返す。この `-EAGAIN` はhard backend failureではなく「channel適用済み・lock未確定」のtyped pending結果として扱い、`IFrontend.tune()` のcaller-visible failureへ写像しない。HALは当該sessionを維持してstreamingを開始し、その後のfresh `PTX_GET_LOCK_STATUS` と `tuneTerminalDeadlineMs` により `LOCKED` または `NO_SIGNAL` を終端させる。その他errnoは従来どおりbackend failureである。ISDB-Sでは採用driverがlock成功後にstream-id適用を行うため、`PTX_SET_CHANNEL -EAGAIN` を同じpending成功へ一般化せず、selector適用完了を保証できる別契約が定義されるまでbackend failureとして扱う。
