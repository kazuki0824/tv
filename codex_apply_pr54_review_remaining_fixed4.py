...
a = '''                val scanTargets = targets.getOrElse { emptyList() }\n                val scanResult = controller.startBootEpgSync(scanTargets)\n'''
b = '''                val scanTargets = targets.getOrElse { emptyList() }\n                val requiredTargetKeys = scanTargets.mapTo(linkedSetOf()) { it.serviceKey }\n                val scanResult = controller.startBootEpgSync(scanTargets)\n'''
if s.count(a) != 1: raise SystemExit(f'boot target anchor count={s.count(a)}')
s = s.replace(a,b,1)
a = '''                val allRequiredTargetsCommitted = scanResult.scanned > 0 &&\n                    scanResult.successfulCandidates == scanResult.scanned &&\n                    !terminalCancel\n'''
b = '''                val allRequiredTargetsCommitted = requiredTargetKeys.isNotEmpty() &&\n                    scanResult.committedServiceKeys.containsAll(requiredTargetKeys) &&\n                    !terminalCancel\n'''
if s.count(a) != 1: raise SystemExit(f'boot completion condition count={s.count(a)}')
...
print('applied resilient Direct Boot ledger and PCM mappings')
