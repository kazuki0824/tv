...
//!
//! r50dz25 WP-03 では binder_service の record event builder から呼ばれる。
...
#[derive(Debug, Default)]
pub struct RecordIndexEventBuilder;
...
pub struct RecordEventState {
    ...
    // r50dz53/G2-02,G2-03: scanner/parser carry across TS payload boundaries.
...
fn build_ts_record_event_data(
    ...
    let packet_view = crate::packet_pipeline::TsPacketValidator::validate(packet).ok()?;
...
mod tests {
    ...
    impl CodecStartCodeScanner for NullScanner {
        fn codec(&self) -> CodecKind {
            CodecKind::H264
        }
