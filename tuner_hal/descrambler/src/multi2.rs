...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum Multi2RuntimeError {}

pub fn multi2_decrypt_payload(payload: &mut [u8], key: &PreparedMulti2Key) -> Result<(), Multi2RuntimeError> {
    decrypt_cbc_ofb(payload, key.cbc_iv, key.work_key, key.rounds);
    Ok(())
}

pub fn multi2_encrypt_payload(payload: &mut [u8], key: &PreparedMulti2Key) -> Result<(), Multi2RuntimeError> {
