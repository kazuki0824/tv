...
pub struct StreamBoundaryResetPlan {
...
pub trait StreamBoundaryResources {
    ...
    fn notify_worker_boundary(&mut self, plan: &StreamBoundaryResetPlan)
        -> Result<(), Self::Error>;
...
impl StreamBoundaryResetPlan {
    pub fn for_demux(reason: StreamBoundaryReason, demux_id: i32, generation: u64) -> Self {
        Self {
            reason,
            demux_id,
            generation,
    ...
    pub fn execute<R: StreamBoundaryResources>(
        &self,
        resources: &mut R,
    ) -> Result<StreamBoundaryResetResult, R::Error> {
        self.execute_from_step(resources, None)
    }

    pub fn execute_from_step<R: StreamBoundaryResources>(
        &self,
        resources: &mut R,
        resume_from: Option<StreamBoundaryStep>,
    ) -> Result<StreamBoundaryResetResult, R::Error> {
        self.try_execute_from_step(resources, resume_from)
            .map_err(|failure| failure.error)
    }

    pub fn try_execute_from_step<R: StreamBoundaryResources>(
        &self,
        resources: &mut R,
        resume_from: Option<StreamBoundaryStep>,
    ) -> Result<StreamBoundaryResetResult, StreamBoundaryExecutionError<R::Error>> {
        let steps: &[(
            StreamBoundaryStep,
            fn(&mut R, &StreamBoundaryResetPlan) -> Result<(), R::Error>,
        )] = &[
            (StreamBoundaryStep::AdvanceGeneration, R::advance_generation),
            (
                StreamBoundaryStep::NotifyWorkerBoundary,
                R::notify_worker_boundary,
            ),
            (StreamBoundaryStep::FlushRuntimeIo, R::flush_runtime_io),
            (StreamBoundaryStep::ClearFmq, R::clear_fmq),
            (
                StreamBoundaryStep::ResetPacketPipeline,
                R::reset_packet_pipeline,
            ),
            (
                StreamBoundaryStep::DiscardAvPayloads,
                R::discard_av_payloads,
            ),
...
#[derive(Debug, Clone, Eq, PartialEq)]
pub struct StreamBoundaryResetResult {
    pub reason: StreamBoundaryReason,
    pub demux_id: Option<i32>,
    pub generation: Option<u64>,
    pub attempted_steps: usize,
    pub boundary_failed: bool,
...
impl PendingStreamBoundaryPlan {
    ...
    pub fn try_execute<R: StreamBoundaryResources>(
        &self,
        resources: &mut R,
    ) -> Result<StreamBoundaryResetResult, StreamBoundaryExecutionError<R::Error>> {
...
#[cfg(test)]
impl StreamBoundaryResources for TestStreamBoundaryResources {
    type Error = ();
    fn advance_generation(&mut self, _: &StreamBoundaryResetPlan) -> Result<(), Self::Error> {
        Ok(())
    }
    fn notify_worker_boundary(&mut self, _: &StreamBoundaryResetPlan) -> Result<(), Self::Error> {
        Ok(())
    }
    fn flush_runtime_io(&mut self, _: &StreamBoundaryResetPlan) -> Result<(), Self::Error> {
        Ok(())
    }
    fn clear_fmq(&mut self, _: &StreamBoundaryResetPlan) -> Result<(), Self::Error> {
        Ok(())
    }
    fn reset_packet_pipeline(&mut self, _: &StreamBoundaryResetPlan) -> Result<(), Self::Error> {
        Ok(())
    }
    fn discard_av_payloads(&mut self, _: &StreamBoundaryResetPlan) -> Result<(), Self::Error> {
        Ok(())
    }
    fn reset_dvr_playback(&mut self, _: &StreamBoundaryResetPlan) -> Result<(), Self::Error> {
        Ok(())
    }
    fn commit_generation(&mut self, _: &StreamBoundaryResetPlan) -> Result<(), Self::Error> {
        Ok(())
    }
}
...
mod tests {
    ...
    impl StreamBoundaryResources for FailingBoundaryResources {
        ...
        fn advance_generation(&mut self, _: &StreamBoundaryResetPlan) -> Result<(), Self::Error> {
            self.step(StreamBoundaryStep::AdvanceGeneration)
        }
        fn notify_worker_boundary(
            &mut self,
            _: &StreamBoundaryResetPlan,
        ) -> Result<(), Self::Error> {
            self.step(StreamBoundaryStep::NotifyWorkerBoundary)
        }
        fn flush_runtime_io(&mut self, _: &StreamBoundaryResetPlan) -> Result<(), Self::Error> {
            self.step(StreamBoundaryStep::FlushRuntimeIo)
        }
        fn clear_fmq(&mut self, _: &StreamBoundaryResetPlan) -> Result<(), Self::Error> {
            self.step(StreamBoundaryStep::ClearFmq)
        }
        fn reset_packet_pipeline(
            &mut self,
            _: &StreamBoundaryResetPlan,
        ) -> Result<(), Self::Error> {
            self.step(StreamBoundaryStep::ResetPacketPipeline)
        }
        fn discard_av_payloads(&mut self, _: &StreamBoundaryResetPlan) -> Result<(), Self::Error> {
            self.step(StreamBoundaryStep::DiscardAvPayloads)
        }
        fn reset_dvr_playback(&mut self, _: &StreamBoundaryResetPlan) -> Result<(), Self::Error> {
            self.step(StreamBoundaryStep::ResetDvrPlayback)
        }
        fn commit_generation(&mut self, _: &StreamBoundaryResetPlan) -> Result<(), Self::Error> {
            self.step(StreamBoundaryStep::CommitGeneration)
        }
    }
    ...
    fn pending_stream_boundary_retries_from_failed_step() {
        ...
        let mut first = FailingBoundaryResources {
            fail_once_at: Some(StreamBoundaryStep::ResetPacketPipeline),
            steps: Vec::new(),
        };
        let failure = plan.try_execute_from_step(&mut first, None).unwrap_err();
        assert_eq!(
            failure.result.failed_step,
            Some(StreamBoundaryStep::ResetPacketPipeline)
        );
        let pending = PendingStreamBoundaryPlan::new(plan, failure.result.failed_step);
        let mut retry = FailingBoundaryResources {
            fail_once_at: None,
            steps: Vec::new(),
        };
        ...
        assert_eq!(
            retry.steps.first().copied(),
            Some(StreamBoundaryStep::ResetPacketPipeline)
        );
        assert_eq!(
            retry.steps.last().copied(),
            Some(StreamBoundaryStep::CommitGeneration)
        );
    }
}
