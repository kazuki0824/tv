...
fn fmq_queue_create(num_bytes: usize, configure_event_flag: bool) -> *mut TunerFmqQueue {
    unsafe { native_queue_create(num_bytes, configure_event_flag) }
}

unsafe fn fmq_queue_destroy(queue: *mut TunerFmqQueue) {
    native_queue_destroy(queue);
}

unsafe fn fmq_queue_available_to_read(queue: *const TunerFmqQueue) -> usize {
    native_queue_available_to_read(queue)
}

unsafe fn fmq_queue_available_to_write(queue: *const TunerFmqQueue) -> usize {
    native_queue_available_to_write(queue)
}

unsafe fn fmq_queue_write_checked(
    queue: *mut TunerFmqQueue,
    data: *const u8,
    size: usize,
    out_written: *mut usize,
) -> i32 {
    native_queue_write_checked(queue, data, size, out_written)
}

unsafe fn fmq_queue_read(queue: *mut TunerFmqQueue, data: *mut u8, size: usize) -> usize {
    native_queue_read(queue, data, size)
}

unsafe fn fmq_queue_wake(queue: *mut TunerFmqQueue, bits: u32) -> i32 {
    native_queue_wake(queue, bits)
}

unsafe fn fmq_queue_quantum(queue: *const TunerFmqQueue) -> i32 {
    native_queue_quantum(queue)
}

unsafe fn fmq_queue_flags(queue: *const TunerFmqQueue) -> i32 {
    native_queue_flags(queue)
}

unsafe fn fmq_queue_grantor_count(queue: *const TunerFmqQueue) -> usize {
    native_queue_grantor_count(queue)
}

unsafe fn fmq_queue_grantor_at(
    queue: *const TunerFmqQueue,
    index: usize,
    fd_index: *mut i32,
    offset: *mut i32,
    ...
    native_queue_grantor_at(queue, index, fd_index, offset, extent)
}

unsafe fn fmq_queue_fd_count(queue: *const TunerFmqQueue) -> usize {
    native_queue_fd_count(queue)
}

unsafe fn fmq_queue_dup_fd_at(queue: *const TunerFmqQueue, index: usize) -> i32 {
    native_queue_dup_fd_at(queue, index)
}

unsafe fn fmq_queue_int_count(queue: *const TunerFmqQueue) -> usize {
    native_queue_int_count(queue)
}

unsafe fn fmq_queue_int_at(queue: *const TunerFmqQueue, index: usize, value: *mut i32) -> bool {
    native_queue_int_at(queue, index, value)
}
