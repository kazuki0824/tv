...
#[derive(Debug)]
pub struct DemuxLedger<R = ()> {
    records: BTreeMap<LedgerId, LedgerRecord<R>>,
    next_generation: u64,
}
...
impl<R: Clone> DemuxLedger<R> {
    ...
    fn open_or_recover(&mut self, id: LedgerId) -> Result<Option<R>, LedgerError> {
        match self.records.get_mut(&id) {
            Some(entry) if entry.state == LedgerState::Live && entry.ref_count > 0 => {
                Ok(entry.record.clone())
            }
            Some(entry) if entry.record.is_none() => {
    ...
    fn acquire_ref(&mut self, id: LedgerId) -> Result<R, LedgerError> {
        ...
        if entry.state != LedgerState::Live || entry.ref_count == 0 {
    ...
    fn commit_close(&mut self, id: LedgerId) -> Result<R, LedgerError> {
        ...
        match self.records.remove(&id) {
            Some(entry) => entry.record.ok_or(LedgerError::InvalidState),
            None => Err(LedgerError::NotFound),
        }
    }
    fn rollback_open(&mut self, id: LedgerId) -> Result<(), LedgerError> {
        self.records
            .remove(&id)
            .map(|_| ())
            .ok_or(LedgerError::NotFound)
    ...
    #[cfg(test)]
    fn insert_record(&mut self, id: LedgerId, record: R) -> Result<(), LedgerError> {
        self.create_live(id, record).map(|_| ())
    }
    fn get_record(&self, id: LedgerId) -> Option<R> {
        self.get_live(id).ok()
    }
    #[cfg(test)]
    fn get_record_any_state(&self, id: LedgerId) -> Option<R> {
        self.records.get(&id).and_then(|e| e.record.clone())
    }
    #[cfg(test)]
    fn contains_live(&self, id: LedgerId) -> bool {
        self.records
            .get(&id)
            .is_some_and(|e| e.state == LedgerState::Live && e.ref_count > 0)
    }
    fn records(&self) -> impl Iterator<Item = R> + '_ {
        self.records.values().filter_map(|e| e.record.clone())
    }
    fn first_available<I>(&self, ids: I) -> Option<i32>
    where
        I: IntoIterator<Item = i32>,
    {
        ids.into_iter()
            .find(|id| !self.records.contains_key(&LedgerId(*id)))
    }
    ...
    fn commit_binding(
        ...
        let entry = self.records.get_mut(&id).ok_or(LedgerError::NotFound)?;
        if entry.state != LedgerState::Live {
            return Err(LedgerError::InvalidState);
        }
    ...
    fn clear_binding_if_matches(
        ...
        let entry = self.records.get_mut(&id).ok_or(LedgerError::NotFound)?;
        if entry.bound_frontend_id == Some(frontend_id)
            && entry.bound_frontend_generation == Some(generation)
        {
    ...
    #[cfg(test)]
    fn contains_id_for_test(&self, id: i32) -> bool {
        self.records.contains_key(&LedgerId(id))
    }
}
...
impl DemuxLifecycleTxn {
    pub fn create_live<R: Clone>(
        ledger: &mut DemuxLedger<R>,
        id: LedgerId,
        record: R,
    ) -> Result<u64, LedgerError> {
        ledger.create_live(id, record)
    }
    pub fn get_record<R: Clone>(ledger: &DemuxLedger<R>, id: LedgerId) -> Option<R> {
        ledger.get_record(id)
    }
    pub fn get_live<R: Clone>(ledger: &DemuxLedger<R>, id: LedgerId) -> Result<R, LedgerError> {
        ledger.get_live(id)
    }
    pub fn acquire_ref<R: Clone>(
        ledger: &mut DemuxLedger<R>,
        id: LedgerId,
    ) -> Result<R, LedgerError> {
        ledger.acquire_ref(id)
    }
    pub fn begin_close<R: Clone>(
        ledger: &mut DemuxLedger<R>,
        id: LedgerId,
    ) -> Result<DemuxCloseAction<R>, LedgerError> {
        ledger.begin_close_ref(id)
    }
    pub fn rollback_open<R: Clone>(
        ledger: &mut DemuxLedger<R>,
        id: LedgerId,
    ) -> Result<(), LedgerError> {
        ledger.rollback_open(id)
    }
    pub fn quarantine<R: Clone>(
        ledger: &mut DemuxLedger<R>,
        id: LedgerId,
    ) -> Result<(), LedgerError> {
        ledger.quarantine(id)
    }
    pub fn commit_close<R: Clone>(
        ledger: &mut DemuxLedger<R>,
        id: LedgerId,
    ) -> Result<R, LedgerError> {
        ledger.commit_close(id)
    }
    pub fn mark_cleanup_failed<R: Clone>(
        ledger: &mut DemuxLedger<R>,
        id: LedgerId,
        step: DemuxCleanupStep,
    ) -> Result<(), LedgerError> {
        ledger.mark_cleanup_failed(id, step)
    }
    pub fn mark_cleanup_progress<R: Clone>(
        ledger: &mut DemuxLedger<R>,
        id: LedgerId,
        step: DemuxCleanupStep,
    ) -> Result<(), LedgerError> {
        ledger.mark_cleanup_progress(id, step)
    }
    pub fn generation<R: Clone>(ledger: &DemuxLedger<R>, id: LedgerId) -> Option<u64> {
        ledger.generation(id)
    }
    pub fn current_binding<R: Clone>(
        ledger: &DemuxLedger<R>,
        id: LedgerId,
    ) -> Result<(Option<i32>, Option<u64>), LedgerError> {
        ledger.current_binding(id)
    }
    pub fn commit_binding<R: Clone>(
        ledger: &mut DemuxLedger<R>,
        id: LedgerId,
        frontend_id: Option<i32>,
        generation: Option<u64>,
    ) -> Result<(), LedgerError> {
        ledger.commit_binding(id, frontend_id, generation)
    }
    pub fn clear_binding_if_matches<R: Clone>(
        ledger: &mut DemuxLedger<R>,
        id: LedgerId,
        frontend_id: i32,
        generation: u64,
    ) -> Result<bool, LedgerError> {
        ledger.clear_binding_if_matches(id, frontend_id, generation)
    }
    pub fn first_available<R: Clone, I>(ledger: &DemuxLedger<R>, ids: I) -> Option<i32>
    where
        I: IntoIterator<Item = i32>,
    {
        ledger.first_available(ids)
    }
    pub fn records<R: Clone>(ledger: &DemuxLedger<R>) -> impl Iterator<Item = R> + '_ {
        ledger.records()
    }
}
...
impl ResourceLedger {
    fn reserve(&mut self, id: LedgerId) -> Result<u64, LedgerError> {
        ...
        self.generation.insert(id, self.next_generation);
        self.cleanup_step.remove(&id);
        Ok(self.next_generation)
    }
    fn commit_open(&mut self, id: LedgerId) -> Result<(), LedgerError> {
    ...
    fn begin_close(&mut self, id: LedgerId) -> Result<u64, LedgerError> {
        if self.closing.contains(&id) || self.quarantined.contains(&id) {
            return Ok(*self.generation.get(&id).unwrap_or(&0));
        }
    ...
    fn commit_close(&mut self, id: LedgerId) -> Result<(), LedgerError> {
        if !self.closing.remove(&id) && !self.quarantined.remove(&id) {
            return Err(LedgerError::NotFound);
        }
        self.generation.remove(&id);
        self.cleanup_step.remove(&id);
    ...
    fn cleanup_step(&self, id: LedgerId) -> Option<&'static str> {
        self.cleanup_step.get(&id).copied()
    }
...
#[derive(Debug, Default)]
pub struct DvrLedger {
    inner: ResourceLedger,
}
#[derive(Debug, Default)]
pub struct DescramblerLedger {
    inner: ResourceLedger,
}
macro_rules! resource_ledger_api_with_cleanup {
    ($t:ty) => {
        impl $t {
            ...
            fn commit_open(&mut self, id: LedgerId) -> Result<(), LedgerError> {
                self.inner.commit_open(id)
            }
            fn rollback_open(&mut self, id: LedgerId) -> Result<(), LedgerError> {
                self.inner.rollback_open(id)
            }
            fn begin_close(&mut self, id: LedgerId) -> Result<u64, LedgerError> {
                self.inner.begin_close(id)
            }
            fn quarantine(&mut self, id: LedgerId) -> Result<(), LedgerError> {
                self.inner.quarantine(id)
            }
            fn commit_close(&mut self, id: LedgerId) -> Result<(), LedgerError> {
                self.inner.commit_close(id)
            }
            fn generation(&self, id: LedgerId) -> Option<u64> {
                self.inner.generation(id)
            }
            fn cleanup_step(&self, id: LedgerId) -> Option<&'static str> {
                self.inner.cleanup_step(id)
            }
            fn mark_cleanup_step(
                &mut self,
                id: LedgerId,
                step: &'static str,
            ) -> Result<(), LedgerError> {
                self.inner.mark_cleanup_step(id, step)
            }
        }
    };
}
...
impl ResourceLifecycleTxn {
    pub fn reserve_filter(ledger: &mut FilterLedger, id: LedgerId) -> Result<u64, LedgerError> {
        ledger.reserve(id)
    }
    pub fn commit_filter_open(ledger: &mut FilterLedger, id: LedgerId) -> Result<(), LedgerError> {
        ledger.commit_open(id)
    }
    pub fn rollback_filter_open(
        ledger: &mut FilterLedger,
        id: LedgerId,
    ) -> Result<(), LedgerError> {
        ledger.rollback_open(id)
    }
    pub fn begin_filter_close(ledger: &mut FilterLedger, id: LedgerId) -> Result<u64, LedgerError> {
        ledger.begin_close(id)
    }
    pub fn quarantine_filter(ledger: &mut FilterLedger, id: LedgerId) -> Result<(), LedgerError> {
        ledger.quarantine(id)
    }
    pub fn commit_filter_close(ledger: &mut FilterLedger, id: LedgerId) -> Result<(), LedgerError> {
        ledger.commit_close(id)
    }
    pub fn filter_cleanup_step(ledger: &FilterLedger, id: LedgerId) -> Option<&'static str> {
        ledger.cleanup_step(id)
    }
    pub fn filter_generation(ledger: &FilterLedger, id: LedgerId) -> Option<u64> {
        ledger.generation(id)
    }
    pub fn mark_filter_cleanup_step(
        ledger: &mut FilterLedger,
        id: LedgerId,
        step: &'static str,
    ) -> Result<(), LedgerError> {
        ledger.mark_cleanup_step(id, step)
    }

    pub fn reserve_dvr(ledger: &mut DvrLedger, id: LedgerId) -> Result<u64, LedgerError> {
        ledger.reserve(id)
    }
    pub fn commit_dvr_open(ledger: &mut DvrLedger, id: LedgerId) -> Result<(), LedgerError> {
        ledger.commit_open(id)
    }
    pub fn rollback_dvr_open(ledger: &mut DvrLedger, id: LedgerId) -> Result<(), LedgerError> {
        ledger.rollback_open(id)
    }
    pub fn begin_dvr_close(ledger: &mut DvrLedger, id: LedgerId) -> Result<u64, LedgerError> {
        ledger.begin_close(id)
    }
    pub fn quarantine_dvr(ledger: &mut DvrLedger, id: LedgerId) -> Result<(), LedgerError> {
        ledger.quarantine(id)
    }
    pub fn commit_dvr_close(ledger: &mut DvrLedger, id: LedgerId) -> Result<(), LedgerError> {
        ledger.commit_close(id)
    }
    pub fn dvr_cleanup_step(ledger: &DvrLedger, id: LedgerId) -> Option<&'static str> {
        ledger.cleanup_step(id)
    }
    pub fn dvr_generation(ledger: &DvrLedger, id: LedgerId) -> Option<u64> {
        ledger.generation(id)
    }
    pub fn mark_dvr_cleanup_step(
        ledger: &mut DvrLedger,
        id: LedgerId,
        step: &'static str,
    ) -> Result<(), LedgerError> {
        ledger.mark_cleanup_step(id, step)
    ...
    pub fn reserve_descrambler(
        ledger: &mut DescramblerLedger,
        id: LedgerId,
    ) -> Result<u64, LedgerError> {
        ledger.reserve(id)
    }
    pub fn commit_descrambler_open(
        ledger: &mut DescramblerLedger,
        id: LedgerId,
    ) -> Result<(), LedgerError> {
        ledger.commit_open(id)
    }
    pub fn rollback_descrambler_open(
        ledger: &mut DescramblerLedger,
        id: LedgerId,
    ) -> Result<(), LedgerError> {
        ledger.rollback_open(id)
    }
    pub fn begin_descrambler_close(
        ledger: &mut DescramblerLedger,
        id: LedgerId,
    ) -> Result<u64, LedgerError> {
        ledger.begin_close(id)
    }
    pub fn quarantine_descrambler(
        ledger: &mut DescramblerLedger,
        id: LedgerId,
    ) -> Result<(), LedgerError> {
        ledger.quarantine(id)
    }
    pub fn commit_descrambler_close(
        ledger: &mut DescramblerLedger,
        id: LedgerId,
    ) -> Result<(), LedgerError> {
        ledger.commit_close(id)
    }
    pub fn descrambler_generation(ledger: &DescramblerLedger, id: LedgerId) -> Option<u64> {
        ledger.generation(id)
    }
}

#[derive(Debug, Default)]
pub struct LnbLedger;
            ...
            match locks.lock() {
                Ok(mut active) => {
                    if !active.remove(&self.lnb_id) {
                        record_lnb_operation_failure(
                            self.lnb_id,
                            "lnb_operation_guard_release_missing",
                        );
                    }
                }
                Err(_) => {
                    record_lnb_operation_failure(
                        self.lnb_id,
                        "lnb_operation_guard_release_poisoned",
                    );
                }
            }
        } else {
...
impl LnbLedger {
    pub fn operation_guard(lnb_id: i32) -> Result<LnbOperationGuard, LnbOperationGuardError> {
        ...
        {
            let mut failures = lnb_operation_failures()
                .lock()
                .map_err(|_| LnbOperationGuardError::Poisoned)?;
        ...
        let mut active = locks
            .lock()
            .map_err(|_: HalLockError| LnbOperationGuardError::Poisoned)?;
    ...
    pub fn operation_failure_diagnostic(lnb_id: i32) -> Option<&'static str> {
        lnb_operation_failures()
            .lock()
            .ok()
            .and_then(|failures| failures.get(&lnb_id).copied())
    }
...
#[derive(Debug, Default)]
pub struct FrontendBindingLedger;
...
mod tests {
    ...
    fn demux_ledger_owns_record_and_live_id_together() {
        ...
        assert!(matches!(
            ledger.begin_close_ref(LedgerId(3)),
            Ok(DemuxCloseAction::Final {
    ...
    fn demux_ledger_cleanup_failure_retries_from_recorded_step() {
        ...
        assert!(matches!(
            ledger.begin_close_ref(LedgerId(4)),
            Ok(DemuxCloseAction::Final {
                next_step: DemuxCleanupStep::UnbindFrontend,
                ..
            })
        ...
        assert_eq!(
            ledger.mark_cleanup_failed(LedgerId(4), DemuxCleanupStep::CloseHandle),
            Ok(())
        );
        assert!(matches!(
            ledger.begin_close_ref(LedgerId(4)),
            Ok(DemuxCloseAction::Final {
                next_step: DemuxCleanupStep::CloseHandle,
                ..
            })
        ));
    }
    ...
    fn demux_ledger_quarantine_blocks_id_reuse_until_close_commit() {
        ...
        assert!(matches!(
            ledger.begin_close_ref(LedgerId(5)),
            Ok(DemuxCloseAction::Final { .. })
        ));
        assert_eq!(ledger.quarantine(LedgerId(5)), Ok(()));
        assert_eq!(
            ledger.create_live(LedgerId(5), 51),
    ...
    fn demux_ledger_removed_only_after_cleanup_success() {
        ...
        assert!(matches!(
            ledger.begin_close_ref(LedgerId(903)),
            Ok(DemuxCloseAction::Final { .. })
        ));
        assert_eq!(
    ...
    fn demux_quarantine_excluded_from_open_demux_by_id() {
        ...
        assert_eq!(
            ledger.acquire_ref(LedgerId(908)),
            Err(LedgerError::InvalidState)
        );
        assert_eq!(
            ledger.create_live(LedgerId(908), 9081),
            Err(LedgerError::AlreadyExists)
        );
    }
    ...
    fn frontend_unbind_boundary_failure_keeps_binding() {
        ...
        assert_eq!(
            ledger.commit_binding(LedgerId(906), Some(10), Some(77)),
            Ok(())
        );
        ...
        assert_eq!(
            ledger.current_binding(LedgerId(906)),
            Ok((Some(10), Some(77)))
        );
    }
    ...
    fn frontend_unbind_boundary_success_clears_binding() {
        ...
        assert_eq!(
            ledger.commit_binding(LedgerId(907), Some(10), Some(77)),
            Ok(())
        );
...
mod r50ea3_filter_dvr_cleanup_step_tests {
    ...
    fn filter_ledger_records_cleanup_step_for_retry() {
        ...
        assert_eq!(
            ledger.mark_cleanup_step(LedgerId(20), "runtime_unregister_filter"),
            Ok(())
        );
        assert_eq!(
            ledger.cleanup_step(LedgerId(20)),
    ...
    fn dvr_ledger_records_cleanup_step_for_retry() {
        ...
        assert_eq!(
            ledger.mark_cleanup_step(LedgerId(30), "demux_unregister_dvr"),
            Ok(())
        );
        assert_eq!(
            ledger.cleanup_step(LedgerId(30)),
            Some("demux_unregister_dvr")
        );
