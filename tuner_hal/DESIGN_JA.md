...
| STD-B32 | 4.1 | 3.11-E1 英語版 Fascicle 1・2・3 | `差分未証明` | Fascicle 1 Chapter 5 5.1.1、Attachment 2 Chapter 5 5.1、Attachment 5 Chapter 5 5.1.1 / MPEG-2・AVC・HEVC video PESのPTS明示。Fascicle 2 Attachment Chapter 2 2.1 / audio parameter切替後の先頭frameへのPTS。Fascicle 3 Chapter 3 3.1 / PES構文 | 本書 |
...
一方、同3.11-E1 Fascicle 2 Attachment Chapter 2 2.1のaudio規定はparameter切替等の境界における先頭frameへのPTSを要求するが、全audio PESへの明示を保証しない。現行backendはPTSのないaudio media outputへ対応するauthoritative timestampを持たないため、製品既定profileは`numAudioFilter=0`を維持する。将来有効化する条件は、backendまたはextractorがevent/output境界、generation/discontinuity、33-bit wrapを含む対応関係を確定し、PTS-header由来でない値では`isPtsPresent=false`を維持したまま本節の最低試験を通すことである。`getAvSyncTime()`用のPCR/wallclock補間を個別eventのPTSとして流用しない。
