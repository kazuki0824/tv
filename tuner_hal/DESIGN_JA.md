...
`releaseAvHandle()`の入力分類、戻り値、資源変化、再release、close後releaseは「表1-C-AVH. `releaseAvHandle()` 全域判定表」だけを正本とし、他節では再定義しない。
...
フィルターの通常FMQペイロード、DVR記録ストリーム、TS記録コールバックのメタデータは、互いに独立した3つの経路として扱う。TS/MMTP記録フィルターは通常のフィルターFMQを公開しない。ペイロードは接続先のRecord DVR FMQだけへ書き込み、PID、索引、バイト番号、PTS、開始コードのメタデータは `DemuxFilterTsRecordEvent` のコールバックで通知する。Section、PES、TS生データのペイロードフィルターは通常のフィルターFMQを使用する。PESは有効な明示`streamId 0..255`と`0xFFFF`は`INVALID_STREAM_ID`として拒否する値 `0xFFFF`を同じ能力で受理し、映像`0xE0..0xEF`の長さ0 PESもruntime組立て対象とする。
...
| DVR-012 | `stop()` record | D2 | 成功 | D3 | record productionを停止し、Record DVR FMQへ確定済みの未消費データとqueue identityを維持する。stop前のRecord-path partial由来結果を次start後の結果へ連結・配送しない。record parser / index / assemblerのboundary mutationは`StreamBoundaryTxn`参照 | `dvr_stop_success` | client-visible queued dataは`stop()`で破棄せず、未消費Record DVR FMQの破棄は`flush()`だけが行う |
...
| 2 | PES | 有効なPES設定を一般に受理 | 明示`streamId 0..255`とwildcard `0xFFFF`のPES能力を宣言する | `openFilter()`は成功。`configure()`は全有効stream IDと`0xFFFF`は`INVALID_STREAM_ID`として拒否する値を成功させる。映像`0xE0..0xEF`の長さ0 PESもruntimeで扱う | 表1のFMQ対象状態に従う | 通常FMQ + `DemuxFilterPesEvent` | TISのARIB字幕経路は利用設定として`0xBD`を指定する。video/audio本体はAV filter経路を使用してよい |
...
| raw section | true | 外形完全、CRC一致 | 元のsection全体を配送 | `IFilterCallback.onFilterStatus(DATA_READY)`（EventFlagは追加のFMQ wakeとして使用可） | 生成しない | 正常配送を記録 |
| raw section | true | 外形完全、CRC不一致またはCRC検査に必要な末尾不足 | しない | データ到着としては通知しない | 生成しない | CRC破棄を記録し、overflowへ写像しない |
| raw section | false | 外形完全。CRC、予約ビット、表固有構文のいずれかが不正 | 元のsection全体を配送 | `IFilterCallback.onFilterStatus(DATA_READY)`（EventFlagは追加のFMQ wakeとして使用可） | 生成しない | 不正内容とraw配送を併記 |
...
|---:|---|---|---|---|---|---|---|---|---|---|---|---|---|
| AVM-001 | `configure(AV)` | A8, A9, A10, A11 | 成功 | 入力状態を維持 | 入力状態を維持 | なし | 未発行 | 補助種別を維持し、routing種別をopen subtypeから導出 | 累積カウンタを変更しない | 不可 | 設定状態軸だけ設定済みに変更 | 設定前に取得可能な共有ハンドルを無効化せず、TsAudio/TsVideoは補助種別未設定でもrouting可能 | 設定状態以外の軸を維持する |
...
|---|---|---|---|---|---|
| Frontend | `LogicalClosed` / `CleanupPending` / `Quarantined` | `close()`および0-S-3B `ObjectCloseTxn`のrecovery/cleanup入口を除く全ての通常public method | `INVALID_STATE`または各recovery契約 | hard-coded method列挙を第二正本にしない |
...
| clear / `flush()` | API固有のeligibilityとflush対象は表1・表2で確定し、内部cleanupは`QueueCleanupTxn`へ渡す | `FilterProducerDrainGate` / `QueueEpochProtocol`のcanonical contractを参照し、本表ではcommit pointを再定義しない | 下位protocolのtyped resultと診断は0-S-3Bに従う | 公開結果は表1・表2に従い、本表からrollback / retry semanticsを追加しない | queue identityとdescriptor、およびAPI表が維持すると定めた資源を維持する |
| playback read | 公開 data-path の入力範囲と予約済み processing-buffer 容量を検証する | read / parse / inject / consume の内部確定点は0-S-3Bの`PlaybackConsumeTxn`参照 | partial inject、retryable / fatal failureの内部処理は同契約参照 | worker結果と損失診断はDVR公開契約と`PlaybackConsumeTxn`に従う | processing-buffer / cursor のphase・stateを本表では再定義しない |

`CleanupPending` の generic retry / reaper / generation fence / lease return / terminal failure branching は 0-S-3B の `WorkerRuntime` / `WorkerHandle` を唯一の正本とし、public close 由来の cleanup authority は `ObjectCloseTxn` を正とする。本節では FMQ / data-path 固有の公開結果と診断だけを定める。

Playback の read / parse / inject / consume cursor、retry、stop / flush / close 時の処理は 0-S-3B の `PlaybackConsumeTxn` を唯一の正本とし、本節では再定義しない。
...
本表は、複数の所有者または外部副作用をまたぐ操作について、API固有の公開対象・公開結果・必要な順序と、その論理契約の索引先を固定する。単一object内で完結する戻り値と状態遷移は表1、表2、表4、表5、表D-1を正とし、本表へ複製しない。0-S-3Bに同名transactionがある操作では、transaction内部のphase、commit、rollback / cleanup、failure semanticsを本表で再定義せず0-S-3Bを正とする。
...
| AT-007 | 複数demux stream boundary | frontend tune / scan / stopに伴う対象一覧固定・per-demux境界実行・結果集約の内部phaseはcanonical `frontend tune/scan`、各demuxのboundary mutationは`StreamBoundaryTxn`参照 | caller-visibleな成功/失敗条件は表0-F / 表19、内部確定点はcanonical契約参照 | rollback / partial failure semanticsはcanonical契約参照 | 同契約参照 | 表0-F / 表19に従う | 一部失敗を全体rollbackで隠さず、内部batch state machineを本索引で再定義しない |
...
`addPid()` / `removePid()` の PID / source 入力分類と公開 status は表D-1を正とする。claim、backend apply、ledger commit、compensation を含む内部 mutation は 0-S-3B の `DescramblerPidTxn` だけを正本とし、本節では再定義しない。


| 番号 | 操作 / 事象 | 変更順序 | 成功の確定点 | 確定点前の失敗 | 巻き戻し不能時の対象 | 公開戻り値 / 作業スレッド終了 | 設計上の成立条件 |
|---:|---|---|---|---|---|---|---|
| AT-011 | `ILnb.setVoltage()` / `setTone()` / `setSatellitePosition()` | operation/valueごとの公開入力・戻り値・未対応判定は「ILnb公開操作」を正とし、persistent control内部transactionは0-S-3Bの`LnbControlTxn`参照 | 公開成功条件は「ILnb公開操作」、内部確定点は0-S-3B参照 | backend/registry failure、quarantine semanticsは0-S-3B参照 | 0-S-3B参照 | 「ILnb公開操作」と`LnbControlTxn`に従う | 3 APIの内部control state machineを本索引で再定義せず、`sendDiseqcMessage()`を同transactionへ吸収しない |
| AT-012 | `IFrontend.setLnb()` | satellite frontendへhardware LNB resource assignmentを設定・更新する公開操作として扱う。assignment/leaseの内部transactionは0-S-3Bの`FrontendLnbRelationTxn`参照 | 指定assignmentが公開成功状態になったことをAPI成功条件とし、内部composite commit点は0-S-3B参照 | rollback / commit不明 / post-commit旧lease cleanup semanticsは0-S-3B参照 | 0-S-3B参照 | `IFrontend.setLnb()`公開契約と`FrontendLnbRelationTxn`に従う | assignmentは複数回更新可能でFrontend close時に解放するAOSP契約を維持し、relationとleaseを別commitしない |


### 表8. 資源寿命・所有権・破棄失敗表

本表は、公開objectの状態表だけでは寿命を表せない長寿命資源について、owner、取得契機、最終解放条件、解放結果を確定できない場合の非再利用だけを固定する。transaction phase、retry / handoff authority、quarantine / `ServiceCritical`判定は0-S-3Bの各canonical contractを正とし、本表では再定義しない。短命な局所変数と、単一APIの排他区間だけで生存する準備物は対象外とする。

| 番号 | 資源 | 所有者 | 作成 / 取得 | 通常破棄 | 異常時破棄契機 | 破棄失敗時 | 設計上の成立条件 |
|---:|---|---|---|---|---|---|---|
| RL-002 | `frontend_operation_generation` | `FrontendRuntime`。発行・fenceはcanonical `frontend tune/scan` | tune/scan operation開始時に取得 | operation終端 / replacementでfence | fence結果を確定できない場合は再利用せずcanonical frontend failure契約へ接続 |
| RL-003 | `stream_boundary_generation` | `StreamBoundaryTxn` | stream boundary prepare時に取得 | boundary replacement / object cleanupで失効 | 発行・fence不明時は再利用せず`StreamBoundaryTxn`の局所failure契約へ接続 |
...
| RL-006 | callback artifactとregistry entry | callback artifact store / `RuntimeCallbackRegistry` / domain logical owner | callback registration成功時に各ownerの寿命を開始 | registration解除またはobject固有cleanup完了時に各ownerから解放 | Binder死亡、registry障害、解放結果不明 | 解放結果を確認できないartifact / entryを再利用しない。登録transaction semanticsは`CallbackRegistrationUseCase`参照 | artifact / runtime / domainの寿命ownerを混同しない |
| RL-007 | workerとworker使用資源 | `WorkerRuntime` / `WorkerHandle` | domain ownerがworkerを開始した時 | workerの実終了と当該workerが保持する依存使用権の解放完了時 | abnormal exit、終了結果不明、依存使用権の解放結果不明 | 実終了または解放を確認できないworker slot、FD、endpointを再利用しない。stop / wake / join / reaper / fence semanticsは`WorkerRuntime` / `WorkerHandle`参照 | 実終了前に使用枠を再利用しない |
| RL-008 | descrambler key、token、PID claim | key table / `DescramblerSession` | key / token / PIDの公開寿命開始時 | refcount、session、PID relationの公開寿命終了時 | backend解除失敗、台帳不整合、解放結果不明 | 解放を確認できないclaimまたはkey参照を再割当てしない。mutation / cleanup semanticsはDescrambler canonical txns参照 | key material、opaque token、PID claimを別寿命にする |
| RL-009 | backend FDとdevice endpoint | frontend / LNB / demuxのbackend adapter | probe / open reservationの確定時 | backend resourceの利用終了を確認できた時 | close / I/O failure、残存使用者、解放結果不明 | 解放を確認できないFD / endpointを新generationへ再利用しない。worker failure / terminal branchingは`WorkerRuntime` / `WorkerHandle` / `WorkerFailureClassifier`参照 | 実終了前に同じ専有endpointを新世代へ渡さない |
| RL-010 | close cleanup lifetime | `ObjectCloseTxn` | objectのlogical close開始時 | object固有cleanupが全て完了した時 | cleanup未完、解放結果不明 | cleanup完了を確認できない依存資源を再利用しない。authority / retry / handoff / quarantine semanticsは`ObjectCloseTxn`参照 | logical closeとphysical resource final releaseを分離する |
...
表8でopen時生成と明記したobject-owned physical backingだけがopen起点の寿命を持つ。callback artifact、descrambler key/PID claim、AV allocation、capacity reservation等は表8の取得契機に従う。logical close / flush / configureによるlogical state・generationの無効化と、physical backing / allocationの最終解放は分離し、通常破棄・最終解放条件は表8および各canonical owner契約を正とする。
...
| 番号 | 資源 | 所有者 | 作成 / 取得 | 通常破棄 | 異常時破棄契機 | 破棄失敗時 | 設計上の成立条件 |
|---:|---|---|---|---|---|---|---|
| RL-011 | LNB registry state | `LnbRegistry`。persistent mutation ownerは`LnbControlTxn` | LNB open / set系APIによりstate lifetimeを開始 | `ILnb.close()`のobject固有cleanup完了時 | backend反映失敗、registry障害、実状態不明 | backend / registryの解放または実状態を確認できないendpointを再利用しない。control failure semanticsは`LnbControlTxn`参照 | registry状態とbackend状態を成功扱いで乖離させない |
...
source filter boundaryはdownstream lifecycle、queued payload、pending event、stream/parser boundary、DVR attachを分けて扱う。relation mutationは`SourceBoundaryTxn`、stream/parser boundaryは`StreamBoundaryTxn`を唯一の正本とし、本表ではprepare / commit / rollback、relation generation、parser / assembler mutationを再定義しない。

| 操作 | downstream lifecycle / relationの公開意味 | queued payload | pending event | stream/parser boundary | DVR attach | public状態 |
|---|---|---|---|---|---|---|
| `setDataSource(NULL)` | downstreamを維持し、入力元をdemux inputへ戻す。relation mutationは`SourceBoundaryTxn`参照 | 旧source由来のcommit前entryを境界後に配送しない | 旧source由来の未配送eventを境界後に配送しない | `StreamBoundaryTxn`参照 | downstream filterのRecord DVR接続は維持 | 非開始状態を維持して成功。開始中は`INVALID_STATE` |
| non-null sourceの新規接続 | sourceとsinkの公開eligibilityを検証し、成功時は指定sourceを入力元とする。relation mutationは`SourceBoundaryTxn`参照 | 接続前originのcommit前entryを新originとして配送しない | 接続前originの未配送eventを新originとして配送しない | `StreamBoundaryTxn`参照 | sinkのRecord DVR接続は維持 | 非開始状態を維持して成功。開始中は`INVALID_STATE` |
| non-null sourceの置換 | 成功時は新sourceを入力元とする。relation mutationと失敗時状態は`SourceBoundaryTxn`参照 | 旧source由来のcommit前entryを新source由来として配送しない | 旧source由来の未配送eventを新source由来として配送しない | `StreamBoundaryTxn`参照 | sinkのRecord DVR接続は維持 | 成否と状態は表1-Dおよび`SourceBoundaryTxn`に従う |
| source filter `flush()` | downstreamの公開lifecycleと接続を維持 | 当該sourceのflush対象となるcommit前entryを境界後に配送しない | 当該sourceのflush対象となる未配送eventを境界後に配送しない | `FilterProducerDrainGate` / `QueueCleanupTxn` / `StreamBoundaryTxn`参照 | 変更なし | downstreamを自動停止・失敗にしない |
| source filter再設定 | downstreamの接続を維持し、公開互換性を再検証する | 旧設定由来のcommit前entryを新設定由来として配送しない | 旧設定由来の未配送eventを新設定由来として配送しない | `StreamBoundaryTxn`参照 | 変更なし | source開始中の再設定は拒否し、その他は表1に従う |
| source filter実行失敗 | downstreamはsource lostを観測し、新規配送を停止する | source由来のcommit前entryを以後配送しない | source由来の未配送eventを以後配送しない | `StreamBoundaryTxn`参照 | 変更なし | downstreamを自動failedにしない |
| source filter close / unlink | source relationを解除し、downstreamはsource lostを観測する。relation cleanupは`SourceBoundaryTxn`参照 | source由来のcommit前entryを以後配送しない | source由来の未配送eventを以後配送しない | `StreamBoundaryTxn`参照 | relation unlinkだけではsource自身のRecord DVR接続を変更しない。source close時は表5に従う | downstreamを自動failedにしない。閉鎖済みsourceの再指定は`INVALID_STATE` |
| downstream `stop()` | source接続を維持 | client-visible queueの扱いは表1を正とする | pending eventの扱いは表1を正とする | stop時のboundaryは`StreamBoundaryTxn`参照 | 変更なし | stopの状態表に従う |
| downstream `close()` | source relationをcleanup対象とする | downstream所有dataの寿命は表5を正とする | downstream所有eventの寿命は表5を正とする | `StreamBoundaryTxn`参照 | downstream自身のRecord DVR接続は表5に従う | 表5に従う |
| upstream generation mismatch | relationを変更しない | stale entryを配送しない | stale eventを配送しない | `StreamBoundaryTxn`参照 | 変更なし | runtime failedにはしない |
...
`scan END配送use-case` の戻り値を callback配送結果の破棄 で捨ててはならない。
...
queue 容量は profile 依存にできる構造にする。VTS/lab profile の小容量で overflow test を行えることと、product profile で record / DVR raw TS を大容量化できることの両方を満たす。overflow 時に古いデータを捨てるか新しいデータを捨てるかは用途別に固定し、ライブ AV の 旧データ破棄 方針を TS raw / section / PES / 録画経路 に流用してはならない。Filter/DVR queueの論理overflow方針、容量、producer/consumer結果だけを本書で規定する。具体helper/type/alias名は`CODE_CONVENTION.md`を正本とし、公開契約のSSOTにしない。
...
boundary処理は0-S-3Bの`StreamBoundaryTxn`を唯一の正本とする。`StreamBoundaryTxn`はstream/parser boundaryとprepared invalidation dispatchを所有し、Filter/DVR queue、A/V sync relation、PCR anchor、callback registration等の内部stateを直接変更しない。各下位ownerへtyped invalidationを渡し、本節ではそれらの内部mutationを再定義しない。

| 対象 | 処理 |
|---|---|
| Filter / DVR queue | 旧stream generation由来dataを新generationとして配送しないためのtyped invalidationをqueue ownerへ渡す。物理queue mutationは各queue ownerのcanonical contractを正とする |
| Event / wake経路 | boundary後にstale eventを配送しないためのtyped generation fenceを該当ownerへ渡す。wake / delivery failureの分類は各ownerと`WorkerFailureClassifier`を正とする |
...
| 対象 | 処理 |
|---|---|
| section / PES parser output | 旧boundaryで開始されたpartial outputを新boundaryへ連結・配送しない。内部parser/assembler mutationは`StreamBoundaryTxn`を正とする |
| continuity | 旧boundaryのcontinuity状態を新boundaryの判定へ混在させない。内部mutationは`StreamBoundaryTxn`を正とする |
| source filter origin | frontend / playback / source-filterのoriginを混在させない。origin boundaryの内部mutationは`StreamBoundaryTxn`を正とする |
| record queue | 旧boundary dataを新boundary dataとして扱わないためのtyped invalidationをqueue ownerへ渡す |
| callback | old stream generation由来eventを配送しないためのtyped generation fenceをcallback ownerへ渡す |

1つのfilter flushが、同じsource origin/PIDを共有する無関係なfilterのassemblerまたはcontinuityを壊してはならない。


#### 表SB-1. 複数 demux boundary 公開結果・診断表

frontend tune / scan / stopに伴って複数demuxへstream boundaryが必要な場合、本表はcaller-visibleな結果と診断だけを固定する。対象一覧固定、per-demux実行順序、commit / abort、未処理集合、retry、quarantine、aggregate failureの内部semanticsはcanonical `frontend tune/scan`、各demuxのstream / parser mutationは`StreamBoundaryTxn`だけを正本とし、本表では再定義しない。

| caller-visible状況 | 公開API / frontend状態 | data / callback可視性 | 診断 |
|---|---|---|---|
| 全対象boundary成功 | 操作固有の成功状態へ進む | 成功後に旧operation / 旧stream generation由来のdata・callbackを新結果として配送しない | 対象demuxごとのtyped boundary success |
| 1件以上のboundary失敗 | 表0-F / 表19の原因別エラーと`FailedBoundary`等へ写像する | boundary失敗を成功済みとして公開せず、旧generationの結果を新operation結果として復元しない | demuxごとのtyped result、commit可否、隔離根拠 |
| stale結果を遮断できない | `UNKNOWN_ERROR` / `Quarantined` | 遮断不能な依存から新operation結果を公開しない | 遮断不能demux / generation / dependencyを記録 |

一部demuxの失敗だけを理由に、無関係なdemuxまたはservice全体を隔離しない。どのdemuxが変更済みか、未変更か、commit結果不明かの判定と後続retry / isolationはcanonical `frontend tune/scan` / `StreamBoundaryTxn`を正とする。
...
- `byteNumber`は当該Record Filterのoutput lifetime先頭から、Record DVRへ実際にcommit済みのTS byte数を累積した`record_output_byte_offset`とする。configure/reconfigure、`flush()`、source/stream boundary、parser resetでは0へ戻さず、新しいFilter object/output lifetimeだけ0から開始する。
...
PES filterは、有効な明示`streamId 0..255`と`0xFFFF`は`INVALID_STREAM_ID`として拒否する値 `0xFFFF`を同じ公開能力で受理する。先頭6 byteを検証後、宣言長ありPESではassemblerが`PES_packet_length + 6` byteだけをservice共通台帳からclaimする。映像`stream_id 0xE0..0xEF`の`PES_packet_length == 0`は、同一PIDの次PUSIまでを収集し、`MAX_PES_BUFFER_BYTES`を上限とする。起動前に`ProductProfile`を検証して`CapabilitySnapshot.pesRuntimeBudgetBytes`へ固定し、その値を`MAX_PES_BUFFER_BYTES * CapabilitySnapshot.pesFilterCount`以上とする。各filterは同時に1 assemblerだけを所有するため、公開個数上限まで最大保持量を同時に保証できる。
...
| 入力元 | 意味 | 世代キー |
|---|---|---|
| `Frontend` | frontend backendから来るlive TS | `Frontend(frontend_operation_generation)` |
| `PlaybackDvr` | playback DVR input FMQから読み、demuxへ投入するTS | `PlaybackDvr(dvr_id, queue_identity, queue_epoch)` |
| `SourceFilter` | `IFilter.setDataSource()`により、上流raw TS filter出力を下流filterへ再投入するTS | `SourceFilter(filter_id, filter_delivery_generation)` |
...
soft-demux で必要な `stream_boundary_generation`とsteady-state ownerへのtyped reset / invalidate dispatch と、上位 transaction との prepare / commit composition は 0-S-3B の `StreamBoundaryTxn` を唯一の正本とし、本節では再定義しない。以下では入力 origin と generation ごとの data-path 結果だけを定める。

旧input origin / generationで開始されたsection / PES partial outputは、新しいboundary後のoutputへ連結または配送しない。`flush()`、`setDataSource()`、filter close、source unlink、stream boundaryに伴うparser / assembler stateとgenerationのmutationは0-S-3Bの`StreamBoundaryTxn`を唯一の正本とし、本節では再定義しない。
...
- adaptation-only packet に `discontinuity_indicator` が立つ場合は、境界前のsection / PESを境界後へ連結しない。continuity / assemblerのmutationは`StreamBoundaryTxn`参照とする。
...
| T-TS-10a | 同一CC・packet不一致 | raw/recordへ保持し、境界前の意味解析結果を後続へ連結しない。内部boundary mutationは`StreamBoundaryTxn`参照 |
| T-TS-11 | discontinuity_indicator | 境界前後の意味解析結果を連結しない。continuity / assembler mutationは`StreamBoundaryTxn`参照 |
...
| ContinuityDiscontinuity | PIDの連続性とadaptation discontinuity | CC欠落、同じCCで188バイトTS packetが不一致、`discontinuity_indicator` | 保持 | 保持 | 境界前後のsection / PES等を連結しない。assembler / generationのmutationは`StreamBoundaryTxn`参照 | 継続 | なし | PIDと世代を含む不連続診断 | 隔離しない |
