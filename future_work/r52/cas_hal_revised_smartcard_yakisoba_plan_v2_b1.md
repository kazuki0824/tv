...
`setPrivateData()` はbinding前ならplugin-localに保持し、binding commit時に選択backendへ適用してからbackend-dependent operationを成功させる。
...
`closeSession()` は先にClosingへcommitして新規処理を遮断し、registry revokeとbackend closeを行う。部分失敗時は成功済みstepを繰り返さず、未完了stepだけを次回close/release/service-owned cleanupで再試行する。成功はClosed到達時だけ返す。

`release()` はReleasingへcommitし全session cleanupを全件試行する。全完了時だけReleasedとして成功する。Binder artifact消滅後も未完了cleanup stateはservice ownerが保持するが、worker/timer構成は実装詳細とする。

method成功確定点:
- `setPrivateData()`: plugin-local値と、binding済みならbackend値のcommit完了。
- `setSessionPrivateData()`: backend成功後にsession stateをcommit。
- `openSession*()`: backend open + registry reservation後にActive session ID公開。
- `processEcm()`: 完全な新epochをatomic publishし同session IDから即resolve可能。
- `processEmm()`: bind済みB25 backendが成功。別backendへfallbackしない。
- `closeSession()`: revoke + backend close完了。
- `release()`: 全session cleanup完了。
...
B25/B1公開Tuner tokenは `MediaCas.Session.getSessionId()` bytesそのものとし、1..16 bytes、opaque、raw key非包含、service process lifetime中no-reuseとする。ECM成功前はunresolvedでよく、ECM成功時は完全contextを同session IDから即resolve可能にする。close/release/fatal failureでは新規resolveをrevokeする。
...
## 10. validation
ClearKey AOSP/VTS、unknown ID false/null、smartcard_only B25、yakisoba_only B25、preferのSmartCard valid/known unavailable/timeout、B1 ECM-only、EMM owner==plugin binding、binding後cross-backend fallbackなし、close/release partial failure retry、listener failure非rollback、Yakisoba outcome-unknown、token revoke/ref drain/zeroize、VOID token→MediaCas closeを確認する。
...
7. B25 backendはplugin generation単位で一度だけbindし全session/EMMで共有する。
...
11. method commit point/error semanticsを固定する。
12. cleanup部分失敗は未完了stepだけ再試行する。
13. listener failureでcommit済みstateをrollbackしない。
14. backend I/Oはboundedとし送信後結果不明を成功/fallbackへ丸めない。
15. Tuner tokenはMediaCas session ID bytesでservice process lifetime中再利用しない。
16. `processEcm()`成功時に完全contextを即resolve可能にする。
17. MediaCas close前にMediaCas由来tokenを全descramblerからVOIDで解除する。
18. raw key materialをBinder、TIS、通常logへ出さない。
19. CAS HALはTS demux / AV / DVRを担当しない。
