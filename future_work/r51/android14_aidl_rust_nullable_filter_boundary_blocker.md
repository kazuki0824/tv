...
r51 で確認できるのは、Rust generated trait で到達できる non-null filter 経路の実装、error mapping、state cleanup、regression 防止に限定する。
...
上記を満たす実装方式が固定されるまで、`tuner_hal/DESIGN_JA.md` では nullable filter 経路を AOSP 意味論として扱い、r51 実装済みの範囲には含めない。
