...
この文書は、Tuner HAL を APEX install へ移行する場合に後続で固定する事項だけを管理する。通常 vendor install における ueventd import、SELinux、VINTF/init、product makefile、二重登録禁止の現行統合手順は `tuner_hal/INTEGRATION.md` を正とする。
...
- `tuner_hal/INTEGRATION.md` の通常 vendor install 手順を壊す形で APEX 後続仕様を固定すること。
