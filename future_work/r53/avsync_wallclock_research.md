...
この文書は、A/V sync のうち本製品で採用しない高度な clock discipline と、その判断に使った調査結果を記録する未解決条件資料である。現行設計判断、戻り値、状態遷移、対応宣言条件の正本ではない。現行の `getAvSyncHwId()` / `getAvSyncTime()` 契約は `TUNER_HAL_DESIGN_JA.md` を正とする。
