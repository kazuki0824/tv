- Fixed frontend tune/scan rollback expected-post terminal generation capture so the matcher observes the committed generation rather than the pre-operation value.
- Replaced release-build `debug_assert!` boundary authority assumptions with checked one-shot demux generation boundary commits, rejecting duplicate IDs, missing runtimes and stale capabilities.
- Prevented record-index rollback from combining restored settings with incompatible or newly synthesized parser carry.
- Promoted section CRC support to production code, rejected invalid section numbering, and prevented bounded PES completion from absorbing bytes beyond its declared length.
# r50eo84_frontend_boundary_playback_cleanup_followup_fix2_static_unverified

- Made frontend replacement/stop-object stop failures explicitly discard or restore all frontend and bound-demux rollback authorities with typed demux-vs-token-list cleanup targets.
- Pre-authorized every bound-demux tune-start generation boundary before applying any destructive boundary, committed the complete set through one-shot infallible boundary capabilities, and preserved packet-pipeline data-plane progress during internal control-plane restore.
- Committed frontend generation, live-reader identity, lifecycle, active tune request/scan session, and rollback expected-post matcher atomically after all fallible preparation.
- Made DVR playback replacement depend on worker-owned runtime fail-close completion rather than diagnostic-store success, fail-closed runtime state on replacement spawn failure, and treated successful join/terminal collection as authoritative despite an auxiliary wake failure.
- Added cleanup operation kinds, checked attempt-id exhaustion, attempt completion markers, and expected step counts to playback-worker cleanup reports.
- Made playback packet failures packet-local and typed by failure phase, retained successful events, retried DATA_CONSUMED wake, and bounded per-record packet-failure/anomalous-report vectors with omitted counts.
- Updated DESIGN_JA.md for the revised frontend generation-boundary, internal rollback, playback replacement, cleanup-attempt, packet-failure, wake, and diagnostic-bounding contracts.
- Source syntax and unified-diff whitespace were checked statically; Rust/Soong build, rustfmt, unit tests, atest, VTS, and device verification were not available.

# r50eo84_customer26_dvr_playback_rollback_fmq_identity_fix_static_unverified
- Terminal playback-worker replacement cleanup failure now fail-closes the DVR runtime to `Stopped` before returning, preventing an `AlreadyStarted` DVR from remaining active without a playback consumer.

- Rechecked all 26 customer findings against r50eo83 implementation, DESIGN_JA.md, CODE_CONVENTION.md, and the AOSP synchronized FMQ/DVR direction contracts; accepted all 26 findings and applied source-level fixes.
- Corrected DVR status fill calculation for record/playback roles and changed filter/record/playback tests to use descriptor-derived framework peer endpoints instead of reversing the HAL endpoint role.
- Added generation/lifecycle/session fences to frontend live-data completion and rollback, single-active rollback authority, post-success token consumption, production rollback-ledger failure observability, all-attempt bound-demux rollback, per-demux quarantine, and demux-first/frontend-quarantine split-state handling.
- Preserved filter/DVR FMQ and AV backing instance identity across configure/rollback, restricted queue creation to initial runtime registration before descriptor export, rejected role/capacity/identity replacement repair paths, retained stale AV data IDs across rollback, and exempted AV filters from normal-FMQ source-boundary validation.
- Removed production shared-pointer reset after descriptor export; queue clear now preserves descriptor identity and requires an already-empty queue. Added typed DVR flush reports/diagnostics instead of returning a plain failure without phase/quarantine context.
- Added a production playback DVR EventFlag consumer worker that waits through a descriptor-derived wait endpoint, reads through the HAL-reader endpoint, wakes `DATA_CONSUMED`, and feeds FMQ bytes into the TS packet/demux ingress path; start/stop/close/reset own cancel/wake/join lifecycle.
- Hardened DVR notifier replacement so preflight/store/spawn failures preserve the old notifier and a gated replacement cannot run before store ownership and superseded-worker cleanup are established.
- Updated DESIGN_JA.md and CODE_CONVENTION.md for role-separated FMQ, no-reset descriptor identity, rollback authority fences, AV backing identity, typed DVR flush, playback worker lifecycle, and notifier replacement ordering.
- Rust/Soong build, rustfmt, unit tests, atest, VTS, native Android-header C++ compilation, and device verification were not available in this environment.

# r50eo83_customer26_fmq_rollback_notifier_fix_static_unverified

- Rechecked all 26 customer findings against the implementation, `DESIGN_JA.md`, `CODE_CONVENTION.md`, Rust ownership rules, and the AOSP synchronized FMQ contract; accepted and fixed 24 findings, partially accepted the generic frontend transaction-composer finding, and rebutted only the reset first-error projection claim.
- Fixed the `DvrStatusNotifierCleanupDiagnosticRecord` enum migration compile regression by using the typed `phase()` accessor instead of the removed struct field.
- Split synchronized FMQ handles into HAL-reader/HAL-writer roles, removed the shared `Sync` surface, replaced read-drain clear with descriptor-preserving lifecycle pointer reset, and made queue identity part of rollback validation.
- Removed the generic public frontend runtime transaction composer, made rollback-ledger poison fail closed, limited frontend rollback snapshots to lifecycle state, and made rollback tokens independently owned one-shot RAII authorities.
- Made source-boundary, object-cleanup, notifier-cleanup, configure, stop, and flush outcomes variant-specific; partial or unprovable queue effects now quarantine rather than claiming full rollback.
- Integrated DVR status-reporting configuration into the typed DVR configure transaction and preserved full primary/rollback error context in reports.
- Serialized DVR notifier lifecycle operations, joined a superseded notifier before spawning its replacement, treated policy/artifact exits as normal termination, and limited callback-unhealthy marking to post-Strong Binder delivery failure.
- Updated `DESIGN_JA.md`, `CODE_CONVENTION.md`, and affected source tests to match the revised ownership, rollback, FMQ, configure, and notifier contracts.
- Build, rustfmt, Rust unit tests, Soong, atest, VTS, and device verification were not run in this environment.

# r50eo82_customer26_typed_diagnostic_query_fix_static_unverified

- Reviewed the 26 customer findings against `DESIGN_JA.md` and `CODE_CONVENTION.md`; accepted the typed diagnostic/query boundary defects and rejected visibility-only or already registry-owned façade claims.
- Replaced `DemuxRuntimeError` kind-plus-optional-id storage with variant-specific errors and retained full primary/rollback errors in configure, source-boundary, and filter-operation reports.
- Kept `PacketPid` typed through service_runtime and moved raw PID projection to the terminal AIDL filter-event conversion boundary.
- Replaced DVR status bool/Option policy reconstruction with variant-specific poll/preflight outcomes and preserved artifact-missing versus store-failure notifier availability.
- Removed optional projection accessors from frontend worker cleanup diagnostics and replaced optional public error storage with a variant-specific public outcome.
- Made PCR query helpers fail closed on missing demux runtime or filter snapshot failure, and split source-boundary diagnostics into connect/disconnect variants with mandatory context.
- Build, rustfmt, Rust unit tests, Soong, atest, VTS, and device verification were not run in this environment.

# r50eo81_pub_typed_frontend_runtime_pid_boundary_static_unverified

- Replaced the crate-public full frontend runtime snapshot/restore surface with purpose-specific status and diagnostic DTOs plus a narrow query/transaction facade.
- Added an opaque, non-cloneable frontend rollback token backed by a frontend-runtime-owned ledger; the snapshot remains in that ledger, restore consumes the token once, and dropping an unused token removes its ledger entry.
- Moved frontend rollback preparation into `service_runtime::FrontendTxn`, kept replacement consistency checks on the opaque token, and changed cleanup diagnostics from snapshot restore to rollback-token restore terminology.
- Converted AIDL filter TPID input to `ConfigInputPid` at the adapter boundary and kept production filter configuration, routing views, source-filter matching, flush, and packet-policy decisions typed thereafter.
- Removed the raw `Option<i32>` config matcher and the typed-diagnostic-to-raw-optional-PID projection; raw PID projection remains only at the terminal AIDL event boundary.
- Added source tests for one-shot frontend rollback restore and unused-token ledger cleanup. Build, rustfmt, unit tests, atest, VTS, and device verification were not run in this environment.

# r50eo81_pub_visibility_boundary_recheck_static_unverified

- Rechecked 26 visibility/design-boundary claims after the pub narrowing pass.
- Fixed frontend-bound demux quarantine so bindings whose demux runtime is missing are surfaced as an `InvalidLifecycle` failure after all available bound demux runtimes are quarantined instead of being silently skipped.
- Converted `DvrStatusNotifierCleanupDiagnosticRecord` from a kind + optional object/generation field bag into variant-specific records for reset-store recovery, reset notifier cleanup, worker terminal, and supersede cleanup.
- Left cross-crate frontend runtime domain APIs and diagnostic/query projection helpers unchanged where the current DESIGN/CODE_CONVENTION permits DTO/accessor boundaries or where a larger facade redesign would be required.

# r50eo81_pub_visibility_narrowing_static_unverified

- Narrowed test-only plain `pub` helpers in demux parser/runtime and service_runtime frontend worker cleanup test hooks to `pub(crate)`.
- Narrowed binder_adapter command planning internals by making per-domain `*Command::plan()` parent-module only, making `AidlMethodPlan` fields private, and exposing read-only/consuming accessors instead of public plan fields.
- Narrowed service_runtime internal dispatch, queue runtime, descrambler cleanup report, object close internals, object lifecycle/table helpers, and registry raw mutators from plain `pub` where crate-local visibility is sufficient.
