...
impl LnbApplyTxn {
    ...
    fn record_step(&mut self, step: LnbApplyStep) { self.steps.push(step); }
    ...
    pub fn apply<B: LnbBackendOps>(
        ...
        if let Err(record) = runtime.begin_apply() {
            return LnbApplyOutcome { steps: self.steps, result: Err(record) };
        }

        self.record_step(LnbApplyStep::ApplyBackend);
        if let Err(kind) = backend.apply_lnb_state(runtime.lnb_id(), target_state) {
            let record = runtime.record_failure(map_backend_failure(kind), LnbFailureStep::ApplyBackend);
            return LnbApplyOutcome { steps: self.steps, result: Err(record) };
        }
        ...
        runtime.commit_open();
        LnbApplyOutcome { steps: self.steps, result: Ok(target_state) }
    }
}
...
mod tests {
    ...
    struct TestBackend { fail: bool, applied: Vec<LnbElectricalState> }
    impl TestBackend { fn new() -> Self { Self { fail: false, applied: Vec::new() } } }
    impl LnbBackendOps for TestBackend {
        fn apply_lnb_state(&mut self, _lnb_id: i32, state: LnbElectricalState) -> Result<(), LnbFailureKind> {
            if self.fail { return Err(LnbFailureKind::BackendApplyFailed); }
            self.applied.push(state);
            Ok(())
        }
        fn clear_lnb_callback(&mut self, _lnb_id: i32) -> Result<(), LnbFailureKind> { Ok(()) }
    }
    ...
    fn apply_commits_backend_and_registry_together() {
        ...
        let target = LnbElectricalState { voltage: LnbVoltage::V13, tone: LnbTone::On, satellite_position: Some(3) };
    ...
    fn registry_commit_failure_is_not_normal_state() {
        ...
        let target = LnbElectricalState { voltage: LnbVoltage::V18, tone: LnbTone::Off, satellite_position: None };
        let outcome = LnbApplyTxn::new().apply(&mut runtime, &mut backend, target);
        assert_eq!(outcome.result.unwrap_err().kind, LnbFailureKind::RegistryCommitFailed);
