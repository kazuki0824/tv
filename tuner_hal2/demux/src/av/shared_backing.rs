...
pub enum ClientHandleState { NotExported, ExportedActive, ClientReleased }
...
impl AvSharedBacking {
    pub fn new() -> Self { Self::with_layout(DEFAULT_AV_SHARED_SLOT_COUNT, DEFAULT_AV_SHARED_SLOT_SIZE_BYTES) }

    pub fn with_layout(slot_count: usize, slot_size: usize) -> Self {
        let slots = (0..slot_count)
            .map(|idx| AvSlotState { slot_id: AvSlotId(idx as u32), active_data_id: None, data_length: 0 })
            .collect();
        Self { state: ClientHandleState::NotExported, next_data_id: 1, slot_size, slots, stale_data_ids: Vec::new(), ever_exported: false }
    }

    pub fn client_state(&self) -> ClientHandleState { self.state }
    pub fn slot_size(&self) -> usize { self.slot_size }
    pub fn slot_count(&self) -> usize { self.slots.len() }
    pub fn shared_handle_exported(&self) -> bool { self.ever_exported }
    pub fn active_slot_count(&self) -> usize { self.slots.iter().filter(|slot| slot.active_data_id.is_some()).count() }
    pub fn active_data_ids(&self) -> Vec<AvDataId> { self.slots.iter().filter_map(|slot| slot.active_data_id).collect() }
    ...
    pub fn mark_client_released(&mut self) { self.state = ClientHandleState::ClientReleased; }
    ...
    pub fn data_id_state(&self, data_id: AvDataId) -> AvDataIdState {
        if self.slots.iter().any(|slot| slot.active_data_id == Some(data_id)) {
    ...
    pub fn allocate_payload(&mut self, data_length: usize) -> AvPayloadDeliveryOutcome {
        ...
        let Some(slot_index) = self.slots.iter().position(|slot| slot.active_data_id.is_none()) else {
    ...
    fn release_slot(&mut self, data_id: AvDataId) -> bool {
        if let Some(slot) = self.slots.iter_mut().find(|slot| slot.active_data_id == Some(data_id)) {
    ...
    pub fn classify_release(&self, has_fd: bool, data_id: AvDataId, filter_state: AvFilterReleaseState) -> AvHandleReleaseOutcome {
    ...
    pub fn apply_release(&mut self, has_fd: bool, data_id: AvDataId, filter_state: AvFilterReleaseState) -> AvHandleReleaseOutcome {
        ...
        match outcome {
            ...
            AvHandleReleaseOutcome::SlotReleased { data_id } => { self.release_slot(data_id); }
            _ => {}
        }
        outcome
    }
...
impl Default for AvSharedBacking { fn default() -> Self { Self::new() } }
...
mod tests {
    ...
    fn payload_delivery_requires_exported_active_client_handle() {
        ...
        assert_eq!(backing.allocate_payload(188), AvPayloadDeliveryOutcome::SharedHandleNotExported);
        backing.mark_exported();
        backing.mark_client_released();
        assert_eq!(backing.allocate_payload(188), AvPayloadDeliveryOutcome::ClientHandleReleased);
        backing.reactivate_client_handle();
        assert!(matches!(backing.allocate_payload(188), AvPayloadDeliveryOutcome::Delivered(_)));
    }
    ...
    fn data_id_zero_release_does_not_clear_active_slots() {
        ...
        backing.mark_exported();
        let delivered = match backing.allocate_payload(188) { AvPayloadDeliveryOutcome::Delivered(event) => event, other => panic!("unexpected outcome: {other:?}") };
        assert_eq!(backing.active_slot_count(), 1);
        assert_eq!(backing.apply_release(false, AvDataId(0), AvFilterReleaseState::OpenAv), AvHandleReleaseOutcome::ClientHandleReleased);
        assert_eq!(backing.client_state(), ClientHandleState::ClientReleased);
        assert_eq!(backing.active_slot_count(), 1);
        assert_eq!(backing.data_id_state(delivered.data_id), AvDataIdState::Active);
    }
    ...
    fn active_slot_release_marks_data_id_stale() {
        let mut backing = AvSharedBacking::with_layout(1, 188);
        backing.mark_exported();
        let delivered = match backing.allocate_payload(100) { AvPayloadDeliveryOutcome::Delivered(event) => event, other => panic!("unexpected outcome: {other:?}") };
        assert_eq!(backing.apply_release(false, delivered.data_id, AvFilterReleaseState::OpenAv), AvHandleReleaseOutcome::SlotReleased { data_id: delivered.data_id });
        assert_eq!(backing.active_slot_count(), 0);
        assert_eq!(backing.data_id_state(delivered.data_id), AvDataIdState::Stale);
        assert_eq!(backing.apply_release(false, delivered.data_id, AvFilterReleaseState::OpenAv), AvHandleReleaseOutcome::StaleReleaseAccepted { data_id: delivered.data_id });
    }
