...
- frontend device crate も同様に、`service_runtime` registry が唯一所有する `&mut FrontendRuntime` に対する operation-specific typed request method は crate 間 domain API として `pub` を許容する。許容条件は、AIDL / binder / domain_request が runtime 所有権を取得せず `service_runtime::FrontendTxn` を迂回できないこと、各 method が generation / lifecycle / session relation を再検証すること、opaque rollback token の snapshot / restore authority を caller が偽造・複製できないことである。prepare / restore / install / record / tune / scan を任意順に並べる汎用 `transaction()` façade、raw setter、snapshot 本体の public restore は禁止する。
- 一回性 token に `Clone` / `Copy` を付けない。rollback token は opaque id とし、snapshot 本体を runtime 内部 ledger に置く。同一 runtime の active rollback authority は一つに限定し、restore は owner/ledger/active token/current generation/lifecycle/resource identity を検証した後に実行する。検証またはrestore失敗前にledger entryをremove/tokenをdisarmしてはならず、consumeはrestore成功後だけにする。commit/Dropも当該token idだけを破棄し、別tokenを一括clearしない。Drop cleanup failureはsaturating counterへ残し、production diagnostic snapshotから観測可能にする。poisoned authority ledgerを`into_inner()`で継続利用しない。rollback tokenのgeneration accessorはconsistency check専用とし、restore先やsnapshot本体を読ませない。
...
- frontend worker のblocking joinを`TunerServiceRuntime` lock保持中に実行しない。runtime lock内ではcancel設定済みjoin ticket取得までに限定し、joinはlock外で行う。join前にfrontend/bound demux rollback authorityを固定し、join後はgeneration/lifecycle/session/queue identityを再検証する。bound demux rollbackはall-attempt、失敗demux個別quarantine、demux-first順序とし、demux失敗後にfrontendだけ旧状態へrestoreしない。
- worker start 成功後に fallible commit を置く場合、commit failure path で起動済み worker を stop/join し、frontend runtime 内部 ledger の one-shot rollback token / demux snapshot rollback を試行する。
...
- `SynchronizedReadWrite` FMQ wrapperはHAL writer/HAL readerを別型・別endpoint roleに分け、同一safe handleにreadとwriteを同時公開しない。filter/recordのtest client read、playbackのtest client writeはexport descriptorから`resetPointers=false`でmappingしたpeer endpointを使い、HAL endpointを逆roleで使わない。HAL writerのfillは`capacity-availableToWrite()`、HAL readerのfillは`availableToRead()`で求める。HAL writerがclearのためにread/drainして第二readerになることを禁止する。queue instance生成はdescriptor export前の初回filter/DVR runtime登録に限定する。export後のshared pointer resetはframework peerのquiescenceをHAL内で証明できないためproduction禁止とし、stop/flush/reconfigure/source-boundary repairはqueue identityを維持してemptyを検証する。role/capacity/instance mismatchでqueueを新規置換して成功扱いしない。playback workerはwait-only endpointで`DATA_READY`を待ち、HAL-reader endpointだけがFMQ dataを消費し、read成功後はframework producerへ`DATA_CONSUMED`をwakeする。
...
- 必須診断storeはbounded storeとdropped counterを持つ。startup/descrambler/child open rollback/DVR post-commit/DVR notifier cleanup/callback artifact split/demux transaction/queue descriptor/filter/frontend callback診断を無制限`Vec`へ積まない。production accessorはrecordsとdropped counterを返す。demux transaction snapshotはdemux rollback-token Drop cleanup failure countも集約し、authority cleanup failureをtest-onlyに閉じない。
