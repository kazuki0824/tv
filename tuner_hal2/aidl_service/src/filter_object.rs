...
use maleicacid_tuner_hal2_service_runtime::{configure_ip_cid_result, configure_monitor_event_result};
...
impl FilterAidlObject {
    ...
    pub fn validate_configure_monitor_event_profile(monitor_event_types: i32) -> Result<(), HalError> {
...
mod tests {
    ...
    fn filter_profile_boundary_keeps_ip_cid_unavailable_before_value_validation() {
        for ip_cid in [-1, 0, 1, i32::MAX] {
            ...
            assert_eq!(AidlStatusMapper::map_error(&error), TunerStatusCode::Unavailable);
        }
    }
    ...
    fn filter_profile_boundary_keeps_monitor_event_zero_success_nonzero_unavailable() {
        ...
        for mask in [-1, 1, i32::MAX] {
            let error = FilterAidlObject::validate_configure_monitor_event_profile(mask)
                .expect_err("non-zero monitor event mask must remain unavailable when profile disabled");
            assert_eq!(AidlStatusMapper::map_error(&error), TunerStatusCode::Unavailable);
        }
    }
}
