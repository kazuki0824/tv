...
use binder::{Binder, BinderFeatures, Result as BinderResult, Status, Strong};
use maleicacid_tuner_hal2_binder_adapter::{
    AidlApi, AidlFailureSource, AidlInputField, AidlInputSnapshot, AidlMethodAdapter, AidlMethodCall, AidlMethodPlan, AidlObjectGeneration, AidlObjectId, AidlObjectKind,
    AidlStatusMapper, TunerStatusCode,
};
use maleicacid_tuner_hal2_common::{
    is_japan_bs_if_frequency_hz, is_japan_cs110_if_frequency_hz, is_japan_isdbt_frequency_contract_hz, japan_isdbt_frequency_contract_range_hz,
    FrontendBackendKind, FrontendDevicePath, FrontendScanMode, FrontendStreamIdKind, FrontendSystem, FrontendTuneRequest,
    HalError, HalInternalKind, HalInvalidArgumentKind,
};
use maleicacid_tuner_hal2_service_runtime::{
    FrontendRegistryEntry, FrontendRuntimeId, LnbRegistryProfile, RuntimeCommandDispatchError, RuntimeCommandDispatchPlan, RuntimeOwnerRelation, TunerServiceRuntime,
    start_frontend_demux_live_pump_from_reader,
};
use maleicacid_tuner_hal2_demux::packet_pipeline::PipelineOpenKind;
use maleicacid_tuner_hal2_device::{
    FrontendBackendSession, FrontendBackendTunePlan, FrontendRuntimeState, FrontendSignalState, FrontendWorkerCancelReason, FrontendWorkerContext, FrontendWorkerKind,
    FrontendLivePumpJoinOutcome, FrontendWorkerStartError, FrontendWorkerStopOutcome,
};
...
use crate::input_snapshot::{
    ...
    snapshot_dvr_settings, snapshot_filter_delay_hint, snapshot_filter_settings, snapshot_strong_handle,
};
...
impl TunerAidlService {
    pub fn new(runtime: TunerServiceRuntime) -> Self { Self { runtime: Arc::new(Mutex::new(runtime)) } }


    pub fn lock_runtime(&self) -> Result<MutexGuard<'_, TunerServiceRuntime>, Status> {
        self.runtime.lock().map_err(|_| status_unknown_error("service runtime lock poisoned"))
    }

    fn frontend_entry(&self, frontend_id: i32) -> BinderResult<FrontendRegistryEntry> {
        self.lock_runtime()?
            .frontend_entry(frontend_id)
            .ok_or_else(|| service_error(TunerResult::UNAVAILABLE.0, "frontend id is not available"))
    }

    pub fn plan_method(&self, method: AidlMethodCall) -> Result<RuntimeCommandDispatchPlan, RuntimeCommandDispatchError> {
        let method_plan = AidlMethodAdapter::plan(method);
        self.plan_from_method_plan(&method_plan)
    }

    pub fn plan_from_method_plan(&self, method_plan: &AidlMethodPlan) -> Result<RuntimeCommandDispatchPlan, RuntimeCommandDispatchError> {
        match self.runtime.lock() {
            Ok(mut runtime) => runtime.plan_command_dispatch(method_plan.command_plan, method_plan.command.runtime_executable_request()),
    ...
    fn register_child(&self, kind: AidlObjectKind, public_runtime_id: i64, owner: RuntimeOwnerRelation) -> BinderResult<AidlObjectHandle> {
        let entry = self.lock_runtime()?
            .register_aidl_object_for_runtime_auto_generation(kind, public_runtime_id, owner)
            .map_err(|_| service_error(TunerResult::INVALID_STATE.0, "AIDL object registration failed"))?;
        Ok(AidlObjectHandle::new(entry.object_kind, entry.object_id, entry.generation))
    }
    ...
    fn rollback_registered_child(&self, handle: AidlObjectHandle) -> BinderResult<()> {
        self.lock_runtime()?
            ...
            .map(|_| ())
            .map_err(|_| service_error(TunerResult::UNKNOWN_ERROR.0, "AIDL object registration rollback failed"))
    }
    ...
    fn frontend_object(&self, id: i32) -> BinderResult<Strong<dyn IFrontend>> {
        let handle = self.register_child(AidlObjectKind::Frontend, i64::from(id), RuntimeOwnerRelation::Root)?;
    ...
    fn demux_object(&self, id: i32) -> BinderResult<Strong<dyn IDemux>> {
        let handle = self.register_child(AidlObjectKind::Demux, i64::from(id), RuntimeOwnerRelation::Root)?;
    ...
    fn descrambler_object(&self, id: i32) -> BinderResult<Strong<dyn IDescrambler>> {
        let handle = self.register_child(AidlObjectKind::Descrambler, i64::from(id), RuntimeOwnerRelation::Root)?;
    ...
    fn lnb_object(&self, id: i32) -> BinderResult<Strong<dyn ILnb>> {
        let handle = self.register_child(AidlObjectKind::Lnb, i64::from(id), RuntimeOwnerRelation::Root)?;
...
const DVB_FRONTEND_VARIANT_MASK: i32 = 0x0f;
...
fn lnb_profile_supports_voltage_status(profile: Option<LnbRegistryProfile>) -> bool {
    matches!(profile, Some(LnbRegistryProfile::Px4Device15VOnly | LnbRegistryProfile::EarthPt1FixedLnb))
}

fn lnb_profile_status_voltage(profile: Option<LnbRegistryProfile>) -> LnbVoltage {
    match profile {
        Some(LnbRegistryProfile::Px4Device15VOnly | LnbRegistryProfile::EarthPt1FixedLnb) => LnbVoltage::NONE,
...
fn is_supported_frontend_status(entry: &FrontendRegistryEntry, status_type: FrontendStatusType) -> bool {
...
fn frontend_status_for_types(
    ...
    if status_types.iter().any(|ty| !is_supported_frontend_status(entry, *ty)) {
        return Err(service_error(TunerResult::INVALID_ARGUMENT.0, "unsupported frontend status type requested"));
    }
    Ok(status_types.iter().map(|ty| match *ty {
        FrontendStatusType::DEMOD_LOCK => FrontendStatus::IsDemodLocked(matches!(signal_state, FrontendSignalState::Locked)),
        FrontendStatusType::LNB_VOLTAGE => FrontendStatus::LnbVoltage(lnb_profile_status_voltage(entry.lnb_profile)),
    ...
    status_types
        ...
        .map(|ty| {
            ...
            match runtime_state {
                ...
                FrontendRuntimeState::Tuning { .. } | FrontendRuntimeState::Scanning { .. } => match signal_state {
                    FrontendSignalState::Locked => FrontendStatusReadiness::STABLE,
                    FrontendSignalState::NoSignal | FrontendSignalState::SignalDetected | FrontendSignalState::Unknown => {
...
fn frontend_runtime_state_for_object(
    ...
    let guard = runtime.lock().map_err(|_| status_unknown_error("service runtime lock poisoned"))?;
    guard
        .registry()
...
fn frontend_signal_state_for_object(
    ...
    let guard = runtime.lock().map_err(|_| status_unknown_error("service runtime lock poisoned"))?;
    guard.frontend_signal_state(frontend_id).map_err(status_from_hal_error)
}

fn isdbt_mode_caps() -> i32 { FrontendIsdbtMode::AUTO.0 | FrontendIsdbtMode::MODE_3.0 }
fn isdbt_bandwidth_caps() -> i32 { FrontendIsdbtBandwidth::AUTO.0 | FrontendIsdbtBandwidth::BANDWIDTH_6MHZ.0 }
...
fn runtime_frontend_entry_for_object(runtime: &SharedTunerRuntime, handle: AidlObjectHandle) -> BinderResult<FrontendRegistryEntry> {
    let frontend_id = runtime_entry_public_id(runtime, handle, AidlObjectKind::Frontend)?;
    runtime
        .lock()
        .map_err(|_| status_unknown_error("service runtime lock poisoned"))?
        .frontend_entry(frontend_id)
        .ok_or_else(|| service_error(TunerResult::UNAVAILABLE.0, "frontend runtime entry is not available"))
}
...
impl ITuner for TunerAidlService {
    ...
    fn openFrontendById(&self, frontend_id: i32) -> BinderResult<Strong<dyn IFrontend>> {
        plan_tuner_query_method(self, AidlApi::TunerOpenFrontendById, Some(snapshot_value("frontend_id", frontend_id.to_string())))?;
        {
            let runtime = self.lock_runtime()?;
            if !runtime.has_frontend_id(frontend_id) {
                return Err(status_from_hal_error(HalError::Unsupported("frontend id is not available")));
            }
        }
    ...
    fn getDemuxCaps(&self) -> BinderResult<DemuxCapabilities> { unavailable_after_tuner_method_plan(self, AidlApi::TunerGetDemuxCaps, None, "demux capability is not implemented in WP-R03")?; Err(status_unknown_error("getDemuxCaps unavailable path unexpectedly returned success")) }
    ...
    fn getFrontendInfo(&self, frontend_id: i32) -> BinderResult<FrontendInfo> {
        plan_tuner_query_method(self, AidlApi::TunerGetFrontendInfo, Some(snapshot_value("frontend_id", frontend_id.to_string())))?;
    ...
    fn openLnbById(&self, lnb_id: i32) -> BinderResult<Strong<dyn ILnb>> {
        plan_tuner_query_method(self, AidlApi::TunerOpenLnbById, Some(snapshot_value("lnb_id", lnb_id.to_string())))?;
        let runtime = self.lock_runtime()?;
        if !runtime.has_lnb_id(lnb_id) {
            return Err(status_from_hal_error(HalError::Unsupported("LNB id is not available")));
        }
        drop(runtime);
        self.lnb_object(lnb_id)
    }

    fn openLnbByName(&self, lnb_name: &str, lnb_id: &mut Vec<i32>) -> BinderResult<Strong<dyn ILnb>> {
        lnb_id.clear();
        plan_tuner_query_method(self, AidlApi::TunerOpenLnbByName, Some(snapshot_value("lnb_name", lnb_name)))?;
        let id = {
            let runtime = self.lock_runtime()?;
        ...
        match id {
            ...
            None => Err(status_from_hal_error(HalError::Unsupported("LNB name is not available"))),
        }
    }

    fn setLna(&self, b_enable: bool) -> BinderResult<()> { unavailable_after_tuner_method_plan(self, AidlApi::TunerSetLna, Some(snapshot_value("enable", b_enable.to_string())), "LNA is unsupported") }

    fn setMaxNumberOfFrontends(&self, frontend_type: FrontendType, max_number: i32) -> BinderResult<()> {
        let input = Some(snapshot_fields("SetMaxNumberOfFrontends", vec![AidlInputField::new("frontend_type", format!("{:?}", frontend_type)), AidlInputField::new("max_number", max_number.to_string())]));
        if max_number == 0 {
            plan_tuner_query_method(self, AidlApi::TunerSetMaxNumberOfFrontends, input)?;
            Ok(())
        } else {
            unavailable_after_tuner_method_plan(self, AidlApi::TunerSetMaxNumberOfFrontends, input, "frontend max override is unavailable without probed frontend")
        }
    }

    fn getMaxNumberOfFrontends(&self, frontend_type: FrontendType) -> BinderResult<i32> { plan_tuner_query_method(self, AidlApi::TunerGetMaxNumberOfFrontends, Some(snapshot_fields("GetMaxNumberOfFrontends", vec![AidlInputField::new("frontend_type", format!("{:?}", frontend_type))])))?; Ok(0) }

    fn isLnaSupported(&self) -> BinderResult<bool> { plan_tuner_query_method(self, AidlApi::TunerIsLnaSupported, None)?; Ok(false) }
    ...
    fn openDemuxById(&self, demux_id: i32) -> BinderResult<Strong<dyn IDemux>> {
        plan_tuner_query_method(self, AidlApi::TunerOpenDemuxById, Some(snapshot_value("demux_id", demux_id.to_string())))?;
        let runtime = self.lock_runtime()?;
        if !runtime.has_demux_id(demux_id) {
            return Err(status_from_hal_error(HalError::Unsupported("demux id is not available")));
        }
        drop(runtime);
        self.demux_object(demux_id)
    }

    fn getDemuxInfo(&self, demux_id: i32) -> BinderResult<DemuxInfo> { unavailable_after_tuner_method_plan(self, AidlApi::TunerGetDemuxInfo, Some(snapshot_value("demux_id", demux_id.to_string())), "demux info is not available")?; Err(status_unknown_error("getDemuxInfo unavailable path unexpectedly returned success")) }
}
...
fn cast_u64_field(value: i64, field: &'static str) -> Result<u64, HalError> {
    u64::try_from(value).map_err(|_| {
        HalError::invalid_argument(HalInvalidArgumentKind::NumericRange, format!("{field} must be non-negative"))
    })
}

fn optional_positive_i64_to_u64_field(value: i64, field: &'static str) -> Result<Option<u64>, HalError> {
    if value < 0 {
...
fn validate_isdbt_fixed_settings(
    ...
    if !matches!(s.bandwidth, FrontendIsdbtBandwidth::AUTO | FrontendIsdbtBandwidth::BANDWIDTH_6MHZ) {
        return Err(HalError::invalid_argument(HalInvalidArgumentKind::UnsupportedBandwidth, "ISDB-T bandwidth must be AUTO or 6MHz"));
    }
    if !matches!(s.mode, FrontendIsdbtMode::AUTO | FrontendIsdbtMode::MODE_3) {
        return Err(HalError::invalid_argument(HalInvalidArgumentKind::UnsupportedBandwidth, "ISDB-T mode must be AUTO or MODE_3"));
    }
    if !matches!(
        s.guardInterval,
        ...
        return Err(HalError::invalid_argument(HalInvalidArgumentKind::UnsupportedBandwidth, "unsupported ISDB-T guard interval"));
    }
    for layer in &s.layerSettings {
        if !matches!(
            ...
            return Err(HalError::invalid_argument(HalInvalidArgumentKind::UnsupportedBandwidth, "unsupported ISDB-T layer modulation"));
        }
        if !matches!(
            layer.coderate,
            ...
            return Err(HalError::invalid_argument(HalInvalidArgumentKind::UnsupportedBandwidth, "unsupported ISDB-T layer coderate"));
        }
        if !matches!(
            ...
            return Err(HalError::invalid_argument(HalInvalidArgumentKind::UnsupportedBandwidth, "unsupported ISDB-T layer time interleave"));
        }
    }
    Ok(())
}
...
fn validate_isdbs_fixed_settings(
    ...
    if !matches!(
        ...
        return Err(HalError::invalid_argument(HalInvalidArgumentKind::UnsupportedBandwidth, "unsupported ISDB-S modulation"));
    }
    if !matches!(
        s.coderate,
        ...
        return Err(HalError::invalid_argument(HalInvalidArgumentKind::UnsupportedSymbolRate, "unsupported ISDB-S coderate"));
    }
    if s.symbolRate != 0 {
        return Err(HalError::invalid_argument(HalInvalidArgumentKind::UnsupportedSymbolRate, "ISDB-S symbolRate must be 0 in this product scope"));
    }
    Ok(())
}
...
fn map_isdbs_stream_selector(
    ...
    match stream_id_type {
        FrontendIsdbsStreamIdType::UNDEFINED => {
            if stream_id != 0 {
                return Err(HalError::invalid_argument(HalInvalidArgumentKind::UnsupportedStreamSelector, "streamId must be 0 when streamIdType is UNDEFINED"));
            }
            Ok((None, None))
        }
        FrontendIsdbsStreamIdType::STREAM_ID => {
            ...
            if stream_id < 0 {
                return Err(HalError::invalid_argument(HalInvalidArgumentKind::InvalidStreamIdRange, "negative ISDB-S stream selector"));
            }
            if is_japan_cs110_if_frequency_hz(frequency_hz) {
                return Err(HalError::invalid_argument(HalInvalidArgumentKind::UnsupportedStreamSelector, "CS110 tune must not carry TSID or relative stream selector"));
            }
            let value = u32::try_from(stream_id).map_err(|_| {
                HalError::invalid_argument(HalInvalidArgumentKind::InvalidStreamIdRange, "ISDB-S stream selector out of range")
            })?;
            Ok((Some(value), Some(FrontendStreamIdKind::AbsoluteStreamId)))
        }
        FrontendIsdbsStreamIdType::RELATIVE_STREAM_NUMBER => {
            if stream_id < 0 {
                return Err(HalError::invalid_argument(HalInvalidArgumentKind::InvalidStreamIdRange, "negative ISDB-S relative stream selector"));
            }
            if is_japan_cs110_if_frequency_hz(frequency_hz) {
                return Err(HalError::invalid_argument(HalInvalidArgumentKind::UnsupportedStreamSelector, "CS110 tune must not carry TSID or relative stream selector"));
            }
            let value = u32::try_from(stream_id).map_err(|_| {
                HalError::invalid_argument(HalInvalidArgumentKind::InvalidStreamIdRange, "ISDB-S relative stream selector out of range")
            })?;
            Ok((Some(value), Some(FrontendStreamIdKind::RelativeStreamNumber)))
        }
        _ => Err(HalError::invalid_argument(HalInvalidArgumentKind::UnsupportedStreamSelector, "unsupported ISDB-S streamIdType")),
    }
}

fn aidl_frontend_settings_to_request(settings: &FrontendSettings) -> Result<FrontendTuneRequest, HalError> {
    match settings {
        FrontendSettings::Isdbt(s) => {
            ...
            Ok(FrontendTuneRequest {
                ...
                end_frequency: optional_positive_i64_to_u64_field(s.endFrequency, "isdbt.endFrequency")?,
                stream_id: None,
                stream_id_kind: None,
        ...
        FrontendSettings::Isdbs(s) => {
            ...
            let (stream_id, stream_id_kind) = map_isdbs_stream_selector(s.streamId, s.streamIdType, frequency)?;
            Ok(FrontendTuneRequest {
                system: FrontendSystem::IsdbS,
                frequency,
                end_frequency: optional_positive_i64_to_u64_field(s.endFrequency, "isdbs.endFrequency")?,
                stream_id,
                stream_id_kind,
                bandwidth_hz: None,
                symbol_rate: None,
        ...
        FrontendSettings::Isdbs3(_) => Err(HalError::Unsupported("ISDB-S3 is outside the r51 product scope")),
        FrontendSettings::Dvbs(_) => Err(HalError::Unsupported("DVB-S is outside the r51 product scope")),
        _ => Err(HalError::Unsupported("frontend setting is outside the r51 product scope")),
    }
}


fn validate_frontend_request_against_entry(entry: &FrontendRegistryEntry, request: &FrontendTuneRequest) -> Result<(), HalError> {
    if entry.system != request.system {
        return Err(HalError::invalid_argument(
            HalInvalidArgumentKind::MissingDeliverySystem,
            format!("requested frontend system {} does not match exported frontend {}", request.system.as_hint(), entry.system.as_hint()),
        ));
    }
    ...
    match request.system {
        FrontendSystem::IsdbT => {
            ...
            if let Some(end_frequency) = request.end_frequency {
                if end_frequency < request.frequency || !is_japan_isdbt_frequency_contract_hz(end_frequency) {
                    return Err(HalError::invalid_argument(
        ...
        FrontendSystem::IsdbS => {
            ...
            if let Some(end_frequency) = request.end_frequency {
                ...
                if !(is_japan_bs_if_frequency_hz(end_frequency) || is_japan_cs110_if_frequency_hz(end_frequency)) {
                    return Err(HalError::invalid_argument(
                        HalInvalidArgumentKind::UnsupportedScanRange,
        ...
        FrontendSystem::IsdbS3 | FrontendSystem::DvbS => {
            return Err(HalError::Unsupported("frontend system is outside the r51 product scope"));
        }
    }
    Ok(())
}
...
fn validate_frontend_lnb_candidate(
    ...
    match (entry.lnb_profile, lnb) {
        ...
        (Some(_), Some(_)) => Err(status_unknown_error("frontend/LNB profile mismatch in runtime registry")),
        _ => Err(service_error(
...
fn validate_backend_tune_preflight(
    ...
    match entry.backend {
        ...
        FrontendBackendKind::LinuxDvb => {
            let normalized = maleicacid_tuner_hal2_device::dvb::normalized_tune_request_from_common(request)?;
...
fn mark_scan_end_callback_failed(
    ...
    let mut guard = runtime.lock().map_err(|_| HalError::internal(
...
fn deliver_scan_end_callback(
    ...
    let callback = match frontend_callback_for_owner(handle) {
        ...
        Ok(None) => {
            if let Err(mark_error) = mark_scan_end_callback_failed(runtime, handle, frontend_id, generation) {
...
fn map_frontend_worker_start_error(error: FrontendWorkerStartError) -> Status {
    match error {
        FrontendWorkerStartError::AlreadyRunning { .. } => {
            service_error(TunerResult::INVALID_STATE.0, "frontend worker is already running")
        }
        FrontendWorkerStartError::SpawnFailed { detail } => {
            service_error(TunerResult::UNKNOWN_ERROR.0, &format!("frontend worker spawn failed: {detail}"))
        }
    }
}
...
fn start_frontend_backend_tune_worker_for_object(
    ...
    let mut guard = runtime.lock().map_err(|_| status_unknown_error("service runtime lock poisoned"))?;
    if guard
    ...
    if let Err(error) = guard.install_frontend_live_reader_descriptor_for_generation(frontend_id, kind, generation) {
        guard
            .restore_frontend_runtime_snapshot(frontend_id, snapshot)
            .map_err(status_from_hal_error)?;
        guard
    ...
    if let Err(error) = guard.start_frontend_worker(frontend_id, kind, generation, move |ctx| {
        let session = match FrontendBackendSession::open_and_submit_with_previous_report(&plan, previous_tune_for_worker) {
            Ok(session) => session,
            Err(failure) if failure.rollback_succeeded => {
                let report_error = failure.error;
                let mut guard = runtime_for_worker.lock().map_err(|_| HalError::internal(
                    HalInternalKind::InvariantViolation,
                    "service runtime lock poisoned while restoring tune rollback state",
                ))?;
                guard.restore_frontend_runtime_snapshot(frontend_id, frontend_snapshot_for_worker.clone())?;
                guard.restore_bound_demux_runtime_snapshots(demux_snapshots_for_worker.clone())?;
                return Err(report_error);
            }
            Err(failure) => {
                let report_error = failure.error.clone();
                match mark_tune_worker_failed(&runtime_for_worker, frontend_id, generation, failure.error) {
        ...
        };
        {
            ...
            guard.record_frontend_signal_state(frontend_id, generation, session.initial_signal_state())?;
        }
        let mut live_pump = None;
        while !ctx.cancel_requested() {
            if live_pump.is_none() {
                let live_reader_descriptor = {
                    let guard = runtime_for_worker.lock().map_err(|_| HalError::internal(
                        HalInternalKind::InvariantViolation,
                ...
                if let Some(descriptor) = live_reader_descriptor {
                    ...
                    live_pump = Some(start_frontend_demux_live_pump_from_reader(Arc::clone(&runtime_for_worker), frontend_id, reader)?);
                }
            }
        ...
        if let Some(owner) = live_pump {
            let pump_report = owner.join_after_stop()?;
            drop(pump_report);
        }
        session.stop()
    }) {
...
fn run_frontend_backend_scan_session_worker(
    ...
    demux_snapshots: Vec<(maleicacid_tuner_hal2_service_runtime::DemuxRuntimeId, maleicacid_tuner_hal2_demux::DemuxRuntimeSnapshot)>,
) -> Result<(), HalError> {
    for candidate in candidates {
        if ctx.cancel_requested() {
            return Ok(());
        }
        let plan = FrontendBackendTunePlan::new(ctx.frontend_id(), backend, device_path.clone(), candidate);
        let session = match FrontendBackendSession::open_and_submit_with_previous_report(&plan, previous_request.clone()) {
            Ok(session) => session,
            Err(failure) if failure.rollback_succeeded => {
                let mut guard = runtime.lock().map_err(|_| HalError::internal(
                    HalInternalKind::InvariantViolation,
        ...
        };
        {
            ...
            guard.record_frontend_signal_state(ctx.frontend_id(), ctx.generation(), session.initial_signal_state())?;
        }
        for _ in 0..5 {
            if ctx.cancel_requested() {
                break;
        ...
        let has_next = guard.advance_frontend_scan_session_after_candidate(ctx.frontend_id(), ctx.generation())?;
        drop(guard);
        if !has_next {
...
fn start_frontend_backend_scan_session_for_object(
    ...
    if let Err(error) = guard.install_frontend_live_reader_descriptor_for_generation(frontend_id, FrontendWorkerKind::Scan, generation) {
        guard
            .restore_frontend_runtime_snapshot(frontend_id, snapshot)
            .map_err(status_from_hal_error)?;
        guard
    ...
    if let Err(error) = guard.begin_frontend_scan_session(frontend_id, generation, fingerprint, candidates.clone()) {
        guard
            .restore_frontend_runtime_snapshot(frontend_id, snapshot)
            .map_err(status_from_hal_error)?;
        guard
    ...
    if let Err(error) = guard.start_frontend_worker(frontend_id, FrontendWorkerKind::Scan, generation, move |ctx| {
...
fn map_frontend_worker_stop_outcome(outcome: FrontendWorkerStopOutcome) -> BinderResult<()> {
    match outcome {
        FrontendWorkerStopOutcome::NotRunning
        | FrontendWorkerStopOutcome::CancelRequested { .. }
        | FrontendWorkerStopOutcome::Completed { result: Ok(()), .. } => Ok(()),
        FrontendWorkerStopOutcome::Completed { result: Err(error), .. } => Err(status_from_hal_error(error)),
    }
}

fn prepare_frontend_worker_generation_for_object(
    runtime: &SharedTunerRuntime,
    handle: AidlObjectHandle,
    kind: FrontendWorkerKind,
) -> BinderResult<u64> {
    let frontend_id = runtime_entry_public_id(runtime, handle, AidlObjectKind::Frontend)?;
    let mut runtime = runtime.lock().map_err(|_| status_unknown_error("service runtime lock poisoned"))?;
    runtime
        .prepare_frontend_worker_generation(frontend_id, kind)
        .map_err(status_from_hal_error)
}

fn request_frontend_worker_stop_for_object(
    runtime: &SharedTunerRuntime,
    handle: AidlObjectHandle,
    kind: FrontendWorkerKind,
    ...
    let outcome = runtime
        .lock()
        .map_err(|_| status_unknown_error("service runtime lock poisoned"))?
        .request_frontend_worker_stop(frontend_id, kind, reason);
    map_frontend_worker_stop_outcome(outcome)
}
...
fn record_scan_cancelled_from_stop_outcome(
    ...
    let generation = match outcome {
        ...
        FrontendWorkerStopOutcome::Completed { result: Err(error), .. } => {
            return Err(status_from_hal_error(error.clone()));
        }
    };
...
fn request_all_frontend_workers_stop_for_object(
    ...
    let frontend_id = runtime_entry_public_id(runtime, handle, AidlObjectKind::Frontend)?;
    let (tune_outcome, scan_outcome) = {
        let mut runtime = runtime
            .lock()
            .map_err(|_| status_unknown_error("service runtime lock poisoned"))?;
        (
            runtime.request_frontend_worker_stop(frontend_id, FrontendWorkerKind::Tune, reason),
            runtime.request_frontend_worker_stop(frontend_id, FrontendWorkerKind::Scan, reason),
        )
    };
    map_frontend_worker_stop_outcome(tune_outcome)?;
    map_frontend_worker_stop_outcome(scan_outcome)
}
...
fn aidl_scan_type_to_mode(scan_type: FrontendScanType) -> Result<FrontendScanMode, HalError> {
    match scan_type {
        ...
        FrontendScanType::SCAN_BLIND => Err(HalError::Unsupported("blind scan is outside the r51 product scope; TIS must submit explicit candidates")),
        FrontendScanType::SCAN_UNDEFINED => Err(HalError::invalid_argument(HalInvalidArgumentKind::NumericRange, "scan type must be SCAN_AUTO or SCAN_BLIND")),
        _ => Err(HalError::Unsupported("frontend scan type is outside the r51 product scope")),
    }
}
...
fn ts_pid_from_demux_pid(pid: &DemuxPid) -> Result<u16, HalError> {
    match pid {
        ...
        DemuxPid::TPid(_) => Err(HalError::invalid_argument(HalInvalidArgumentKind::NumericRange, "TS PID must be in 0..=0x1ffe for descrambler binding")),
        _ => Err(HalError::Unsupported("non-TS descrambler PID is outside the r51 TS-only profile")),
    }
}

fn unavailable_after_method_plan(plan: BinderResult<AidlMethodPlan>, message: &'static str) -> BinderResult<()> {
    let plan = plan?;
    let failure = AidlFailureSource::RuntimeDispatch(HalError::Unsupported(message));
    let failures = [failure];
    let status = AidlStatusMapper::resolve_failure_by_precedence(plan.command_plan.api, &failures, false)
        .unwrap_or(TunerStatusCode::UnknownError);
    Err(status_from_tuner_status(status, message))
}

fn unsupported_public_api_call(object: AidlObjectKind, api: AidlApi, input: Option<AidlInputSnapshot>) -> AidlMethodCall {
    AidlMethodCall::UnsupportedPublicApi { object, api, input }
}

fn unavailable_after_tuner_method_plan(service: &TunerAidlService, api: AidlApi, input: Option<AidlInputSnapshot>, message: &'static str) -> BinderResult<()> {
    let method = unsupported_public_api_call(AidlObjectKind::Tuner, api, input);
    let method_plan = AidlMethodAdapter::plan(method);
    service.plan_from_method_plan(&method_plan).map_err(|err| status_from_hal_error(err.into_hal_error()))?;
    let failure = AidlFailureSource::RuntimeDispatch(HalError::Unsupported(message));
    let failures = [failure];
    let status = AidlStatusMapper::resolve_failure_by_precedence(method_plan.command_plan.api, &failures, false)
        .unwrap_or(TunerStatusCode::UnknownError);
    Err(status_from_tuner_status(status, message))
}

fn plan_tuner_query_method(service: &TunerAidlService, api: AidlApi, input: Option<AidlInputSnapshot>) -> BinderResult<()> {
    let method = unsupported_public_api_call(AidlObjectKind::Tuner, api, input);
    let method_plan = AidlMethodAdapter::plan(method);
    service.plan_from_method_plan(&method_plan).map_err(|err| status_from_hal_error(err.into_hal_error()))?;
    Ok(())
...
fn unavailable_after_object_public_api_plan(plan: BinderResult<AidlMethodPlan>, message: &'static str) -> BinderResult<()> {
    unavailable_after_method_plan(plan, message)
}

fn snapshot_debug<T: std::fmt::Debug>(source_type: &'static str, value: &T) -> AidlInputSnapshot {
    AidlInputSnapshot::from_fields(
        source_type,
        vec![AidlInputField::new("debug", format!("{:?}", value))],
    )
}
...
fn runtime_entry_public_id(runtime: &SharedTunerRuntime, handle: AidlObjectHandle, expected_kind: AidlObjectKind) -> BinderResult<i32> {
    let runtime = runtime.lock().map_err(|_| status_unknown_error("service runtime lock poisoned"))?;
    let entry = runtime
        .object_table()
    ...
    i32::try_from(entry.ledger_id.0).map_err(|_| status_unknown_error("public runtime id out of i32 range"))
}
...
fn register_child_aidl_object(
    ...
    let entry = runtime
        ...
        .map_err(|_| service_error(TunerResult::INVALID_STATE.0, "AIDL child object registration failed"))?;
    Ok(AidlObjectHandle::new(entry.object_kind, entry.object_id, entry.generation))
}

fn rollback_child_aidl_object(runtime: &SharedTunerRuntime, handle: AidlObjectHandle) -> BinderResult<()> {
    runtime
        .lock()
        .map_err(|_| status_unknown_error("service runtime lock poisoned"))?
        .unregister_aidl_object_after_registration_failure(handle.object_id(), handle.generation())
        .map(|_| ())
        .map_err(|_| service_error(TunerResult::UNKNOWN_ERROR.0, "AIDL child object rollback failed"))
}

fn unregister_child_public_runtime(runtime: &SharedTunerRuntime, kind: AidlObjectKind, public_runtime_id: i32) -> BinderResult<()> {
    let mut runtime = runtime.lock().map_err(|_| status_unknown_error("service runtime lock poisoned"))?;
    match kind {
...
fn allocate_filter_public_runtime(runtime: &SharedTunerRuntime, owner_demux_id: i32) -> BinderResult<i32> {
    let entry = runtime
        .lock()
        .map_err(|_| status_unknown_error("service runtime lock poisoned"))?
...
fn allocate_dvr_public_runtime(runtime: &SharedTunerRuntime, owner_demux_id: i32) -> BinderResult<i32> {
    let entry = runtime
        .lock()
        .map_err(|_| status_unknown_error("service runtime lock poisoned"))?
...
fn local_filter_handle_from_strong(filter: &Strong<dyn IFilter>) -> BinderResult<AidlObjectHandle> {
    let binder_native: Binder<BnFilter> = filter.as_binder().try_into().map_err(|_| {
        service_error(TunerResult::INVALID_ARGUMENT.0, "source filter is not a local HAL filter")
    })?;
    let Some(local_filter) = binder_native.downcast_binder::<FilterAidlObject>() else {
        return Err(service_error(TunerResult::INVALID_ARGUMENT.0, "source filter is not a local HAL filter"));
    };
...
fn filter_entry_public_id_and_owner(
    ...
    let public_id = i32::try_from(entry.ledger_id.0).map_err(|_| status_unknown_error("filter runtime id out of i32 range"))?;
...
impl IFrontend for FrontendAidlObject {
    fn setCallback(&self, callback: &Strong<dyn IFrontendCallback>) -> BinderResult<()> {
        let snapshot = snapshot_fields("IFrontendCallback", vec![AidlInputField::new("strong_present", "true")]);
    ...
    fn stopTune(&self) -> BinderResult<()> {
        ...
        request_frontend_worker_stop_and_join_for_object(&self.runtime(), self.handle(), FrontendWorkerKind::Tune, FrontendWorkerCancelReason::StopRequested)?;
        stop_frontend_live_data_and_unbind_for_object(&self.runtime(), self.handle())
    }
    fn close(&self) -> BinderResult<()> {
        self.ensure_open()?;
        self.plan_method(AidlMethodCall::FrontendClose)?;
        request_all_frontend_workers_stop_for_object(&self.runtime(), self.handle(), FrontendWorkerCancelReason::FrontendClosing)?;
    ...
    fn scan(&self, settings: &FrontendSettings, scan_type: FrontendScanType) -> BinderResult<()> {
        ...
        let request = aidl_frontend_settings_to_request(settings).map_err(status_from_hal_error)?;
        let entry = validate_frontend_request_for_object(&self.runtime(), self.handle(), &request)?;
        let candidates = backend_scan_candidates(&entry, &request, scan_mode).map_err(status_from_hal_error)?;
    ...
    fn setLnb(&self, lnb_id: i32) -> BinderResult<()> {
        ...
        let exported_lnb_id = {
            ...
            guard
                .lnb_for_frontend_id(entry.id.0)
                .ok_or_else(|| service_error(TunerResult::UNAVAILABLE.0, "frontend has no exported LNB"))?
                .id
                .0
        };
        if exported_lnb_id != lnb_id {
            return Err(service_error(TunerResult::INVALID_ARGUMENT.0, "LNB does not belong to this frontend"));
        }
        unavailable_after_object_public_api_plan(
            self.plan_method(unsupported_public_api_call(AidlObjectKind::Frontend, AidlApi::FrontendSetLnb, Some(snapshot_value("lnb_id", lnb_id.to_string())))),
            "frontend LNB backend binding is not implemented in this WP",
        )
    }
    fn linkCiCam(&self, ci_cam_id: i32) -> BinderResult<i32> { unavailable_after_object_public_api_plan(self.plan_method(unsupported_public_api_call(AidlObjectKind::Frontend, AidlApi::FrontendLinkCiCam, Some(snapshot_value("ci_cam_id", ci_cam_id.to_string())))), "CI CAM is unsupported")?; Err(status_unknown_error("linkCiCam unavailable path unexpectedly returned success")) }
    fn unlinkCiCam(&self, ci_cam_id: i32) -> BinderResult<()> { unavailable_after_object_public_api_plan(self.plan_method(unsupported_public_api_call(AidlObjectKind::Frontend, AidlApi::FrontendUnlinkCiCam, Some(snapshot_value("ci_cam_id", ci_cam_id.to_string())))), "CI CAM is unsupported") }
    fn getHardwareInfo(&self) -> BinderResult<String> { unavailable_after_object_public_api_plan(self.plan_method(unsupported_public_api_call(AidlObjectKind::Frontend, AidlApi::FrontendGetHardwareInfo, None)), "frontend backend is not probed")?; Err(status_unknown_error("getHardwareInfo unavailable path unexpectedly returned success")) }
    fn removeOutputPid(&self, pid: i32) -> BinderResult<()> { unavailable_after_object_public_api_plan(self.plan_method(unsupported_public_api_call(AidlObjectKind::Frontend, AidlApi::FrontendRemoveOutputPid, Some(snapshot_value("pid", pid.to_string())))), "frontend output PID removal is unsupported") }
    fn getFrontendStatusReadiness(&self, status_types: &[FrontendStatusType]) -> BinderResult<Vec<FrontendStatusReadiness>> {
        self.ensure_open()?;
        self.plan_method(unsupported_public_api_call(
            AidlObjectKind::Frontend,
        ...
        let signal_state = frontend_signal_state_for_object(&self.runtime(), self.handle())?;
        Ok(frontend_readiness_for_types(&entry, runtime_state, signal_state, status_types))
    }
}
...
impl IDemux for DemuxAidlObject {
    ...
    fn openFilter(&self, filter_type: &DemuxFilterType, buffer_size: i32, _cb: &Strong<dyn IFilterCallback>) -> BinderResult<Strong<dyn IFilter>> {
        self.ensure_open()?;
        ...
        let runtime = self.runtime();
        let owner_demux_id = runtime_entry_public_id(&runtime, self.handle(), AidlObjectKind::Demux)?;
        let filter_id = allocate_filter_public_runtime(&runtime, owner_demux_id)?;
        if let Err(error) = runtime
            .lock()
            .map_err(|_| status_unknown_error("service runtime lock poisoned"))?
            .register_demux_filter_runtime(owner_demux_id, filter_id, open_kind_for_demux_filter_type(filter_type))
        {
            unregister_child_public_runtime(&runtime, AidlObjectKind::Filter, filter_id)?;
            return Err(status_from_hal_error(error));
        }
        let owner = RuntimeOwnerRelation::Demux { demux: self.handle().object_id(), generation: self.handle().generation() };
        let child_handle = match register_child_aidl_object(&runtime, AidlObjectKind::Filter, filter_id, owner) {
            Ok(handle) => handle,
            Err(status) => {
                unregister_child_public_runtime(&runtime, AidlObjectKind::Filter, filter_id)?;
                return Err(status);
    ...
    fn openTimeFilter(&self) -> BinderResult<Strong<dyn ITimeFilter>> { unavailable_after_object_public_api_plan(self.plan_method(unsupported_public_api_call(AidlObjectKind::Demux, AidlApi::DemuxOpenTimeFilter, None)), "time filter is unsupported")?; Err(status_unknown_error("openTimeFilter unavailable path unexpectedly returned success")) }
    fn getAvSyncHwId(&self, _filter: &Strong<dyn IFilter>) -> BinderResult<i32> { unavailable_after_object_public_api_plan(self.plan_method(unsupported_public_api_call(AidlObjectKind::Demux, AidlApi::DemuxGetAvSyncHwId, Some(snapshot_strong_handle("IFilter")))), "AV sync is not implemented in WP-R03")?; Err(status_unknown_error("getAvSyncHwId unavailable path unexpectedly returned success")) }
    fn getAvSyncTime(&self, av_sync_hw_id: i32) -> BinderResult<i64> { unavailable_after_object_public_api_plan(self.plan_method(unsupported_public_api_call(AidlObjectKind::Demux, AidlApi::DemuxGetAvSyncTime, Some(snapshot_value("av_sync_hw_id", av_sync_hw_id.to_string())))), "AV sync is not implemented in WP-R03")?; Err(status_unknown_error("getAvSyncTime unavailable path unexpectedly returned success")) }
    fn close(&self) -> BinderResult<()> { self.close_object_after_plan(AidlMethodCall::DemuxClose) }
    fn openDvr(&self, dvr_type: DvrType, buffer_size: i32, _cb: &Strong<dyn IDvrCallback>) -> BinderResult<Strong<dyn IDvr>> {
        self.ensure_open()?;
    ...
    fn connectCiCam(&self, ci_cam_id: i32) -> BinderResult<()> { unavailable_after_object_public_api_plan(self.plan_method(unsupported_public_api_call(AidlObjectKind::Demux, AidlApi::DemuxConnectCiCam, Some(snapshot_value("ci_cam_id", ci_cam_id.to_string())))), "CI CAM is unsupported") }
    fn disconnectCiCam(&self) -> BinderResult<()> { unavailable_after_object_public_api_plan(self.plan_method(unsupported_public_api_call(AidlObjectKind::Demux, AidlApi::DemuxDisconnectCiCam, None)), "CI CAM is unsupported") }
}

impl IFilter for FilterAidlObject {
    fn getQueueDesc(&self, _queue: &mut TunerQueueDesc) -> BinderResult<()> {
        unavailable_after_method_plan(self.plan_method(AidlMethodCall::FilterGetQueueDesc), "FMQ runtime is not implemented in WP-R03")
    }
    fn close(&self) -> BinderResult<()> { self.close_object_after_plan(AidlMethodCall::FilterClose) }
    fn configure(&self, _settings: &DemuxFilterSettings) -> BinderResult<()> {
        unavailable_after_method_plan(self.plan_method(AidlMethodCall::FilterConfigure(snapshot_filter_settings(_settings))), "filter runtime is not implemented in WP-R03")
    }
    fn configureAvStreamType(&self, _av_stream_type: &AvStreamType) -> BinderResult<()> {
        unavailable_after_method_plan(self.plan_method(AidlMethodCall::FilterConfigureAvStreamType(snapshot_av_stream_type(_av_stream_type))), "AV stream configuration runtime is not implemented in WP-R03")
    }
    fn configureIpCid(&self, ip_cid: i32) -> BinderResult<()> {
        unavailable_after_method_plan(
            self.plan_method(AidlMethodCall::FilterConfigure(AidlInputSnapshot::single_field("ipCid", "ip_cid", ip_cid.to_string()))),
    ...
    fn configureMonitorEvent(&self, monitor_event_types: i32) -> BinderResult<()> {
        let plan = self.plan_method(AidlMethodCall::FilterConfigure(AidlInputSnapshot::single_field("monitorEventTypes", "monitor_event_types", monitor_event_types.to_string())))?;
    ...
    fn start(&self) -> BinderResult<()> { unavailable_after_method_plan(self.plan_method(AidlMethodCall::FilterStart), "filter runtime is not implemented in WP-R03") }
    fn stop(&self) -> BinderResult<()> { unavailable_after_method_plan(self.plan_method(AidlMethodCall::FilterStop), "filter runtime is not implemented in WP-R03") }
    fn flush(&self) -> BinderResult<()> { unavailable_after_method_plan(self.plan_method(AidlMethodCall::FilterFlush), "filter runtime is not implemented in WP-R03") }
    fn getAvSharedHandle(&self, _av_memory: &mut TunerNativeHandle) -> BinderResult<i64> {
        unavailable_after_method_plan(self.plan_method(AidlMethodCall::FilterGetAvSharedHandle), "AV shared memory is not implemented in WP-R03")?;
        Err(status_unknown_error("getAvSharedHandle unavailable path unexpectedly returned success"))
    }
    fn getId(&self) -> BinderResult<i32> {
        self.plan_method(AidlMethodCall::FilterGetId)?;
        i32::try_from(self.handle().object_id().0).map_err(|_| status_unknown_error("filter id out of i32 range"))
    }
    ...
    fn releaseAvHandle(&self, _av_memory: &TunerNativeHandle, av_data_id: i64) -> BinderResult<()> {
        unavailable_after_method_plan(
            self.plan_method(AidlMethodCall::FilterReleaseAvHandle(snapshot_fields(
                "releaseAvHandle",
                vec![AidlInputField::new("av_data_id", av_data_id.to_string()), AidlInputField::new("native_handle_present", "true")],
            ))),
    ...
    fn setDataSource(&self, filter: &Strong<dyn IFilter>) -> BinderResult<()> {
        self.ensure_open()?;
        self.plan_method(AidlMethodCall::FilterSetDataSource(snapshot_strong_handle("IFilter")))?;
        let runtime = self.runtime();
        let sink_handle = self.handle();
        let source_handle = local_filter_handle_from_strong(filter)?;
        if source_handle.object_id() == sink_handle.object_id() && source_handle.generation() == sink_handle.generation() {
            return Err(service_error(TunerResult::INVALID_ARGUMENT.0, "filter cannot use itself as source"));
        }
        ...
        if sink_owner != source_owner {
            return Err(service_error(TunerResult::INVALID_ARGUMENT.0, "source filter belongs to a different demux"));
        }
        let RuntimeOwnerRelation::Demux { demux, generation } = sink_owner else {
            return Err(service_error(TunerResult::INVALID_ARGUMENT.0, "filter owner is not a demux"));
        };
        let demux_id = {
            let runtime_guard = runtime.lock().map_err(|_| status_unknown_error("service runtime lock poisoned"))?;
            let demux_entry = runtime_guard
                .object_table()
                .entry_for_kind(demux, generation, AidlObjectKind::Demux)
                .map_err(|_| service_error(TunerResult::INVALID_STATE.0, "owner demux is not live"))?;
            i32::try_from(demux_entry.ledger_id.0).map_err(|_| status_unknown_error("demux runtime id out of i32 range"))?
        };
        runtime
            .lock()
            .map_err(|_| status_unknown_error("service runtime lock poisoned"))?
    ...
    fn setDelayHint(&self, _hint: &FilterDelayHint) -> BinderResult<()> {
        unavailable_after_method_plan(self.plan_method(AidlMethodCall::FilterSetDelayHint(snapshot_filter_delay_hint(_hint))), "filter delay runtime is not implemented in WP-R03")
    }
}

impl IDvr for DvrAidlObject {
    fn getQueueDesc(&self, _queue: &mut TunerQueueDesc) -> BinderResult<()> {
        unavailable_after_method_plan(self.plan_method(AidlMethodCall::DvrGetQueueDesc), "DVR FMQ runtime is not implemented in WP-R03")
    }
    fn configure(&self, _settings: &DvrSettings) -> BinderResult<()> {
        unavailable_after_method_plan(self.plan_method(AidlMethodCall::DvrConfigure(snapshot_dvr_settings(_settings))), "DVR runtime is not implemented in WP-R03")
    }
    fn attachFilter(&self, filter: &Strong<dyn IFilter>) -> BinderResult<()> {
        unavailable_after_method_plan(self.plan_method(AidlMethodCall::DvrAttachFilter(snapshot_strong_handle("IFilter")) ), "DVR runtime is not implemented in WP-R03")
    }
    fn detachFilter(&self, filter: &Strong<dyn IFilter>) -> BinderResult<()> {
        unavailable_after_method_plan(self.plan_method(AidlMethodCall::DvrDetachFilter(snapshot_strong_handle("IFilter")) ), "DVR runtime is not implemented in WP-R03")
    }
    fn start(&self) -> BinderResult<()> { unavailable_after_method_plan(self.plan_method(AidlMethodCall::DvrStart), "DVR runtime is not implemented in WP-R03") }
    fn stop(&self) -> BinderResult<()> { unavailable_after_method_plan(self.plan_method(AidlMethodCall::DvrStop), "DVR runtime is not implemented in WP-R03") }
    fn flush(&self) -> BinderResult<()> { unavailable_after_method_plan(self.plan_method(AidlMethodCall::DvrFlush), "DVR runtime is not implemented in WP-R03") }
    fn close(&self) -> BinderResult<()> { self.close_object_after_plan(AidlMethodCall::DvrClose) }
    fn setStatusCheckIntervalHint(&self, milliseconds: i64) -> BinderResult<()> {
        unavailable_after_method_plan(self.plan_method(AidlMethodCall::DvrSetStatusCheckIntervalHint(milliseconds)), "DVR callback runtime is not implemented in WP-R03")
    }
}

impl IDescrambler for DescramblerAidlObject {
    fn setDemuxSource(&self, demux_id: i32) -> BinderResult<()> {
        unavailable_after_method_plan(self.plan_method(AidlMethodCall::DescramblerSetDemuxSource(demux_id)), "descrambler runtime is not implemented in WP-R03")
    }
    fn setKeyToken(&self, key_token: &[u8]) -> BinderResult<()> { unavailable_after_method_plan(self.plan_method(AidlMethodCall::DescramblerSetKeyToken(key_token.to_vec())), "descrambler key runtime is not implemented in WP-R03") }
    fn addPid(&self, pid: &DemuxPid, _optional_upstream_filter: &Strong<dyn IFilter>) -> BinderResult<()> {
        self.ensure_open()?;
        let pid = ts_pid_from_demux_pid(pid).map_err(status_from_hal_error)?;
        unavailable_after_method_plan(self.plan_method(AidlMethodCall::DescramblerAddPid(pid)), "descrambler PID runtime is not implemented in WP-R03")
    }
    fn removePid(&self, pid: &DemuxPid, _optional_upstream_filter: &Strong<dyn IFilter>) -> BinderResult<()> {
        self.ensure_open()?;
        let pid = ts_pid_from_demux_pid(pid).map_err(status_from_hal_error)?;
        unavailable_after_method_plan(self.plan_method(AidlMethodCall::DescramblerRemovePid(pid)), "descrambler PID runtime is not implemented in WP-R03")
    }
    fn close(&self) -> BinderResult<()> { self.close_object_after_plan(AidlMethodCall::DescramblerClose) }
}

impl ILnb for LnbAidlObject {
    fn setCallback(&self, callback: &Strong<dyn ILnbCallback>) -> BinderResult<()> {
        self.plan_method(AidlMethodCall::LnbSetCallback(snapshot_fields("ILnbCallback", vec![AidlInputField::new("strong_present", "true")])) )?;
        self.retain_callback(callback)
    }
    fn setVoltage(&self, _voltage: LnbVoltage) -> BinderResult<()> {
        unavailable_after_method_plan(self.plan_method(AidlMethodCall::LnbSetVoltage(snapshot_fields("LnbVoltage", vec![AidlInputField::new("voltage", format!("{:?}", _voltage))]))), "LNB runtime is not implemented in WP-R03")
    }
    fn setTone(&self, _tone: LnbTone) -> BinderResult<()> {
        unavailable_after_method_plan(self.plan_method(AidlMethodCall::LnbSetTone(snapshot_fields("LnbTone", vec![AidlInputField::new("tone", format!("{:?}", _tone))]))), "LNB runtime is not implemented in WP-R03")
    }
    fn setSatellitePosition(&self, _position: LnbPosition) -> BinderResult<()> {
        unavailable_after_method_plan(self.plan_method(AidlMethodCall::LnbSetSatellitePosition(snapshot_fields("LnbPosition", vec![AidlInputField::new("position", format!("{:?}", _position))]))), "LNB runtime is not implemented in WP-R03")
    }
    fn sendDiseqcMessage(&self, diseqc_message: &[u8]) -> BinderResult<()> {
        unavailable_after_method_plan(self.plan_method(AidlMethodCall::LnbSendDiseqc(diseqc_message.to_vec())), "DiSEqC runtime is not implemented in WP-R03")
    }
    fn close(&self) -> BinderResult<()> { self.close_object_after_plan(AidlMethodCall::LnbClose) }
}

fn status_unknown_error(message: &str) -> Status { service_error(TunerResult::UNKNOWN_ERROR.0, message) }
...
fn status_from_tuner_status(status: TunerStatusCode, message: &str) -> Status {
    match status {
        TunerStatusCode::Ok => service_error(TunerResult::UNKNOWN_ERROR.0, "unexpected OK status while building error"),
...
mod tests {
    ...
    fn configure_ip_cid_returns_unavailable_for_any_value() {
        ...
        let handle = AidlObjectHandle::new(AidlObjectKind::Filter, AidlObjectId(10), AidlObjectGeneration(1));
        {
            let mut runtime = service.lock_runtime().unwrap();
            runtime.register_aidl_object_for_runtime(
                AidlObjectKind::Filter,
                AidlObjectId(10),
    ...
    fn configure_monitor_event_zero_succeeds_nonzero_unavailable() {
        let service = TunerAidlService::new(TunerServiceRuntime::new());
        let handle = AidlObjectHandle::new(AidlObjectKind::Filter, AidlObjectId(11), AidlObjectGeneration(1));
        {
            let mut runtime = service.lock_runtime().unwrap();
            runtime.register_aidl_object_for_runtime(
                AidlObjectKind::Filter,
    ...
    fn service_plans_aidl_method_without_string_dispatch() {
        ...
        let plan = service.plan_method(AidlMethodCall::FrontendTune(request)).unwrap();
        assert_eq!(plan.command_plan.transaction, maleicacid_tuner_hal2_binder_adapter::RuntimeTransactionName::FrontendTuneTxnApply);
    }

    #[test]
    fn frontend_close_requests_tune_and_scan_worker_stop() {
        let service = TunerAidlService::new(TunerServiceRuntime::new());
        let handle = AidlObjectHandle::new(AidlObjectKind::Frontend, AidlObjectId(20), AidlObjectGeneration(1));
        let (reason_tx, reason_rx) = std::sync::mpsc::channel();
        {
            ...
            runtime.start_frontend_worker(7, FrontendWorkerKind::Tune, 1, move |ctx| {
                while !ctx.cancel_requested() {
            ...
            runtime.start_frontend_worker(7, FrontendWorkerKind::Scan, 1, move |ctx| {
                while !ctx.cancel_requested() {
                    std::thread::sleep(std::time::Duration::from_millis(1));
                }
        ...
        let first = reason_rx.recv_timeout(std::time::Duration::from_secs(1)).unwrap();
        let second = reason_rx.recv_timeout(std::time::Duration::from_secs(1)).unwrap();
        let mut reasons = vec![first, second];
        reasons.sort_by_key(|(kind, _)| *kind);
        assert_eq!(
            reasons,
            vec![
                (FrontendWorkerKind::Tune, Some(FrontendWorkerCancelReason::FrontendClosing)),
                (FrontendWorkerKind::Scan, Some(FrontendWorkerCancelReason::FrontendClosing)),
            ]
        );
    }
}
