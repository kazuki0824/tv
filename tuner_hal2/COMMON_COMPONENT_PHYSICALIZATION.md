...
本書でいう「27件」は、次を合わせた **27個の共通化対象**を指す。

1. `../tuner_hal/DESIGN_JA.md` 0-S-3B の正規共通契約を Rust 識別子単位へ分解した21件。
2. 0-S-3B 外だが、現行設計が共通の処理責務として一意化している6件。
...
次の6件は0-S-3Bの行そのものではないが、現行設計で複数の呼出元が独自実装してはならない共通化対象として扱う。
...
集計を21件へ加えると、**A=14、B=12、C=1、計27件**となる。

この6件について、本メモでは上表の名称を正規論理契約名として扱う。散文の「オブジェクトメソッド」「ルートオープン」「子オープン」「フロントエンド選局・走査」「フロントエンドワーカー終了」は、それぞれ上表の正規名称を参照する。

---

## 5. 検査規則

27件すべてについて、正規部品が存在するだけでは設計適合とみなさない。
...
- 27件すべてにA/B/Cが一意に付与されている。
- 27件すべてについて、正規の状態所有者・手順所有者・分類器所有者、正規名称標識、正規入口または明示された入口集合を一意に追跡できる。
- A=14、B=12、C=1 の集計と全件表が一致する。
- 27件すべてが判定の木による判定結果と一致する。
- 27件すべてが2章の共通実装・接続規則に一致する。
