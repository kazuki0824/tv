...
impl LnbCommand {
    pub(super) fn plan(&self) -> Result<CommandPlan, HalError> {
        match self {
            ...
            Self::SetVoltage(_) => {
                CommandPlan::for_api(AidlObjectKind::Lnb, AidlApi::LnbSetVoltage)
            }
            Self::SetTone(_) => CommandPlan::for_api(AidlObjectKind::Lnb, AidlApi::LnbSetTone),
            Self::SetSatellitePosition(_) => {
                CommandPlan::for_api(AidlObjectKind::Lnb, AidlApi::LnbSetSatellitePosition)
            }
            Self::SendDiseqc(_) => {
                CommandPlan::for_api(AidlObjectKind::Lnb, AidlApi::LnbSendDiseqc)
            }
