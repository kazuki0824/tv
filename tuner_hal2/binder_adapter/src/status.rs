...
pub enum TunerStatusCode { Ok, InvalidArgument, InvalidState, Unavailable, UnknownError }

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct DomainResult<T> { pub status: TunerStatusCode, pub value: Option<T>, pub error: Option<HalError> }

impl<T> DomainResult<T> {
    pub fn ok(value: T) -> Self { Self { status: TunerStatusCode::Ok, value: Some(value), error: None } }
    pub fn err(error: HalError) -> Self { Self { status: AidlStatusMapper::map_error(&error), value: None, error: Some(error) } }
}
...
impl AidlStatusMapper {
    pub const fn new(aidl_version: u32) -> Self { Self { aidl_version } }
    pub const fn aidl_version(&self) -> u32 { self.aidl_version }
    ...
    pub const fn precedence_for_api(api: AidlApi) -> ApiStatusPrecedence {
        ApiStatusPrecedence { api, steps: DEFAULT_MUTATING_STEPS }
    }

    pub const fn unsupported_precedence_for_profile_api(api: AidlApi) -> ApiStatusPrecedence {
        ApiStatusPrecedence { api, steps: UNSUPPORTED_BEFORE_INPUT_STEPS }
    }
...
mod tests {
    ...
    fn status_mapper_uses_error_kind_not_display_text() {
        let invalid_arg = HalError::invalid_argument(HalInvalidArgumentKind::NumericRange, "値域外");
        let invalid_state = HalError::invalid_state(HalInvalidStateKind::InvalidLifecycle, "閉鎖済み");
        assert_eq!(AidlStatusMapper::map_error(&invalid_arg), TunerStatusCode::InvalidArgument);
        assert_eq!(AidlStatusMapper::map_error(&invalid_state), TunerStatusCode::InvalidState);
    }
    ...
    fn device_absent_and_open_failed_are_unavailable_but_runtime_io_is_unknown() {
        ...
        let open = HalError::OpenFailed { path: PathBuf::from("/dev/dvb/adapter0/frontend0"), detail: HalErrorDetail::new("open failed") };
        let ioctl = HalError::IoctlFailed { backend: "dvb", path: None, op: "FE_SET_PROPERTY", errno: 5 };
        assert_eq!(AidlStatusMapper::map_error(&missing), TunerStatusCode::Unavailable);
        assert_eq!(AidlStatusMapper::map_error(&open), TunerStatusCode::Unavailable);
        assert_eq!(AidlStatusMapper::map_error(&ioctl), TunerStatusCode::UnknownError);
    }
    ...
    fn callback_fmq_eventflag_and_cleanup_failures_remain_unknown_not_unsupported() {
        ...
        assert_eq!(AidlStatusMapper::map_error(&callback), TunerStatusCode::UnknownError);
        assert_eq!(AidlStatusMapper::map_error(&fmq), TunerStatusCode::UnknownError);
        assert_eq!(AidlStatusMapper::map_error(&event), TunerStatusCode::UnknownError);
        assert_eq!(AidlStatusMapper::map_error(&cleanup), TunerStatusCode::UnknownError);
    }
    ...
    fn precedence_table_has_entry_for_every_transaction_api() {
        for plan in AIDL_TRANSACTION_TABLE {
            ...
            assert!(precedence.steps.contains(&StatusPrecedenceStep::ObjectLifetime));
            assert!(precedence.steps.contains(&StatusPrecedenceStep::RuntimeDispatch));
        }
    }

    #[test]
    fn profile_unsupported_can_be_declared_before_input_validation_for_ts_only_apis() {
        let precedence = AidlStatusMapper::unsupported_precedence_for_profile_api(AidlApi::FilterConfigure);
        assert_eq!(precedence.steps[0], StatusPrecedenceStep::ObjectLifetime);
        assert_eq!(precedence.steps[1], StatusPrecedenceStep::ProfileUnsupported);
    ...
    fn resolver_prefers_object_lifetime_before_other_failures() {
        let closed = HalError::invalid_state(maleicacid_tuner_hal2_common::HalInvalidStateKind::InvalidLifecycle, "closed object");
        ...
        assert_eq!(
            AidlStatusMapper::resolve_failure_by_precedence(AidlApi::FilterConfigure, &failures, true),
    ...
    fn resolver_prefers_profile_unsupported_before_invalid_argument_when_declared() {
        let unsupported = HalError::Unsupported("DemuxFilterSettings::ip is outside TS-only profile");
        let invalid = HalError::invalid_argument(HalInvalidArgumentKind::NumericRange, "invalid ip cid");
        let failures = [
