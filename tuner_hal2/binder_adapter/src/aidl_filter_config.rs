...
use maleicacid_tuner_hal2_demux::config::{
    AvSettings, FilterConfig, FilterConfigKind, FilterOpenType, OpenFilterRequest,
    PesSettings, RecordIndexSettings, SectionCondition, SectionConditionKind,
};
use maleicacid_tuner_hal2_demux::parser::record_index::{
    ...
    HEVC_SC_AUD, HEVC_SC_BLA_N_LP, HEVC_SC_BLA_W_LP, HEVC_SC_BLA_W_RADL,
    HEVC_SC_IDR_N_LP, HEVC_SC_IDR_W_RADL, HEVC_SC_SPS, HEVC_SC_TRAIL_CRA,
    RECORD_SC_TYPE_NONE, RECORD_SC_TYPE_SC, RECORD_SC_TYPE_SC_AVC, RECORD_SC_TYPE_SC_HEVC,
    RECORD_SC_TYPE_SC_VVC, VVC_SC_AUD, VVC_SC_CRA, VVC_SC_GDR, VVC_SC_IDR_N_LP,
    VVC_SC_IDR_W_RADL, VVC_SC_SPS, VVC_SC_VPS,
};
...
pub fn filter_open_type(filter_type: &DemuxFilterType) -> Result<FilterOpenType, HalError> {
    if !filter_main_type_supported(filter_type.mainType) {
        return Err(HalError::Unsupported("filter main type is outside the TS-only tuner_hal2 profile"));
    }
    match &filter_type.subType {
        ...
        DemuxFilterSubType::TsFilterType(DemuxTsFilterType::SECTION) => Ok(FilterOpenType::TsSection),
        ...
        _ => Err(HalError::Unsupported("filter subtype is outside the TS-only tuner_hal2 profile")),
    }
}
...
pub fn build_open_filter_request(
    ...
    Ok(OpenFilterRequest { open_type: filter_open_type(filter_type)?, buffer_size, callback_present })
}

pub fn validate_ts_pid(pid: i32) -> Result<(), HalError> {
    if (0..=0x1fff).contains(&pid) { Ok(()) } else { Err(invalid("TS PID must be 0..=0x1fff")) }
}
...
pub fn build_section_condition_kind(condition: &DemuxFilterSectionSettingsCondition) -> SectionConditionKind {
    match condition {
...
fn normalize_table_info_version(version: i32) -> Result<Option<i32>, HalError> {
    if version == -1 { Ok(None) } else if (0..=31).contains(&version) { Ok(Some(version)) } else { Err(invalid("section version must be -1 or 0..=31")) }
}

fn normalize_section_table_id(table_id: i32) -> Result<i32, HalError> {
    if (0..=255).contains(&table_id) { Ok(table_id) } else { Err(invalid("section table id must be 0..=255")) }
}

fn build_section_bits_condition(bits: &DemuxFilterSectionBits) -> Result<SectionCondition, HalError> {
    if bits.filter.len() > MAX_SECTION_FILTER_BYTES || bits.mask.len() > MAX_SECTION_FILTER_BYTES || bits.mode.len() > MAX_SECTION_FILTER_BYTES {
...
pub fn build_section_condition(condition: &DemuxFilterSectionSettingsCondition) -> Result<SectionCondition, HalError> {
    match condition {
        DemuxFilterSectionSettingsCondition::SectionBits(bits) => build_section_bits_condition(bits),
...
fn supported_record_sc_index_mask(sc_index_type: i32) -> Option<i32> {
    match sc_index_type {
        ...
        RECORD_SC_TYPE_SC_AVC => Some(AVC_SC_I_SLICE | AVC_SC_P_SLICE | AVC_SC_B_SLICE | AVC_SC_SI_SLICE | AVC_SC_SP_SLICE),
        RECORD_SC_TYPE_SC_HEVC => Some(HEVC_SC_SPS | HEVC_SC_AUD | HEVC_SC_BLA_W_LP | HEVC_SC_BLA_W_RADL | HEVC_SC_BLA_N_LP | HEVC_SC_IDR_W_RADL | HEVC_SC_IDR_N_LP | HEVC_SC_TRAIL_CRA),
        RECORD_SC_TYPE_SC_VVC => Some(VVC_SC_IDR_W_RADL | VVC_SC_IDR_N_LP | VVC_SC_CRA | VVC_SC_GDR | VVC_SC_VPS | VVC_SC_SPS | VVC_SC_AUD),
        _ => None,
    }
}
...
pub fn validate_record_index_settings(
    ...
    if sc_index_type == RECORD_SC_TYPE_NONE {
        if mask_bits == 0 { return Ok(0); }
        return Err(invalid("record.scIndexMask must be zero when scIndexType is NONE"));
    }
    ...
    if variant_type != sc_index_type {
        return Err(invalid("record.scIndexType does not match scIndexMask union variant"));
    }
...
fn build_section_config(open_type: FilterOpenType, tpid: i32, settings: &DemuxFilterSectionSettings) -> Result<FilterConfig, HalError> {
    let length_field_bits = normalize_length_field_bits(settings.bitWidthOfLengthField).ok_or_else(|| invalid("section bitWidthOfLengthField is unsupported"))?;
    Ok(FilterConfig {
        open_type,
        tpid,
...
pub fn build_filter_summary_for_open_type(
    ...
    let DemuxFilterSettings::Ts(ts) = settings else {
        return Err(HalError::Unsupported("filter settings root is outside the TS-only tuner_hal2 profile"));
    };
    validate_ts_pid(ts.tpid)?;
    let tpid = ts.tpid;
    match &ts.filterSettings {
        DemuxTsFilterSettingsFilterSettings::Noinit(_) => {
            if open_type != FilterOpenType::TsRaw { return Err(invalid("noinit settings require TS raw filter")); }
            Ok(FilterConfig { open_type, tpid, kind: FilterConfigKind::TsRaw })
        }
        DemuxTsFilterSettingsFilterSettings::Section(section) => {
            if open_type != FilterOpenType::TsSection { return Err(invalid("section settings require section filter")); }
            build_section_config(open_type, tpid, section)
        }
        DemuxTsFilterSettingsFilterSettings::Av(av) => {
            if open_type != FilterOpenType::TsAudio && open_type != FilterOpenType::TsVideo { return Err(invalid("AV settings require audio/video filter")); }
            if av.isPassthrough { return Err(HalError::Unsupported("AV passthrough is not implemented")); }
            if av.isSecureMemory { return Err(HalError::Unsupported("secure AV memory is not implemented")); }
            Ok(FilterConfig { open_type, tpid, kind: FilterConfigKind::TsAv(AvSettings { is_passthrough: false, is_secure_memory: false }) })
        }
        DemuxTsFilterSettingsFilterSettings::PesData(pes) => {
            if open_type != FilterOpenType::TsPes { return Err(invalid("PES settings require PES filter")); }
            Ok(FilterConfig { open_type, tpid, kind: FilterConfigKind::TsPes(PesSettings { stream_id: normalize_pes_stream_id(pes.streamId)?, raw: pes.isRaw }) })
        }
        DemuxTsFilterSettingsFilterSettings::Record(record) => {
            if open_type != FilterOpenType::TsRecord { return Err(invalid("record settings require record filter")); }
            let mask = validate_record_index_settings(record.tsIndexMask, record.scIndexType.0, &record.scIndexMask)?;
            Ok(FilterConfig { open_type, tpid, kind: FilterConfigKind::TsRecord(RecordIndexSettings { ts_index_mask: record.tsIndexMask, sc_index_type: record.scIndexType.0, sc_index_mask: mask }) })
        }
    }
}
...
mod tests {
    ...
    fn unsupported_filter_type_is_unavailable_profile() {
        let filter_type = DemuxFilterType { mainType: DemuxFilterMainType::MMTP, subType: DemuxFilterSubType::TsFilterType(DemuxTsFilterType::SECTION) };
        assert!(matches!(filter_open_type(&filter_type), Err(HalError::Unsupported(_))));
    }
}
