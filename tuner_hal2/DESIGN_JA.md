...
| Generic key provisioning ingress | protocol framing ownerは`key_provisioning_bridge/src/lib.rs`、I/O ownerは`aidl_service/src/key_provisioning_bridge_server.rs`、runtime command適用ownerは`service_runtime/src/key_provisioning_ops.rs::TunerServiceRuntime::apply_key_provisioning_command`、reservation/key-slot状態ownerは`service_runtime/src/descrambler_key_table.rs` | Tuner外部の鍵供給adapterからの`Ping` / `Reserve` / `Publish` / `Revoke`。I/O完了後にruntime lock下で型付きcommandを一回だけ適用する | AIDL façade、packet path、TISからのkey table直接変更、I/O中のruntime lock保持、再送目的の要求履歴・status・鍵素材copyの保存、予約なしpublish、provider identity不一致、stale epochの成功化、CA system ID/B25/B1/MediaCas session等の上流意味をTuner内部で解釈すること |
...
request IDは一つの接続で受けた要求と応答の対応確認だけに使用する。serverは要求履歴、status、鍵素材を保存せず、decodeに成功したcommandを接続ごとに一回適用して結果を応答する。同じrequest IDを別接続で受けた場合も履歴照合せず、新しいcommandとして現在のruntime stateへ適用する。

CAS側が同じrequest IDで試行するのは、接続または送信準備が`IoUnavailable`となり、Tunerへcommandを渡せていない経路だけとする。要求送信開始後のtimeout、EOF、切断、不正応答では同じmutationを自動再送せず、結果未確定の失敗として上位へ返す。`Reserve`は同一provider identityの未publish予約に対して冪等であり、`Revoke`は同じtokenが既に消滅またはrevoke済みでも成功するため、後続のopen rollbackまたはcleanupは新しいrequest IDで収束できる。`Publish`の応答が未確定ならECM成功を表明せず、session cleanupでtokenをrevokeする。これらのruntime stateで資源寿命を収束させ、別の再送台帳を持たない。
...
- 同一`key_token / provider_id / provider_generation`のReserveは同一reservationとして冪等成功し、leaseをrefreshしてよい。
...
- request/response処理完了後に要求履歴またはstatusを保持しない。
- key schedule/cipher内部で不要な値copyを増やさない。
- secret handlingは鍵供給元の種別に依存させない。

#### 実装owner / anchor

- protocol framing: `key_provisioning_bridge/src/lib.rs`
- socket connection I/O / deadline / decode / command dispatch: `aidl_service/src/key_provisioning_connection.rs`
- socket accept / runtime command binding: `aidl_service/src/key_provisioning_bridge_server.rs`
...
I/O待機中にruntime lockを保持してはならない。socket connectionはrequest frame readとdecodeを終えてからruntime lockを取得し、typed commandを一回だけ適用してlockを解放した後にresponseを書き込む。packet path、AIDL façade、TISがkey tableを直接変更してはならない。

read deadlineとwrite deadlineの設定成功はrequest処理開始の必須条件とする。いずれかの設定に失敗した接続では、frame read、decode、typed command適用を一切開始しない。期限設定失敗をdeadlineなしI/Oへ降格せず、Tuner runtime mutationへ到達させない。
...
- decoded commandを接続ごとに一回だけruntimeへ適用し、同じrequest IDの要求履歴を保持しない。
- mutation適用後のresponse write失敗で、server内部からmutationを再実行しない。
- 同一provider identityのReserve反復でslot数を増やさない。
...
- read timeout設定失敗とwrite timeout設定失敗の各々で、frame read/decode、runtime command適用が0件である。
- 両timeout設定成功時はdecode、runtime command適用、response経路へ進む。
