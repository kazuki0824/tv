...
Snapshot は read-only query DTO として許容する。rollback token は snapshot 本体を保持しない opaque id とし、snapshot 本体は `DemuxRuntime` 内部 ledger に保持する。同一 demux では active rollback authority を一つに限定し、別 transaction が active token と並行して新しい rollback token を発行してはならない。restore / commit は指定された token id だけを one-shot consumeし、同一 demux の無関係な entry を clear / invalidate してはならない。external join 用 rollback snapshot は control-plane identity に限定し、demux lifecycle/generation、filter/DVR の lifecycle・設定・source relation・queue/backing presence・record attachment等のcontrol-plane projection、filter/DVR queue instance identity、AV shared backing instance identityを保持する。filter の callback artifact presence / delay hint / queued bytes / delay deadline / callback unhealthy、DVR の callback artifact presence / status interval・mask・threshold / playback completion buffer / pending overflow / callback unhealthy、packet pipeline assembler/continuity/resync、FMQ read/write pointer・fill・queue contents、AV client handle/active slot/next data ID/payload length/release状態、stale data ID bookkeepingは data-plane または診断上の可変状態であり、runtime lock外で正当に進行し得るため snapshot一致条件にも復元対象にも含めない。restore は demux id、ledger instance、active token id、snapshot generation、現在 generation/lifecycle、filter/DVR control-plane identity、queue/backing instance identityを先に検証し、pre-operation generationまたは当該transactionがtoken ledgerへ明示登録した一回のgeneration boundary後だけを受理する。単なる`generation + 1`一致をprovenanceとして扱ってはならない。成功時は lifecycle/generationだけを戻し、pipeline/FMQ/AV/stale-IDの現在状態を上書きしない。generation boundaryでassemblerを破棄した場合、その破棄はstream discontinuityとして保持し、旧assembler断片をrollbackで復活させない。検証または復元に失敗した時点では token と ledger entry を消費せず、caller が quarantine、再試行、明示 discard のいずれかを選べる状態を維持する。token の disarm と ledger entry 除去は復元成功後だけに行う。runtime entry 自体が失われた場合も、token が所有する ledger へ明示 discard を実行し、restore failure、quarantine不能、discard結果をtyped outcomeへ残す。未使用 token の Drop は自分の token id に対応する ledger entry だけを破棄し、Drop cleanup の ledger lock failureは飽和 counterへ記録する。poisonを `into_inner()` で吸収してauthority ledgerを継続利用してはならない。既存 queue/backingを別instanceへ置換して「restore成功」としてはならず、必要なqueue/backingが欠落または別instanceに置換済みならrollback failureとし、当該demuxをquarantineする。frontend worker rollbackで複数bound demuxを扱う場合は、rollback authorityのprepare自体もdemuxごとにall-attemptで実行し、`PrepareAuthority`の成功・失敗をtyped outcomeへ残す。一件でもprepareに失敗した場合は、同じprepare attemptで既取得の全tokenを明示discardし、`DiscardAuthority`結果をtyped outcomeへ残してから失敗を返す。既取得tokenやfrontend rollback tokenの破棄をDropだけへ委ねてはならず、frontend authority取得や後続generation preparationが失敗した場合もbound demux authorityとfrontend authorityを明示discardし、primary/discard failureをcomposeする。全tokenをall-attemptで復元し、失敗したdemuxを個別quarantineする。bound demux restoreをfrontend restoreより先に完了し、一件でもdemux restoreに失敗した場合はfrontendだけ旧状態へ戻さずFailedへ隔離してsplit stateを通常runtimeとして残さない。rollback prepare requestはrollback authorityではなくdemux-local request DTOであり、authorityは`service_runtime`が所有する`&mut DemuxRuntime`到達経路とprepareが発行するone-shot tokenで固定する。rollback tokenのread-only generation accessorはworker/rollback consistency check用に限り許容する。crate間domain APIとしてrollback prepare/restore façadeが存在してもAIDL/外部入口として扱わず、service_runtime rollback use-caseを通す。queue descriptor exportはread-only query authorityとして扱い、`QueueDescriptorSnapshot::into_parts()`とgrantor accessorはAIDL DTO変換用に限る。

runtime lock内で完結する source-boundary / filter configure 等の内部 transaction snapshot は、external-join rollback tokenとは別契約とする。内部 snapshot はtransactionが変更するfilter/DVR/pipelineの論理状態を復元できなければならず、復元前の現在値がsnapshotと同一であることを要求してはならない。復元前検証は、対象filter/DVRのkey集合と、snapshot時から存続すべきqueue/backingのinstance identityに限定する。transaction中に新規生成したqueue/backingはrollback時に除去してよいが、snapshot時のqueue/backingが欠落または別instanceへ置換された場合はrollback failureとする。FMQ pointer/content、AV client state、stale data ID bookkeepingに加え、filter queued bytes / delivery deadline / callback health、DVR playback completion / pending overflow / callback healthは内部 snapshotでも上書きせず、現在のdata-plane・診断進行を保持する。callback artifact presence、delay/status hint類も当該transactionの変更対象でない限り復元対象に含めない。pipelineのDebug文字列を状態identityとして用いてはならない。packet pipeline の内部 restore は filter/record-index 等のcontrol-plane設定だけを戻し、assembler/continuity/resync/in-flight packet diagnosticなどtransaction中に進行したdata-plane stateをsnapshotから復活させない。
...
ObjectCloseTxn は close 開始遮断の最小共通部品とする。close begin preflight は `Live | CleanupFailed` のみを closeable とし、`Closed` / `Closing` / `Quarantined` を拒否する。`Closed` に対する public `close()` は AOSP-facing API 境界で成功 no-op にせず、close 済み object への再アクセスとして failure を返す。close 開始時点で対象 object cascade を `Closing` へ遷移させ、cascade entries 全体から必要な callback cleanup と typed domain cleanup command を生成し、cleanup 成功時に `Closed`、cleanup 失敗時に `CleanupFailed` へ遷移する。callback cleanup や Binder callback は runtime lock 内で実行せず、object table 上の `Closing` によって並行 public method を遮断する。frontend 固有の worker停止 / LNB owner-loss cleanup、および LNB public close の backend/runtime cleanup もこの typed domain cleanup command として接続し、service_runtime 側で先に begin close しない。Tuner / Demux / Filter / Dvr / Descrambler のように domain cleanup 実体を持たない entry は command を生成しない。AIDL executor に no-op policy を置いて吸収してはならない。
...
| 単一 object quarantine 遷移 | `RuntimeObjectTable` 内の lifecycle helper | 外部から直接呼ばせない。object lifecycle の公開入口は cascade 経路へ統一する |
...
source 切替の demux runtime 内処理は `SourceBoundaryTxn` を通す。`SourceBoundaryTxn` は sink endpoint、AV/non-AV queue要件、generation増分可能性を先に検証し、既存upstream disconnect、demux generation boundary、packet pipeline reset、connect commitをtransaction-owned orderで実行する。AV filterはnormal FMQを持たないためqueue存在を要求しない。export済みFMQのshared read/write pointerをHAL側からresetしてはならず、source disconnect/configure/stop/flush時のqueue clearはdescriptor identityを維持したまま`current_fill()==0`を確認するだけとする。非空queueはframework peerが未drainである状態としてfailureにし、HALがsecond readerとしてdrainしたり、別queueへ置換したりしない。既存upstream disconnect、demux generation boundary、packet pipeline resetをcallerへ手書きしない。source boundary失敗時は新sourceへcommitせず、precondition/generation validation failureでは既存sourceもdisconnectしない。snapshot rollbackはqueue/backing identityを検証し、復元不能ならdemuxをquarantineする。`setDataSource(null)`も同じsource boundaryを通す。`SourceBoundaryReport`はconnect/disconnectをvariant-specific targetとして保持し、finish未実行は`InvariantFailure`として記録する。
...
| frontend worker start / body / rollback / stop cleanup | backend tune / scan session open / worker body / commit の失敗 | worker stop/join、frontend rollback token restore、bound demux snapshot restore、failure marking、live-pump cleanup の失敗 |
...
runtime 再初期化は `TunerServiceRuntime::boot_from_probe_results()` を AIDL service entry から直接呼んではならない。AIDL service 側で runtime を再構成する場合は `AidlServiceContext::reset_runtime_from_probe_results()` を唯一の入口とし、DVR playback worker全停止、DVR notifier全停止、callback artifact全clear、drop-leak診断記録clear、runtime bootの順で同一ownerに閉じて実行する。これらのboot前cleanupはall-attemptであり、DVR playback workerまたはDVR notifierのcleanup failureだけでcallback artifact reset、drop-leak診断記録clear、runtime bootへ進む処理をfail-fastしてはならない。各 attempt result は service boot split diagnostic に記録し、DVR notifier reset については notifier store lock poison recovery と各 notifier join の success/failure を `DvrStatusNotifierCleanupDiagnosticRecord` として bounded store に保存する。DVR notifier runtime loop の terminal reason と supersede cleanup outcome も同じ lifecycle diagnostic store へ保存する。diagnostic record 自体の記録失敗も fail-fast せず全 split outcome の記録を試行してから合成する。public reset failure は first-error projection とする。DVR notifier store lock poison では `poisoned.into_inner()` で store を取得して既存 notifier の停止を試行し、poison error 自体は reset failure composition に含める。runtime boot reset では startup/descrambler/child-open/DVR post-commit/DVR status notifier cleanup/DVR playback worker cleanup/DVR playback consume/queue descriptor query/filter callback/frontend callback/demux transaction/object cleanup の bounded diagnostic store を reset 後状態と混在しないよう clear する。clear failure は startup diagnostic に残すだけで成功扱いにせず、後続 boot attempt を継続したうえで service boot reset の failure composition に含める。drop-leak 診断記録 clear の lock poison は boot 前 cleanup failure として返し、`poisoned.into_inner()` で吸収してはならない。test も production と同じ context-owned callback store / drop-leak diagnostic store を使い、test-only global callback store や process-global drop-leak diagnostic store を置いてはならない。
...
| callback 未登録 / callback store failure | Binder delivery failure ではなく callback artifact absence / store failure として `finish_callback_delivery_failure_use_case()` へ渡す。service_runtime は artifact lookup failure phase だけで runtime registry unhealthy marking を行わない。scan END では scan session failure 記録だけを行い、registered callback unhealthy marking は実際に callback `Strong` を取得した後の Binder delivery failure に限定する。AIDL delivery façade は runtime registry missing を primary failure に置き換えない |
...
scan END delivery の失敗分類は次に固定する。callback store が `None` を返す未登録状態、callback store lock poison、Binder delivery failure の primary error はすべて AIDL delivery façade が `CallbackDeliveryFailureReport` として service_runtime の finish use-case へ渡す。scan session callback failure marking、registered callback unhealthy marking、registry missing の扱い、primary+cleanup failure composition は service_runtime が一元的に決定する。AIDL delivery façade は `RuntimeCallbackRegistry::mark_unhealthy()`、scan session failure marking、`compose_primary_cleanup_failure()` を直接呼ばない。

## 7. Drop leak / callback cleanup の共通部品境界
Drop 経路は public close の代替ではない。Drop leak は通常 close cleanup を実行する経路ではなく、quarantine / diagnostic / runtime unregister によって leak を終端させる経路である。全 AIDL object の `Drop` は `drop_leak_object_from_drop()` だけを呼び、戻り値を返せない `Drop` 上の error は `AidlServiceContext` owned drop-leak 診断記録へ保存する。drop-leak 診断記録を process-global `OnceLock` / `static Mutex` へ置いてはならない。drop-leak 診断記録は service lifetime 中に bounded とし、上限超過で古い record を捨てた件数と、診断 store lock poison により記録できなかった件数を context-owned counter として観測可能にする。object 種別固有の追加記録が必要な場合も、`Drop` 実装や AIDL 側 action enum へ個別手順を書かず、service_runtime 側の drop-leak plan が typed domain cleanup command を生成して決定する。Drop leak 経路で行う quarantine / diagnostic / runtime unregister の failure は Drop leak terminalization failure として composed failure 方針に従う。Drop leak で callback store cleanup を行う場合、callback store lock を `TunerServiceRuntime` lock の内側で取得してはならない。
...
frontend worker start 後に fallible commit を置く場合、commit failure は起動済み worker を二段階停止で stop/join し、frontend runtime 内部 ledger の one-shot rollback token と bound demux snapshot の rollback を試行してから返す。worker start 成功後の commit failure を単に返して worker slot / thread を残してはならない。rollback 中の cleanup failure は primary commit failure を無音で上書きせず、composed failure 方針に従って primary/cleanup の関係が分かる形で表面化する。

frontend runtime の mutation authority は、外部 crate が汎用 `transaction()` façadeを取得して prepare / restore / install / record / tune / scan を任意順に組み立てる形にしない。`service_runtime` registry が `FrontendRuntime` の唯一の production owner となり、`service_runtime::FrontendTxn` が `&mut FrontendRuntime` を所有する critical section 内で、operation-specific typed request method を呼ぶ。これらの method が crate 間利用のため `pub` であること自体は authority 公開とみなさないが、AIDL / binder / domain_request が runtime 所有権を取得したり `FrontendTxn` を迂回したりしてはならない。frontend rollback ledger の lock poison は authority ledger の整合性 failure として fail-closed にし、`poisoned.into_inner()` で capture / match / restore / Drop cleanup を継続しない。rollback snapshot は lifecycle mutation state（state、generation、live reader descriptor、terminal-event minimum generation、scan session、active tune request）だけを保持し、signal state、terminal event history、live-pump report、diagnostic write failure count、last error のような単調追加 diagnostic/history stateを巻き戻してはならない。token Drop 時の ledger cleanup failure は saturating counter に記録する。
...
- queue descriptor snapshot と queue instance identity の保持
- filter / record DVR の HAL-writer endpoint、playback DVR の HAL-reader endpointという role-separated ownership
- filter / DVR の queue descriptor export 境界
- EventFlag wake / wait と、同一descriptorからshared pointerをresetせずpeer/wait endpointをmappingするshim境界

`SynchronizedReadWrite` queue は one-writer / one-reader cardinality を前提とする。safe Rust API は同一endpointからreadとwriteの両方を提供せず、filter/record DVRは`FmqWriter`、playback DVRは`FmqReader`を所有する。framework側を模擬するtest/adapter endpointはexport descriptorから`resetPointers=false`で別mappingし、HAL endpointのroleを反転利用しない。descriptor-only handleはread/write authorityを持たず、productionでpeer endpointを生成する用途はplayback workerのEventFlag waiterに限定する。HAL writerのfill観測は容量と`availableToWrite()`から導出し、HAL readerは`availableToRead()`を使う。HAL writerがclearのためにread/drainして第二readerになることを禁止する。filter/DVR queue instanceの生成はdescriptor export前の初回runtime登録に限定する。descriptor export後のshared pointer resetは、別processのframework peerの停止をHAL内の`&mut`やmutexだけでは証明できないためproduction禁止とする。flush/stop/reconfigure/source-boundary repairは既存queue instanceを保持し、queueが既にemptyであることを検証する。non-empty、role mismatch、capacity mismatch、instance mismatchはfailureとしてtyped reportへ残し、queueを新規置換して成功扱いしない。

queue runtime はAIDL method bodyに局所実装しない。AIDL層はdescriptor query/handle export façadeに留め、実queue role、descriptor identity、readiness、playback consumer worker lifecycleはdemux runtime/service_runtime/AidlServiceContextが所有する。
...
- DVR `start()`後のstatus callback delivery/notifier起動failureはbest-effort notification failureとし、started commit済みのpublic `start()`を後段failureへ反転させない。DVR `stop()`後のnotifier停止failureもstopped commitを反転させない。DVR post-commit diagnosticはBinder delivery、runtime policy skip、artifact lookup/missing、notifier preflight/runtime/cleanupをphase別に保持する。callback unhealthy markingはStrong取得後のBinder delivery failureだけに限定し、policy skip、artifact missing/store failure、cleanup failureを再分類しない。notifier start/supersede/stop/resetはcontext-owned lifecycle lockで直列化する。supersedeはruntime poll、policy preflight、artifact lookup、store lock取得を旧notifier removalより先に完了する。新threadはstore ownershipを確保した状態でstart gate付きでspawn/insertし、旧notifierのcancel/joinが完了するまでcallback loopを開始しない。preflight/store/spawn failureでreplacement不能な場合は旧notifierを維持し、spawn済みthreadをstore/cancel ownerなしでdetachしてはならない。worker terminal errorはdiagnostic記録後も`JoinHandle<Result<...>>`へErrとして返す。service reset/drop-leak cleanupはall-attempt cleanupとし、public resultはfirst-error projectionでもtyped recordsを保持する。DVR post-commit diagnosticはproduction snapshotでrecords、dropped count、record failure countを取得可能にする。
- playback DVR はHAL-reader endpointを所有し、production `AidlServiceContext` がDVR object/generation単位のconsumer workerを管理する。workerはexport descriptorと同じEventFlag wordへmappingしたwait-only endpointで`DATA_READY`を待ち、service_runtimeのobject/generation検証を経てFMQ read→TS packet assembly→全完成packetのservice_runtime packet transaction→`DATA_CONSUMED` wakeを実行する。同一FMQ batchのpacket transactionはall-attemptとし、一部packet failure後も後続packetを試行し、成功packetのgenerated eventとconsume reportを失わない。`DATA_CONSUMED` wake failureはpacket処理後に有限回再試行して記録し、既生成eventのdispatcher配送を妨げない。packet-local transaction failureは当該packetをdropして診断し、workerを継続する。`start()`はworker登録失敗時に今回`StartedNow`へ遷移したruntimeだけをrollbackし、`AlreadyStarted`の既存sessionを停止しない。active workerへの冪等startはreplacementを行わず成功とし、terminal workerだけをjoin回収してruntimeがStartedであることを再検証してから新workerをcommitする。`stop()`/close/resetはcancel、wake、joinをall-attempt cleanupする。playback consumptionを`#[cfg(test)]` helperだけに閉じてはならない。
...
AV shared backing のslot allocation/release/release_allはactive setとfree slot setの片側だけを更新して成功扱いにしてはならない。configure/rollback snapshotはbacking存在markerだけでなくinstance identityを保持する。AV再configureはexport済みbackingを新instanceへ置換せず、既存instanceを再利用する。rollback時にinstance mismatchがあればrestore failureとしてquarantineする。configure中に追加されたstale data IDはpre-operation snapshotで全置換せずunionし、後続`releaseAvHandle()`が既知staleとして受理できるようにする。release時にmarkerとruntime backing実体が乖離している場合はtransient backingを生成せずbacking failureとする。
...
public `close()` は `Live | CleanupFailed` のみを close preflight 成功対象にする。`Closed` / `Closing` / `Quarantined` の close は成功 no-op ではなく public access failure として返す。AIDL façade は `AlreadyClosed => Ok(())` のような close 意味論を所有せず、`ObjectCloseTxn` の preflight 結果を Binder error へ橋渡しするだけにする。
...
- `SourceBoundaryTxn` の construction / step recording / outcome 操作は source boundary transaction 本体が所有し、外部観測は immutable `SourceBoundaryReport` のみにする。
  `SourceBoundaryReport` / `FilterConfigureReport` / `DvrConfigureReport` / `FilterRuntimeOperationReport` / `DvrFlushReport` はservice_runtimeのbounded production diagnostic storeにtyped recordとして接続する。demux transaction diagnosticのproduction accessorはrecords、dropped count、demux rollback-token Drop cleanup failure countを返す。Filter/DVR configureのvalidation-only failureは`Failed`とし、`RolledBack`はfailed stepとrollback stepを保持する。DVR status mask/threshold適用は`DvrConfigureTxn`内に置く。Filter configureはsoft-demux config適用後に`SourceBoundaryTxn`を最終境界として実行する。export済みFMQはpointer reset/new queue replacementを行わず、role/capacity/instanceが一致しqueueがemptyであることを要求する。filter stop/flushおよびDVR flushは各step outcomeをtyped reportに保持し、queue non-empty、native read/write failure、queued payload clear failure等をplain `Result`だけで失わせない。復元不能またはidentity不一致はquarantineし、全体rollback済みと表現しない。filter runtime stop / flush の pipeline step 後に queue clear が失敗する場合も、pipeline-only rollbackで全体が戻ったと表現せず、partial-effect outcome、quarantine、queued payload clear / AV backing flush / stopped marking の実行・skip decisionを `FilterRuntimeOperationReport` に残す。AIDL object method 経由の `IFilter.stop()` / `IFilter.flush()` も service_runtime の transaction façade を通し、typed report を破棄する raw demux runtime helper を直接呼ばない。demux transaction typed diagnostic record は production で取得可能な単調増加 diagnostic id を持ち、public `HalError` detail が source-boundary / configure / filter runtime operation report を要約する場合は同じ diagnostic id を含める。public `HalError` detail は Binder/AIDL status bridge 用の要約であり、typed report の唯一の保持場所にしてはならないが、public failure と typed record の対応を追えない状態にしてはならない。`Unsupported` 相当の public status を維持する必要がある動的 source-boundary failure では、static-only `Unsupported(&'static str)` ではなく diagnostic id を持てる dynamic detail variant を使う。
...
- query surface は `RootQueryRequest` / `RootQueryResponse` と `ObjectQueryRequest` / `ObjectQueryResponse` の DTO 境界に限定し、registry entry を AIDL façade へ返さない。
...
- Drop leak cleanup は service_runtime の quarantine plan が対象 object と DVR notifier artifact command を所有する。AIDL 側で drop leak 対象や DVR notifier stop 要否を再判断しない。
- domain cleanup command は service_runtime が必要な object kind にだけ生成する。`Tuner` / `Demux` / `Filter` / `Dvr` / `Descrambler` へ通常 domain cleanup command を生成してはならない。unsupported object kind を AIDL executor 側の `Ok(())` no-op 分岐で吸収してはならない。
...
- filter flush は packet-derived `PacketPid` ではなく設定由来 `ConfigInputPid` を使い、TS packet 由来 PID と設定 PID を混同しない。
- callback registration rollback failure の rollback command 生成、unhealthy marking、primary+rollback failure composition は service_runtime use-case が所有する。AIDL façade は callback artifact retain bridge と service_runtime 発行 cleanup command の bridge 実行結果を service_runtime outcome / finish use-case へ渡すだけにし、retain 済み flag や domain commit failure 分岐を独自に持たない。
...
frontend workerの停止・置換はprepare/external join/completeの三相で扱う。prepareは対象frontend、AIDL object/generation、worker kind、停止対象worker generation、new generation candidate、frontend rollback token、bound demux rollback token/generation snapshotを固定する。frontend rollback ledgerはsingle active authorityとし、poisonを吸収しない。snapshotはlifecycle復元に必要なstate/generation/live reader/scan session/active tune requestだけを持ち、terminal event、live-pump report、diagnostic write failure、last error、signal historyをrollbackで巻き戻さない。expected post state は generic runtime snapshot setter ではなく operation-specific API で登録する。tune は予定したrequest値、worker generation、live reader descriptor identity、terminal-event minimum generationを固定し、scan はworker generation、live reader identity、terminal-event minimum generation、scan sessionのstable fingerprintを固定する一方、正常なcandidate index/phase進行は許容する。expected post state登録はworker spawnより前に行う。tuneではgeneration/live reader/lifecycle/active tune request commitとexpected post state公開を同一frontend runtime操作にし、scanではgeneration/live reader/lifecycle/scan session commitとexpected post state公開を同一frontend runtime操作にする。いずれもrollback ledger lock取得・token再検証などのfallible stepをruntime mutationより前に完了し、一方だけcommitされたpartial stateを作らない。登録/commit失敗時はfrontend/demux rollbackとquarantineをall-attemptで実行する。external joinはruntime lock外で行う。complete/cleanupはgeneration、worker kind、lifecycle、scan session relation、live-reader presenceを再検証し、古いworkerが新generationのreader/sessionをclearできないようにする。rollback restoreはtoken owner、active authority、現在pre-operation状態またはoperation-specific期待post-operation状態を検証し、成功後だけtokenをconsumeする。bound demux rollbackはall-attemptかつdemux-firstで実行し、失敗demuxを個別quarantineする。bound demux authority の prepare/discard は demux target と token-list/store authority target を別variantで記録し、架空のdemux idへ押し込まない。tune-start generation boundary は全bound demuxでtoken・current generation・次generationのprovenanceをmutation前にpre-authorizeしてopaque one-shot commit capabilityを発行し、全件preflight成功後はruntime lockを保持したままfailureを返さないcommit phaseだけで全boundaryを適用する。一件でもdemux rollbackに失敗した場合はfrontendを旧状態へ戻さずFailedへquarantineし、frontend/demux split stateを通常runtimeとして残さない。join後にfrontend idだけを根拠としてmutationしてはならない。

frontend worker replacement では、旧 worker を stop/join した後に、新 worker generation candidate の算出、bound demux rollback token preparation、request validation、scan candidate calculation、または新 request 開始に必須で旧 worker stop 前に実行可能な fallible pre-start preparation を初めて実行してはならない。旧 worker stop 前に実行できる generation candidate calculation / rollback-token preparation / request preflight は prepare phase へ移し、ticket に含める。new worker generation candidate を実 runtime state へ commit する install / begin step は complete success 後に実行してよいが、その入力は ticket 内の candidate、frontend runtime 内部 ledger を参照する opaque one-shot rollback token、bound demux rollback token / generation snapshot から導出し、ここで新たに generation candidate calculation や rollback-token preparation を行ってはならない。complete 失敗は replacement cleanup report と同じ transaction outcome に `CompleteReplacement` 相当の typed stepとして失敗を記録し、成功として記録してはならない。旧 worker が停止済みの場合、旧active runtime snapshotだけを復元してworker不在のTuning/Scanning状態を作ってはならず、旧workerの再始動を同一transaction内で完了できない限りfrontendをFailedへ隔離する。旧 worker が停止済みであること、新 worker generation が未開始であること、rollback / restart / quarantineの実行結果をtyped diagnosticに残す。replacement stop reportのdiagnostic store障害はshared sinkのrecord-failure counterへ残し、旧worker停止後のlifecycle処理を中断してはならない。旧scan worker停止後のscan cancel state反映が失敗した場合は、新scan開始へ進まず、bound demux rollbackとfrontend quarantineをall-attemptで実行する。complete success 後の install / begin / worker start failure は TuneStartRollback / ScanStartRollback / WorkerStartRollback 等の start rollback diagnostic に、generation candidate と旧 worker stop 済みの文脈を失わない形で記録する。この条件を満たさず、旧 worker stop 成功後の `prepare_frontend_worker_generation()` などの fallible preparation failure だけで public tune / scan を failure へ返す設計は不可とする。
...
DVR playback worker は playback FMQ から完成TS packetを取り出した後、frontend入力と同じ service_runtime packet transaction を通してdescrambler decision、key/source-filter generation検証、packet diagnostic、demux pipeline配送を行う。consume report はbytes/completed packet/malformed/wake failureを飽和集計し、drop/suppression/delivery/packet diagnosticは異常reportだけをdata-path diagnosticへ保持する。最後に観測した異常reportは、後続の正常consumeで空vectorへ上書きしない。consume diagnosticはbounded storeとdropped counterを持ち、boot/reset時にclearし、object世代の増加で無制限に増加させない。record内部のpacket failure vectorとanomalous pipeline report vectorにも件数上限を設け、省略件数を保持する。正常consumeをcleanup diagnostic storeへ投入してはならない。generated filter eventsはDATA_CONSUMED wake結果または同一batch中の別packet failureにかかわらずdispatcherへ渡す。packet-local failureはconsume reportへ記録してworkerを継続し、有限回再試行後も解消しないDATA_CONSUMED wake failureだけをevent抽出・配送後のworker terminal infrastructure failureへ接続する。同一FMQ batchのpacket transactionはall-packet attemptとし、失敗packetのindex、PID、registry lifecycle / descrambler resolution / packet validation / pipeline deliveryのfailure phase、errorをtyped outcomeとしてconsume diagnosticへ保持する。packet-local failureは当該packetをdropして診断し、同batchの成功eventを配送したうえでworkerを継続する。DATA_CONSUMED wakeは有限回再試行し、全試行失敗だけをworker terminal infrastructure failureとする。completed packet総数、成功report数、失敗packet数の意味を混同してはならない。worker lifecycle/store lock failureはpoisoned guardから全worker cleanupを継続し、`LifecycleLock`/`StoreAccess`/`Wake`/`Join`/`Terminal`のtyped outcomeを共通`CleanupExecutionReport`とshared diagnostic sinkへ保持する。各cleanup attemptにはoperation種別（replacement/public stop/object close/service reset）と単調増加attempt IDを付け、attempt ID枯渇は明示failureとして記録し同じIDを再利用しない。各attempt末尾にはexpected step countを持つAttemptComplete markerを記録し、bounded storeで個別stepだけが残った場合も同一attemptの完全性を判定できるようにする。shared diagnostic sinkのrecord failureはsink自身の飽和record-failure counterへ残し、worker cleanup成功/失敗やreplacement可否を反転させない。worker replacementはactive workerには行わず、terminal workerを停止・joinしてからruntime Started状態と現在のFMQ endpointを同一再検証phaseで再取得し、その新しいwaiterだけを新consumerへ渡してstoreへcommit/startする。terminal workerのreplacement可否はdiagnostic記録成功ではなく、そのworker自身のruntime fail-close成功flagを正本とする。old workerのwake失敗はjoinとterminal result回収が成功した場合は診断に残す補助通知failureでありreplacementを反転しない。old worker回収後のnew worker spawn failureはruntimeをfail-closeし、Started/workerなしを残さない。waiter取得からgated workerのstore commitまでruntime lockを保持し、configure/stop/queue replacementが間に入ってstale endpointをcommitできないようにする。複数consumerを同時稼働させず、旧worker cleanup前に取得したwaiterを再利用してはならない。terminal worker entryはterminal markerによって識別し、join後のterminal markerとworker-owned runtime fail-close stateを判定する。terminal errorをreplacement判定から除外できるのはruntime fail-close成功を確認できる場合だけとし、runtimeがStartedのままならreplacement failureとして扱う。public stop commit後のcleanup failureはpublic戻り値を反転しないが、typed cleanup reportとpost-commit diagnosticの双方に残す。

callback artifact cleanup、callback registration rollback、object close callback cleanup、service boot reset callback cleanup は runtime prepare -> artifact command execution -> runtime finish の all-attempt transaction とする。runtime registry は callback lifecycle accounting の正本であり、artifact mutation command id を発行する。AIDL 層は command を実行し、outcome を runtime finish へ返す。artifact failure、runtime finish failure、registry missing は callback artifact/runtime split diagnostic に必ず保持する。primary failure と cleanup / finish failure が同時に存在する場合は failure composition helper 群で composed failure を作成し、同時に split diagnostic も記録する。FirstErrorCollector は同一 cleanup phase の cleanup step first error 収集だけを担当する。object close / drop-leak terminalization、frontend worker cleanup、frontend tune/scan rollback cleanup、frontend close owner-loss cleanup は all-attempt 実行後、各 step outcome を domain-specific adapter (`ObjectCleanupStepOutcome` / `FrontendWorkerCleanupStepOutcome`) として保持し、共通 `CleanupExecutionReport<TStepOutcome, TFailure>` から public failure 用の first-error を射影する。object cleanup の artifact/domain/runtime 差分と frontend worker cleanup の object-target/frontend-target 差分は variant-specific target/step outcome で表現し、nullable field 群で後から意味を復元する形にしてはならない。public failure を first-error に畳む場合でも、plan-owned result から成功 step、失敗 step、cleanup kind、対象 id / generation、cascade target detail、worker kind / worker generation、scan cancel / live-data cleanup result、frontend rollback token restore result、bound demux rollback result、owned LNB close result を失ってはならない。AIDL façade や service_runtime use-case が public `BinderResult` / `HalError` へ変換する前に report を `.into_result()` だけで破棄してはならず、`SharedCleanupDiagnostics<TRecord>` を基盤にした shared diagnostic sink に typed diagnostic record として記録する。production accessor は `CleanupExecutionDiagnosticSnapshot<TRecord>` として records だけでなく bounded store の dropped count も返し、overflow 監査を可能にする。object close / drop-leak terminalization report 記録は runtime lock 再取得に依存してはならず、frontend worker cleanup も可能な限り runtime instance から先に clone した shared sink へ記録する。finish/terminalization/worker cleanup result と診断記録失敗は failure composition で両方表面化する。artifact mutation 成功後に runtime finish lock が失敗した場合も、AIDL が独自診断 store を持つのではなく、service_runtime instance が発行した shared diagnostic sink へ `CallbackArtifactRuntimeSplitDiagnosticRecord` を記録し、artifact/runtime split failure として診断に残す。
...
## 追補: frontend rollback expected-state と demux boundary capability

- frontend tune/scan の rollback expected-post state は、generation、live reader、operation request/session、lifecycle stateに加えて、post-operationの `terminal_event_min_generation` を同一commit内で記録する。pre-operation値をexpected stateへ保存してはならない。
- 複数bound demuxのgeneration boundaryは、全対象のruntime存在、demux ID一意性、rollback authority、next generationをrelease buildで検証したone-shot capabilityだけをcommitする。`debug_assert!`を正当性条件に使用しない。
- capability commitは対象数の完全一致を確認し、missing runtime、duplicate ID、stale authorityを成功へ縮約しない。
- internal control-plane rollbackは、現在進行したpacket data-planeを巻き戻さない。record index設定を戻す場合、設定が変化したparser stateを再利用せず、旧parser carryを持たないparserを途中生成してbyte numberを継続したように見せてはならない。
- syntax sectionは `section_number <= last_section_number` を満たすものだけを正常headerとして扱う。CRC実装はproduction pathから利用可能な共通部品とし、test専用実装へ閉じ込めない。
- bounded PES完成時はdeclared PES lengthを越えるbytesを当該PESへ混入させず、次のPUSIで未完了bounded PESを無診断破棄しない。
