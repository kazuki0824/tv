...
| `FilterProducerDrainGate` | `demux/src/runtime/queue_runtime.rs::FilterProducerDrainGate`。局所取消し失敗はgate内のatomic失敗状態から既存`QueueRuntimeError`へ写像する。 | Filter/SharedFilter data path、`QueueCleanupUseCase`からのtyped入口 | 公開API/worker/`QueueCleanupUseCase`からのgate内部直接変更、DVR ownerとの統合 |
