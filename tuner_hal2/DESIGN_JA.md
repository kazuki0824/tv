...
### ISDB-S dynamic stream-ID device-adaptation境界

backend固有のstream-ID readbackは「機器適合」責務に閉じる。px4 ABI mirrorの実装anchorは `device/src/px4/abi.rs::PtxTmccTsidList` / `PTX_GET_TMCC_TSID_LIST`、raw resultのshape検証と `EAGAIN` のtyped pending化は `device/src/px4/tmcc_tsid.rs` とする。Linux DVB / earth_pt1は`device/src/dvb/abi.rs::FE_GET_PROPERTY` / `DTV_STREAM_ID`からcurrent TSIDを読み戻す。両backendともactive sessionのdevice-open resourceを再利用する共通read entryを `device/src/runtime/backend_worker.rs::FrontendBackendSession::observe_stream_id_list()` とし、公開値、readiness、scan callbackの規範意味は `../tuner_hal/DESIGN_JA.md` を正とする。
...
| `FrontendTuneScanTxn` | `service_runtime/src/frontend_ops.rs::FrontendTuneScanTxn`が、AIDL外形検証済みtyped requestを受けた後のcanonical preflight、固定給電準備、worker start/stop、rollback、operation event / terminal acceptanceを直接所有する。`service_runtime/src/frontend_worker_txn.rs::commit_and_deliver_frontend_scan_lock()`は`LockedReported`を先にcommitしてからcallbackを配送する。product-profile / backend availabilityの第二ownerをAIDL serviceまたはdevice mapperへ置かない | `FrontendTuneScanTxn`の有限正規入口集合 `begin_tune` / `begin_scan` / `stop_tune` / `stop_scan` / `accept_operation_event` / `accept_worker_terminal`。AIDL境界はAIDL外形検証・typed変換後に`begin_*` / `stop_*`だけを呼び、ワーカー・下位機器処理の完了通知橋渡しは`accept_*`だけを呼ぶ | AIDL serviceによるproduct-profile / backend availability判定、worker・機器層・callback層によるcanonical preflightの迂回、callback先行による未commit状態への再入、Demux所有者の吸収、第二の正規所有者化、有限正規入口集合外での選局・走査進行の再実装 |
| `AvSyncRegistry` | `demux/src/runtime/av_sync_registry.rs::AvSyncRegistry`。live PCR ID最小値をcanonical relationとして全media filterのforward/reverse indexをprepared mutation内で揃える | filter configure/unregister/close、demux closeからのtyped relation入口 | API/filter wrapper/`StreamBoundaryTxn`からのregistry直接変更、登録順依存のPCR選択、PCR ownerとの統合 |
