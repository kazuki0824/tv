...
| Filter / DVR start commit後callback failure | `PostCommitCallbackFailureTxn`がcommit済みstartをrollbackせずcallback healthと必須診断の更新を所有する。start use-caseはtyped callback failureを一回だけ渡す | API別use-caseまたはworkerが`stop_*()`でrollbackする、`callback_unhealthy`を直接更新する、同型処理を再実装することを禁止 |
...
| Filter / DVR start post-commit callback failure | `service_runtime/src/demux_filter_dvr_ops.rs`内の`PostCommitCallbackFailureTxn`を、domain start commit後のcallback healthと必須診断更新の単一正本とする。同file内のFilter/DVR object operation use-caseとは型と状態所有を分離し、開始済みdomain state、worker lifecycle、rollbackは所有しない | Filter/DVR start completion use-caseがdomain commit後のtyped callback failureを一回だけ渡す | API別use-caseまたはworkerが`stop_*()`でstartをrollbackする、`callback_unhealthy`を直接更新する、同型のcallback失敗処理を再実装する |
| Filter producer drain protocol | `demux/src/runtime/queue_runtime.rs`内の`FilterProducerDrainGate`を`Open`/`Draining`/`Closed`、`filter_delivery_generation`、`parser_state_generation`、`admitted_producer_count`、bounded pending event queue、`FilterProducerPermit(g)`の単一正本とする | filter/shared-filter data pathのtyped producer admission/finishと、`QueueCleanupTxn`からのtyped drain/flush requestだけを入口とする | 公開API、worker、`QueueCleanupTxn`がgate内部状態を直接変更する、permit/drainを飛び越えてFMQ writeまたはpending event追加を確定する |
| DVR queue epoch protocol | `demux/src/runtime/queue_runtime.rs`内の`QueueEpochProtocol`を`Open(g)`/`Draining(g)`/`Closed`、一回限りのread/write transaction token、受付中transaction数、`queue_epoch`の単一正本とする。`PlaybackQueueBacking.queue_identity`は所有しない | DVR data pathの`beginRead`/`beginWrite`/`commit`/`cancel`相当のtyped入口と、`QueueCleanupTxn`からのtyped flush requestだけを入口とする | 公開API、worker、`QueueCleanupTxn`が`queue_epoch`、queue pointer、token stateを直接変更する、または`PlaybackQueueBacking`のqueue identity所有を二重化する |
...
- Filter / DVR `start()`のdomain commit後にcallback failureが発生した場合は、typed failureを`PostCommitCallbackFailureTxn`へ一回だけ渡す。同transactionだけがcallback healthと必須診断を確定し、開始済みdomain stateをrollbackしない。API別use-caseまたはworkerが`stop_*()`で補償したり、`callback_unhealthy`を別正本として更新したりしない。
- `PostCommitCallbackFailureTxn`は`service_runtime/src/demux_filter_dvr_ops.rs`にFilter/DVR object operation use-caseと同居するが、それらの親型、基底trait、派生型ではない。物理的な同居は状態所有の統合を意味せず、start completion use-caseからtyped failureを受けるcomposition/delegationだけを許可する。
- `QueueCleanupTxn`はFilter/DVR公開`flush()`のcleanup orchestrationと失敗集約を所有する。`FilterProducerDrainGate`はFilter/SharedFilterの生産許可排出、`QueueEpochProtocol`はDVR queue transactionとepochを所有する下位protocolであり、`QueueCleanupTxn`は両protocolのtyped入口だけを使用して内部状態を直接変更しない。公開APIまたは`QueueCleanupTxn`がそれらの内部状態を第二の正本として保持しない。
- `FilterProducerDrainGate`と`QueueEpochProtocol`は同じ`demux/src/runtime/queue_runtime.rs`に配置するが、状態機械と所有状態は独立させる。両者に継承関係または共通の状態ownerを設けず、名称上どちらもqueueに関係することだけを理由に共通base trait/protocolへ統合しない。`QueueCleanupTxn`との関係も継承ではなくtyped operationによるcomposition/delegationとする。
...
- `WorkerFailureClassifier`は失敗種別の分類だけを所有する。各API、worker owner、ワーカー終了契約は自身の契約で所有が明示された停止順序、retry/cleanup、状態遷移だけを所有し、失敗種別を再分類しない。Filter / DVR `start()` commit後callback failureのcallback health遷移は`PostCommitCallbackFailureTxn`だけが所有する。
