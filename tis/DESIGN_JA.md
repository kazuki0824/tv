...
JNIの実行失敗と正常な空値の契約は`../arib_si_engine_rs/DESIGN_JA.md`の「実行失敗と正常な空値の区別」を正とする。`NativeAribSiParser`は`NativeSiException`を呼出し元へ伝え、SI事実なし・番組keyなしへ変換しない。`ProviderDataBridge`もnative呼出しの失敗をJSON内容不正として捕捉しない。走査・番組更新側は既存の操作失敗経路で扱い、失敗したsnapshotを登録・削除判断へ使用しない。
