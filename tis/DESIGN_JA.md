...
登録とliveで同じ`ServicePolicyEvaluator`および`TunerSelectionPolicy`の静的audio/video predicateを使う。許可service_type、current deliveryのSMD、PMT/PCR、codec signalingのresolved状態と対応種別、CA descriptor解決状態を同じsnapshotから評価する。未解決CAを空metadataからclearへ推測せず、登録不適格ならCAS処理・EPG公開・再生開始を抑止して旧再生も停止する。free_ca_mode単独の診断は、構造が確定したPMTのCA有無を上書きしない。video付きserviceの未対応audioはvideo-onlyを許す。AVC descriptor不在は許容し、実SPSとの一致および実MediaCodec能力はlive configureで検証する。

現在ServiceKeyの`authoritativeProgramKeysByService`が確定した空集合の場合は、rating resolverへ型付き`AUTHORITATIVE_EMPTY`を渡す。Providerの旧current行を根拠にせずUNRATED/no-current identityへ進む。未観測・未完成のEITは`UNCONFIRMED`として従来のProvider fallbackを維持する。
