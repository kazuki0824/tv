...
現行 product の平文 non-tunneled AV入力は、Tuner `MediaEvent.getLinearBlock()` と `MediaCodec` block model の型付き経路だけを正式経路とする。AOSP Tuner の media filter output は complete ES に限定されず partial ES も許すため、`MediaEvent` 1件、`LinearBlock` 1個、range 1個、PES 1個のいずれもcodec access unit境界とはみなさない。video／audio decoderは `MediaCodec.CONFIGURE_FLAG_USE_BLOCK_MODEL` で構成し、track別のbounded `AccessUnitAssembler` は到着順source rangeを論理的に連結したread-only ES byte streamとして走査する。start-code prefix、syncword、NAL header、slice header、ADTS header、MPEG audio headerその他の境界判定syntaxが`MediaEvent`／`LinearBlock`境界の任意byte位置で分断されてもparser stateを次rangeへ持ち越し、source payloadはdirect queueまたは完成AU再構成が確定するまで元`LinearBlock`に保持する。range境界や`isPtsPresent()`の変化だけでAUを閉じてはならない。

`AccessUnitAssembler` のcodec別境界は次で固定する。MPEG-2 VideoはH.262のaccess-unit定義に従い、`0x000001` start-code prefixをrange跨ぎで認識する。pictureが直前の`sequence_header_code`／`group_start_code`を伴う場合はその最初のstart codeから、伴わない場合は`picture_start_code`からAUを開始し、次AU開始直前で閉じる。最後のpictureに続く`sequence_end_code`はその最終AUに含める。

H.264/AVCはAnnex B byte streamの3-byte／4-byte start-code prefixとNAL/RBSP syntaxをrange跨ぎで解析し、ITU-T H.264 7.4.1.2.3および7.4.1.2.4のNAL ordering／first-VCL-of-primary-coded-picture規則をAU境界判定の正本とする。AUDは補助には使えるが必須とせず、`first_mb_in_slice == 0`だけへ単純化しない。SPS/PPS/SEI等のnon-VCL NALは同節のassociation規則に従って所属AUを決める。

AACは現行ARIB運用で用いるADTSをrange跨ぎで解析し、syncword、`protection_absent`を含むheader、13-bit `aac_frame_length`を検証し、header 7/9 byteやpayloadがfragmentを跨いでもframe全体が揃うまで閉じない。現行対象のADTSは1 frame=1 raw_data_blockの運用を前提とし、この条件を満たさない入力を暗黙に複数AUとして1 QueueRequestへ詰めない。MPEG audioは4-byte headerをrange跨ぎで解析し、MPEG version／layer／bitrate／sampling-rate／paddingから規格上のframe sizeを確定し、その完全な1 audio frameを1 AUとする。header、syncword、source fragment終端だけではAU完成としない。

完成access unitが1つのsource `LinearBlock`上の1つの連続rangeだけで構成される場合は、そのsource block/rangeを`MediaCodec.QueueRequest.setLinearBlock()`へ直接1回だけ渡す。完成access unitが複数`MediaEvent`、複数`LinearBlock`、または非連続rangeに跨る場合は、source fragmentのclaimを保持したままaccess-unit総byte数をchecked arithmeticで算出し、正で`Int.MAX_VALUE`以下かつ同じ`TisPlaybackBudgetSnapshot`の追加reassembly byte予算を原子的にclaimできる場合だけ、`MediaCodec.LinearBlock.obtain(totalAuBytes, arrayOf(selectedCodecName))`相当でTIS所有のmappable blockを確保する。各source rangeの有効payloadだけを到着順にそのblockへ連続copyして**完成access unit 1個**へ再構成し、copy完了後にsource `MediaEvent`／`LinearBlock`／source claimを解放する。再構成blockは`QueueRequest.setLinearBlock(reassemblyBlock, 0, totalAuBytes)`で1回だけqueueし、queue成功または破棄確定までreassembly claimとともに保持する。現行productの正式経路はvideo/audioのいずれについても`MediaCodec.BUFFER_FLAG_PARTIAL_FRAME`または`FEATURE_PartialFrame`を必須能力とせず、decoderのoptional partial-frame capabilityへ再生成立性を依存させない。PTSはpayload fragment全体へ付与せず、`MediaEvent.isPtsPresent()==true`のsource range開始位置に33-bit raw PTSを持つ`PtsAnchor`を置く。anchorはPES header由来timestampの位置metadataとして扱い、anchor位置より前に開始して既に進行中のAUへ**後方適用しない**。1つのanchorはその位置以後に最初に開始するcodec AUだけの候補とし、1 range内に複数AUがあっても全AUへ反復適用しない。複数anchorが同じAU開始位置の候補になる場合、33-bit raw値が同値なら重複metadataとしてcoalesceし、異値ならfirst/last-winsで選ばず、その完成AUだけを`CONFLICTING_PTS_ACCESS_UNIT`としてcodecへ渡さず解放する。後続fragmentのanchorで既開始AUのPTSを書き換えてはならない。plain `Filter.flush()`、full playback generation reset、track入力境界の切替では、その境界より前のincomplete AUに属する未消費anchorを旧入力artifactとともに破棄する。plain `Filter.flush()`はcompleted／queued AUのtimeline stateには作用しない。完成AUに一意の明示anchorを関連付けられた場合だけ、その90 kHz・33 bit値を`PtsNormalizer`へ渡して1つの`presentationTimeUs`をqueueする。関連付け可能な明示anchorが無い完成AUは0、直前PTS、PCR、wallclock、frame rate、sample rateからこの層で時刻を補間・捏造せず、完成AU全体を`MISSING_PTS_ACCESS_UNIT`としてdropする。これはARIB/H.222.0が全AUへのPTS記載を要求するという規範主張ではなく、timestamp derivationを別途仕様化していない現行productの保守的入力ポリシーである。`isPtsPresent()==false`の`MediaEvent`だけを理由にpayload/rangeを単独破棄してはならず、途中fragmentを欠落させない。incomplete access unitは既存のper-track pending byte/count budgetとstartup/backpressure deadlineに従い、上限超過時は当該incomplete unit全体を`INCOMPLETE_ACCESS_UNIT`として破棄するがdeadlineを延長・リセットしない。syntaxが確定的に不正で境界復旧不能な場合は途中fragmentだけを黙って捨てず、次のcodec-valid resync boundaryまでを`MALFORMED_ACCESS_UNIT`として型付き診断し、bounded resyncする。reassembly用追加claimを確保できない場合もsource fragmentを無制限保持せず、既存deadlineとbounded queue規則に従って`AU_REASSEMBLY_BUDGET_EXCEEDED`へ落とす。PTS欠落access unit単体を理由にplayback generationを再生不能へ遷移させず、generation全体のunavailable遷移は既存のdecoder error、lock喪失、startup/backpressure deadline等の独立条件だけで判定する。

`PlaybackPipeline` はplayback generationごとに1個の`PtsEpochCoordinator`と、active compressed trackごとの`PtsNormalizer`を持つ。`PtsNormalizer`の`rawPrev`／`extendedPrev`はtrack別とし、通常のreorderやwrap処理でaudio/videoの直前値を混用しない。一方、MediaSyncへ渡すextended PTSは同一generationの共通media timeline上に置く必要があるため、33-bit wrap epochだけは`PtsEpochCoordinator`が共有する。`M = 2^33`、`H = 2^32`とし、`signedDelta(rawNew, rawRef) = ((rawNew - rawRef + H) mod M) - H`を`[-H, H-1]`の一意な差とする。ちょうど半周期差は`-H`に固定する。generationで最初に受理した明示PTSは、そのtrackのraw値にかかわらず共通extended seed `H`へ置き、coordinatorのreferenceを`(referenceRaw=rawFirst, referenceExtended=H)`とする。まだseedされていないtrackの最初の明示PTS、および同generation内で置換されたtrackの最初の明示PTSは、current coordinator referenceに対して `extendedSeed = referenceExtended + signedDelta(rawNew, referenceRaw)` として現在のwrap epochへjoinする。seed済みtrackの2件目以後はtrack-localに `extendedNew = extendedPrev + signedDelta(rawNew, rawPrev)` とする。各明示PTSを受理した後、serial playback executor上でcoordinator referenceをその`(rawNew, extendedNew)`へ更新し、後からjoinするtrackは常にcurrent generation timeline近傍へliftする。したがってgeneration開始時にvideo=`2^33-100`、audio=`50`なら両者のextended差は150 tick、逆順でも150 tickの絶対差を維持し、独立seedによる約`2^33` tickの乖離を作らない。`2^33-1 -> 0`は+1の通常wrap、`0 -> 2^33-1`は-1のreorderであり、negative deltaを含むpresentation-order reorder自体をdiscontinuityとみなさない。現行productはPTS差分の大きさだけから独自のforward/backward thresholdで「non-wrap discontinuity」を推定しない。新playback generationではcoordinatorと全track normalizerを破棄して新しい共通seedから開始する。plain `Filter.flush()`はAOSP契約どおりfilterが生成済みで未消費のdataをclearする入力側操作とし、`AccessUnitAssembler`のincomplete AU、未queue source range／`PtsAnchor`、filter由来pending inputと対応claimだけを破棄する。plain flushだけでは`PtsEpochCoordinator`、seed済み`PtsNormalizer`、decoder、MediaSync、AudioTrack、playback generationをresetしない。decoder再生成、AudioTrack切替／再生成、audio route変更、Surface変更、またはcodec／PID／track graphの変更を伴うretune・track切替は後段のlifecycle契約どおりfull playback generation resetとし、coordinatorと全normalizerを新seedから開始する。filterをstop/reconfigure/restartした場合の`RestartEvent`は旧configuration eventを捨てるevent-validity境界であり、playback graphが変わらない限りそれ単独ではgeneration resetにせず、任意のin-stream PTS jump検出器としても使わない。通常wrap／reorder／large deltaだけではgenerationを再生成しない。`PtsEpochCoordinator`はmodulo epoch mappingだけを所有し、独自media clock、PCR→wallclock変換、独自future/late schedulerへ拡張しない。reflection、および本書で明示したMediaSync final-output観測用Framework-private `@hide` contract以外のnon-SDK APIを禁止する。ES stream全体を`LinearBlock.map()`から`ByteArray`へ複製して通常input bufferへ入れる経路、通常ByteBuffer input modelへの代替処理は禁止する。ただし、複数fragmentから**完成access unit 1個だけ**をTIS所有`LinearBlock`へ再構成する上記のbounded copyはこの禁止に含めない。direct pathのsource `MediaEvent`／`LinearBlock`／claimはqueue成功または破棄確定まで、reassembly pathのsourceはcopy完了または破棄確定まで、reassembly block／claimはqueue成功または破棄確定まで保持する。secure `MediaEvent`は現行平文productの対象外とし、non-mappable inputをcopy可能と仮定する暗黙変換を行わない。

`MediaEvent.getOffset()`／`getDataLength()`は`long`のまま境界検査し、`offsetLong >= 0`、`dataLengthLong > 0`、`offsetLong <= Int.MAX_VALUE`、`dataLengthLong <= Int.MAX_VALUE`を先に満たすことを要求する。次に`offsetLong + dataLengthLong`をchecked additionで算出して`long` overflowを拒否し、clear productでmappableな`LinearBlock`のmapped capacityを`long`へ拡張した値以下であることを確認する。これらをすべて満たした後だけ`Math.toIntExact()`相当でoffset／sizeを`int`へnarrowし、`QueueRequest.setLinearBlock()`へ渡す。implicit cast、truncate、narrow後のoverflow検査は禁止する。`getLinearBlock()`がnull、block model configureまたはQueueRequestが利用不能、上記range／narrowing検査違反、decoderが当該blockを受理しない場合は`BLOCK_MODEL_UNAVAILABLE`または入力不正の型付き診断へ落とす。成功を偽装せず、現generationのfilter、未queue event、decoder、MediaSync、startup queue、budget claimを解放して`notifyVideoUnavailable(VIDEO_UNAVAILABLE_REASON_UNKNOWN)`へ進む。

デコード後のA/V同期とSurface提示はAndroid標準`MediaSync`だけを使用する。decoder output の `BufferInfo.presentationTimeUs` をMediaSyncへ渡すmedia timeの正本とする。video decoder出力は`MediaSync.setSurface(sessionSurface)`後の`createInputSurface()`へ接続し、`BufferInfo.presentationTimeUs * 1000` をoverflow検査してnanosecondsへ変換したtimestampでoutput bufferをreleaseする。audio decoder出力PCMは`MediaSync.queueAudio()`へ`BufferInfo.presentationTimeUs`をmicrosecondsのまま渡し、`MediaSync.Callback.onAudioBufferConsumed()`を受けるまでaudio decoderのoutput buffer IDとPCM `ByteBuffer`（block model出力では`OutputFrame`／output `LinearBlock`を含む）を解放・変更・再利用しない。圧縮入力側は、direct pathではsource `MediaEvent`／input `LinearBlock`を`QueueRequest.queue()`成功時にcodecへ移管し、reassembly pathではsourceを完成AUのcopy完了時に解放してreassembly `LinearBlock`を`QueueRequest.queue()`成功時にcodecへ移管する。いずれも圧縮入力artifactをPCM消費完了までは保持しない。独自media clock、`AudioTimestamp`を使う独自同期、独自future render／late drop schedulerを設けない。

### MediaSync Framework-private final-output observation

stock Android 14 / LineageOS 21 の `MediaSync` はvideo scheduling/dropをnative側で所有し、late frameをinputへ返すdrop分岐と、render対象frameをcurrent outputへattachして`queueBuffer()`する分岐を区別する。一方、公開Java APIにはそのfinal-output成功をvideo clientへ通知するcallbackがない。この不足だけを閉じるため、対象LineageOS platformの`android.media.MediaSync`へ、既存public `MediaSync.Callback`とは別の `@hide OnFirstVideoFrameQueuedToOutputListener` と、opaqueな識別子を同時にarmする `@hide setOnFirstVideoFrameQueuedToOutputListener(long registrationToken, listener, handler)` 相当を追加する。Framework側は`registrationToken`をTIF/TIS固有の意味を持たないopaque tokenとして保持し、listener eventは少なくとも`MediaSync` instanceと成功判定時に固定した`registrationToken`を返す。public SDK、`@SystemApi`、`@TestApi`、Tuner AIDL/VINTFは変更しない。

availabilityをarmするたびに、`PlaybackPipeline`はimmutableな`AvailabilityArm`を作り、Framework-private setterへ十分に広いopaque `registrationToken`を渡す。tokenの数値自体にTIF/TIS意味や単調増加意味を持たせず、**旧arm由来eventがまだ配送され得るtokenと同じ値を新armへ割り当てない**ことだけをallocatorの不変条件とする。native MediaSyncはcurrent armの`registrationToken`を保持するだけでavailability semanticsを解釈しない。arm中にvideo bufferがlate-drop分岐を通過し、current `mOutput`へのattachと`queueBuffer()`がともに成功した時点で、その**成功を判定したarmの`registrationToken`をevent payloadへ固定してから**armを解除する。late-drop、attach失敗、queue失敗、output abandonment、inputへ返したbufferではeventを生成せず、arm状態も消費しない。re-arm後に旧eventのJava配送が遅延しても、event payloadのtokenは生成時の旧registrationから書き換えない。旧armについてeventが今後配送されないことが確定するまでtokenをretireせず、session releaseではtoken namespace自体を破棄する。理論上の整数counter exhaustion／wrapはplayback failureでもgeneration境界でもなく、これを理由にMediaSyncやplayback pipelineを再生成するruntime recoveryを設けない。常時すべてのrender成功を通知せず、TISが必要な`AvailabilityArm`だけをarm/re-armする。
...
TIS のライブplaybackは、Tuner AV filterの平文`MediaEvent.LinearBlock`について、完成access unitが単一blockの連続rangeならそのrangeを直接、複数fragmentなら完成AU単位でboundedなTIS所有`LinearBlock`へ再構成してから、いずれもMediaCodec block modelへ投入する。video decoder出力を`MediaSync.createInputSurface()`へ、audio decoder出力PCMを`MediaSync.queueAudio()`へ渡す標準経路だけを採用する。MediaSyncはsession SurfaceとAudioTrackを所有し、A/V同期、映像提示時刻、音声clock追従を担当する。TISはMediaSyncの外側に独自clockまたは独自frame schedulerを置かない。
...
startup queueと台帳claimを確保した後にAV filterを開始し、上限内の`MediaEvent`からMPEG-2 sequence header、H.264 SPS/PPS、AAC ADTS/AudioSpecificConfig、MPEG audio headerを収集して`MediaFormat`を構成する。header解析に必要な最小範囲だけ`LinearBlock.map()`でread-only参照してよいが、ES stream全体を`ByteArray`へ複製して通常input bufferへ移送してはならない。完成AUが複数fragmentに跨る場合に限り、本書「再生経路」のbounded reassembly契約に従ってTIS所有`LinearBlock`へ完成AU 1個分だけcopyしてblock modelへ投入してよい。decoder構成成功後は同じsnapshotのsteady-state上限へ遷移し、direct pathではstartup queueの`MediaEvent`／`LinearBlock`所有権をblock model QueueRequestへ移し、reassembly pathではcopy完了後にsource所有権を返してreassembly blockのclaimをqueue完了まで保持する。runtimeで観測したdecoder block capacityは各sampleの投入可否と製品profile検証の診断にだけ用い、開始済み世代のsnapshotまたは予約量を書き換えない。検証済み最小容量を満たさないdecoderではfilterを停止し、claimとHAL handleを解放して`DECODER_CAPACITY_MISMATCH`を記録し、`notifyVideoUnavailable(VIDEO_UNAVAILABLE_REASON_UNKNOWN)`へ進む。
...
必要なqueue領域とclaim台帳はplayback generation開始時に原子的に予約する。各eventはrange検証後、block model投入前に`dataLength`をsnapshot台帳へclaimし、いずれかのevent、byte、sample、duration上限を超える場合は原因別に`SAMPLE_TOO_LARGE`または`PENDING_QUEUE_FULL`を記録してHAL handleを解放する。fragmented AUのreassemblyではsource claimを保持したまま`totalAuBytes`分の追加reassembly claimを同じsnapshot台帳へ原子的に予約し、二重保持のpeakもoffline検証済み上限へ含める。copy完了後にsource claimを返し、reassembly claimはQueueRequest成功または破棄まで保持する。claim済みbyte、sample、durationはQueueRequest成功、copy完了、破棄、generation変更、stop、releaseの各ownership境界で正確に返す。HALの`avPerFilterLiveBytes`または`avRuntimeBudgetBytes`をTISへ公開・複製・1event上限化しない。
...
playback graph／output ownershipを失効させるretune（物理選局・service・codec/PID graph変更、またはcurrent pipelineがhealthy/lockedでなく再利用不能な場合）、明示的playback generation変更、stop、Surface変更、AudioTrack切替／再生成、audio route変更、decoder再生成では、既存MediaSyncの内部anchorを再利用せず、playback rateを0へ戻して未返却audio bufferと旧decoder outputを回収し、MediaSync input Surface、MediaSync、decoder、AudioTrackを解放して新generationとして再生成する。same-channelかつcurrent physical tuneがlocked/healthyでgraph変更のない非破壊`onTune()`はこのfull reset条件ではなく、前段のper-tune fresh `AvailabilityArm`だけを必須とする。plain `Filter.flush()`はAOSP契約どおり未消費filter dataのclearに限定し、assembler/pending inputを破棄するだけでMediaSync/decoder/AudioTrack/playback generationを再生成しない。PTS raw deltaの大小、通常33-bit wrap、presentation-order reorderだけでもMediaSync generationを再生成しない。旧generationのdecoder／MediaSync／route callbackはstate更新に使わず、旧bufferを非描画解放する。

最低試験契約は、型付きLinearBlock→block model QueueRequest、MediaEvent／LinearBlock／range境界をAU境界にしないこと、start code／syncword／header／NAL／slice headerを各byte位置でfragment分割しても同じAU列を復元すること、MPEG-2 Videoのsequence/GOP/picture association、H.264の3/4-byte Annex-B start code分割・AUD無し・SPS/PPS/SEI association・7.4.1.2.3/.4 first-VCL判定、AACのADTS 7/9-byte header分割・`aac_frame_length`跨ぎ・1 frame=1 raw_data_block運用、MPEG audioの4-byte header分割・frame-size跨ぎを検証すること、`PtsAnchor`を既開始AUへ後方適用しないこと、1 rangeのanchorを複数AUへ複製しないこと、同一AU候補の同値anchorはcoalesceし異値anchorは`CONFLICTING_PTS_ACCESS_UNIT`で当該AUだけdropしてfirst/last-winsにしないこと、後続fragmentのanchorで既開始AUのPTSを書き換えないこと、ES stream全体のByteArray copy／通常input-buffer fallback禁止、**PTSなしのpartial `MediaEvent`を単独dropせず保持して後続fragmentとaccess unitを完成させること、単一block・連続rangeの完成AUはsource LinearBlockを直接queueすること、fragmented audio AUと`FEATURE_PartialFrame`非対応video decoderのfragmented AUはいずれもTIS所有`LinearBlock`へ完成AU単位で再構成して1回queueし`BUFFER_FLAG_PARTIAL_FRAME`へ依存しないこと、reassembly中はsource claimと追加reassembly claimの二重保持peakをbudgetへ計上しcopy完了後だけsourceを解放すること、明示PTSが無い場合はevent fragmentではなく完成access unit単位でだけdropすること**、incomplete access unit／reassembly budget超過、track-local `rawPrev`／`extendedPrev`とgeneration共有`PtsEpochCoordinator`、`2^33-1 -> 0` wrap、`0 -> 2^33-1` reorder、半周期差=`-2^32`、**generation開始時のvideo=`2^33-100`／audio=`50`とその逆順でextended差が±150 tick近傍の同一epochになること、片trackがwrap前・他trackがwrap後に開始すること、同generation内で後から明示PTSを得たtrackがwrapを跨いでもcurrent coordinatorへjoinすること**、PTS deltaの大小だけではgeneration resetしないこと、新generationだけがcoordinatorをresetしplain `Filter.flush()`ではcoordinator／seed済みnormalizerを維持したままassembler・pending inputだけをclearすること、decoder再生成／AudioTrack切替／graph変更track切替ではfull generation resetすること、`MediaEvent`のlong offset／lengthについて負値、0 length、`Int.MAX_VALUE`境界、`Int.MAX_VALUE+1`、checked-add overflow、`end==capacity`、`end>capacity`をnarrow前に検査すること、MediaSync rate-0有限prefillからstartup gate成立後speed 1.0へ遷移すること、`MEDIASYNC_ERROR_SURFACE_FAIL`と`MEDIASYNC_ERROR_AUDIOTRACK_FAIL`の分離、audio-videoでAudioTrack failure時のvideo-only継続、audio-onlyでの再生不能遷移、video-onlyがAudioTrack error遷移を持たないこと、各accepted video `onTune()`でfresh availability armを作り、そのarm後のfinal-output成功前はvideo availableにしないこと、late-drop／attach失敗／queue失敗でepochを消費しないこと、成功event後だけ一回availability通知すること、`available -> recoverable unavailable -> available`で同MediaSync instanceを維持する場合にfresh `AvailabilityArm`でlistenerをre-armして次のfinal-output成功後だけ再availableにすること、**epoch Aのfinal-output成功eventを生成したままJava配送を遅延させ、その間にunavailable遷移とepoch Bのre-armを行った場合、遅延event AをID不一致で破棄し、epoch B arm後の新しいfinal-output成功eventだけで再availableにする競合試験**、generation teardown後は新instanceのinitial armを使うこと、audio bufferのconsume callbackまでの寿命、A/V同期、video-only、audio-only、destructive retune／Surface変更／AudioTrack切替・再生成／audio route変更／decoder再生成後のMediaSync再生成、plain `Filter.flush()`ではMediaSyncを再生成しないこと、same-generation non-destructive `onTune()`でもfresh arm後のfinal-output成功で当該tuneのavailabilityを通知すること、pending旧tokenを新armへ再利用しないこと、およびtoken counter exhaustionをplayback teardownへ接続しないこと、route変更後の旧generation非利用、MediaSync error写像、stale generation非描画を含む。試験のqueue数値上限は選択した`ProductProfile`と一致させる。
...
`ChannelTune` は `deliverySystem`、`frequencyHz`、`streamIdType`、`streamId`、`physicalChannel`、`satelliteBand`、`remoteControlKeyId` だけを持つtyped物理tune復元値とし、`inputId`、backend名、driver名、px4相対slot等を持たない。TV input ownershipはchannel rowのrequired fieldである`TvContract.Channels.COLUMN_INPUT_ID`を唯一のSSOTとする。tune復元前にrowの`COLUMN_INPUT_ID`がcurrent TISの`TvInputInfo.id`と一致することを検証し、不一致rowのprovider-dataを別inputの物理tuneとして使用しない。`decodeChannelProviderData()` は invalid UTF-8、malformed JSON、schema不整合を null または診断付き失敗へ落とし、Kotlin側でJSONを解釈・修復しない。
...
- `MediaEvent` sampleは固定4 MiBを上限にしない。負のoffset、0以下のlength、加算overflow、`offset + length > LinearBlock capacity`は不正入力として確保前に破棄する。正常sampleは同一製品profileのper-event予算をclaimし、range境界をAU境界にせずtrack別`AccessUnitAssembler`の論理ES cursorへ到着順に接続する。完成AUが単一blockの連続rangeなら直接block model QueueRequestへ渡す。fragmented AUだけは完成AU単位の追加reassembly claimを予約してTIS所有`LinearBlock`へ再構成し、ES stream全体のcopyや通常ByteBuffer input modelへのfallbackは行わない。共有領域方式とイベント固有fd方式を同じpending byte予算へ計上する。
