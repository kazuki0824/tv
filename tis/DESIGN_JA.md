...
TISの物理候補表は製品scan実装データのSSOTであり、`開発規則.md`の規範値に従うRF候補を唯一保持する。BS setup/rescanは物理RFごとにstream selector未指定の`IsdbsFrontendSettings`でAOSP `Tuner.scan()`を実行し、`ScanCallback.onInputStreamIdsReported()`で得たcurrent stream IDだけをtyped `STREAM_ID` explicit tune candidateへ変換する。dynamic stream-ID discoveryが失敗・timeout・空報告の場合は代替の固定TSID候補を作らない。AOSPの`Tuner.RESULT_UNAVAILABLE`は資源利用不可であり、能力非対応の通知ではない。本製品HALはdynamic discoveryを提供する。既存候補表はRF列挙に使用し、固定TSIDからのfallback候補生成APIは持たない。将来別frontendの非対応能力を扱う場合は、製品統合で明示的な能力設定を先に定義し、その型付き設定を必須入力とする設計を別途行う。候補を実際にtuneした後、PAT/NIT/SDT actualからONID/TSID/SIDとcurrent transportを確認できたserviceだけを登録・公開する。driver固有slotまたはlegacy数値域への写像はTuner HALへ委ねる。
...
scan用TunerにもTuner Framework標準のresource-lost listenerを登録し、callbackのtune generationがそのscan candidateのactive generationと一致する場合だけ当該taskを`RESOURCE_LOST`で終端する。TunerControllerが同generationのsection filterと再生資源を停止した後、ChannelScanControllerはそれ以後のSI snapshot取得とTvProvider publishをgeneration fenceで拒否し、残りcandidateへ進まない。遅延した旧generationのresource-lostは後続candidate/taskへ伝播させない。setup scanは上位へ失敗を返し、利用者または正規setup flowの再要求を待つ。boot EPG同期はpendingを維持して既存schedulerへ再試行を返し、background maintenanceは失敗終了して次の既存scheduleに委ねる。TIS独自の優先度、resource pool、即時再取得loopは追加しない。
