...
### 公開APIによるvideo availability通知

TISは公開APIの`MediaCodec.OnFrameRenderedListener`を、decoderが`MediaSync.createInputSurface()`へframeをrenderした通知として使用する。この通知はMediaSync内部のlate-drop判定、final outputへの`queueBuffer()`成功、物理display/compositorでのpresent完了を証明しないため、それらを`notifyVideoAvailable()`の意味として表明しない。AOSPのTIF契約に従い、視聴可能なvideo outputが開始したことをSessionへ通知するための通常の実装信号として扱う。Framework-private API、runtime reflection、platform patch、Exact/Compatibilityの二経路は設けない。

TISは**各accepted `onTune(Uri)`ごと**に新しいvideo availability obligationを生成する。videoを持つtuneではcurrent waiting armを取消し、TIS playback graphを終了して新しいplayback generationと新MediaSync instanceのwaiting armを作る。TISはchannel同一性、frontend lock状態、pipeline healthを根拠に`Tuner.tune(settings)`の発行またはplayback generation再生成を省略しない。同一の正規化settingsで既存物理lockを安全に継続できる場合にbackend retuneを省略する判断は、frontend stateを所有するTuner HALだけが行う。audio-only serviceは映像availabilityを偽装せず`notifyVideoUnavailable(VIDEO_UNAVAILABLE_REASON_AUDIO_ONLY)`で閉じる。

callbackはcurrent video codec、playback generation、arm開始時刻を同executor上で照合し、current `sessionSurface`が有効で、視聴制限でblockされておらず、同generationの`MEDIASYNC_ERROR_SURFACE_FAIL`がない場合だけ`notifyVideoAvailable()`を呼ぶ。arm開始より古い遅延callbackは捨て、受理またはarm取消し後はcurrent waiting armを無効化する。一度availableになった後、recoverable unavailableから同じgenerationで復旧する場合は新しい開始時刻でre-armし、それより前のcallbackを捨てる。generation teardownを伴うunavailableは新generationのwaiting armで閉じる。独自sequence、永続token集合、独自clock、compositor観測は追加しない。
...
本productはnon-tunneled MediaCodec + MediaSync経路をarchitectureとして採用し、tunneled / platform passthrough playback capabilityを恒久的に提供しない。`notifyVideoAvailable()`は、公開`MediaCodec.OnFrameRenderedListener`のframe render時刻がcurrent arm時刻以後で、current codec/generation/Surface/error gateを満たす最初のeventを根拠にする。このeventはMediaSync input Surface到達の観測であり、MediaSync final output成功やcompositor presentとはみなさない。`MediaSync.getTimestamp()`のclock進行はavailability根拠にしない。
...
- `notifyVideoAvailable()`は公開`MediaCodec.OnFrameRenderedListener`を型付きで使い、current codec/generationかつcurrent arm時刻以後のframe renderを根拠とし、current Surface有効、視聴制限、Surface errorのgateを満たした場合だけ呼ぶ。これはMediaSync input Surface到達の観測であり、MediaSync final output成功やcompositor presentとはみなさない。`MediaSync.getTimestamp()`のclock進行はavailability根拠にしない。
