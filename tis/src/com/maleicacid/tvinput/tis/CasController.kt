...
class CasController(
    ...
    private val emmPidToSystems = LinkedHashMap<Int, MutableSet<Int>>()
    private val elementaryPidToSystems = LinkedHashMap<Int, MutableSet<Int>>()
        ...
        metadata.forEach { ca ->
            ...
            when (ca.source) {
                CaMetadataSource.PROGRAM -> {
                    ...
                    val ecmPid = ca.ecmPid?.takeIf { it in 0..0x1fff } ?: return@forEach
                ...
                CaMetadataSource.ELEMENTARY_STREAM -> {
                    val serviceKey = ca.serviceKey ?: return@forEach
                    val ecmPid = ca.ecmPid?.takeIf { it in 0..0x1fff } ?: return@forEach
                    val elementaryPid = ca.elementaryPid?.takeIf { it in 0..0x1fff } ?: return@forEach
                    esBindings += EsCaBinding(serviceKey.toString(), ca.caSystemId, ecmPid, elementaryPid, ca.privateData.copyOf())
                }
                CaMetadataSource.CAT -> {
                    val emmPid = ca.emmPid?.takeIf { it in 0..0x1fff } ?: return@forEach
        ...
        systems.forEach { systemId ->
            ...
            when (val ecmResult = tokenResult.getOrNull()) {
                is EcmProcessResult.RealKeyToken -> {
                    ...
                    if (token.isEmpty()) {
                        diagnostics += Diagnostic(State.ERROR, ErrorCode.KEY_TOKEN_MISSING, systemId, pid, "MediaCas session から 不透明 token を取得できません")
                        return@forEach
                    }
...
sealed class EcmProcessResult {
    data class RealKeyToken(val token: ByteArray) : EcmProcessResult()
