...
class CasController(
    ...
    private fun syncContextPidsLocked(
        ...
        (state.linkedElementaryPids - state.desiredElementaryPids).forEach { pid ->
            val removeResult = state.descrambler.removePid(pid)
        ...
        if (state.keyLinked) {
            (state.desiredElementaryPids - state.linkedElementaryPids).forEach { pid ->
                val addResult = state.descrambler.addPid(pid)
    ...
    private fun linkContextKeyLocked(
        ...
        if (failed) {
            ...
            val clearResult = state.descrambler.clearKeyToken()
    ...
    private fun CasSessionState.isFullyLinked(): Boolean =
        keyLinked && desiredElementaryPids == linkedElementaryPids
    ...
    private fun closeContextLocked(key: DescrambleContextKey) {
        ...
        state.linkedElementaryPids.toList().forEach { pid -> runCatching { state.descrambler.removePid(pid) } }
