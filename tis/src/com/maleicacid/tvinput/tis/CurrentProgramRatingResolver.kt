...
class CurrentProgramRatingResolver internal constructor(
    ...
    data class CurrentProgramRatingSet(
        ...
        fun unblockKeyFor(rating: TvContentRating): String =
            unblockKey(
    ...
    sealed class EitAuthority {
        ...
        object AuthoritativeEmpty : EitAuthority()

        data class PresentObserved(
            val event: AribEvent?,
        ) : EitAuthority()
    }
    ...
    fun eitAuthority(
        ...
        val present =
            snapshot.eitInstances.singleOrNull {
                it.serviceKey == key && it.tableId == 0x4e && it.currentNextIndicator
            } ?: return EitAuthority.Unconfirmed
        val presentSectionUnconfirmed =
            present.inconsistent || 0 !in present.receivedSections || 0 !in present.safeSections || 0 in present.missingSections
        if (presentSectionUnconfirmed) {
            return EitAuthority.Unconfirmed
        }
        ...
        if (events.isEmpty() && !excluded) return EitAuthority.AuthoritativeEmpty
    ...
    fun resolveDetailed(
        ...
        if (eitAuthority == EitAuthority.AuthoritativeEmpty) {
            currentProgramResolutionDiagnostic =
                CurrentProgramResolutionDiagnostic(
                    "CURRENT_GENERATION_EMPTY_EIT",
                    0,
                    null,
                    "AUTHORITATIVE_EMPTY_BEFORE_PROVIDER_QUERY",
                )
        ...
        if (eitAuthority is EitAuthority.PresentObserved) {
            ...
            currentProgramResolutionDiagnostic =
                CurrentProgramResolutionDiagnostic(
                    "CURRENT_GENERATION_PRESENT_EIT",
                    0,
                    null,
                    "PRESENT_OBSERVED_BEFORE_PROVIDER_QUERY",
                )
            return ResolveResult.Ratings(
                event?.let { ratingFromEvent(channelUri, it, ratingProfile) }
        ...
        if (latestEit != null) {
            currentProgramResolutionDiagnostic =
                CurrentProgramResolutionDiagnostic(
                    "CURRENT_GENERATION_EIT",
                    0,
                    null,
                    "LATEST_EIT_BEFORE_PROVIDER_QUERY",
                )
    ...
    companion object {
        ...
        ): Pair<CurrentProgramRatingSet?, String> =
            when {
        ...
        private fun sameStableProgram(
            left: CurrentProgramRatingSet,
            right: CurrentProgramRatingSet,
        ): Boolean =
            left.serviceKey != null && left.serviceKey == right.serviceKey &&
                left.eventId != null && left.eventId == right.eventId

        private fun sameProgramOccurrence(
            left: CurrentProgramRatingSet,
            right: CurrentProgramRatingSet,
        ): Boolean =
        ...
        fun stableProgramKey(
            serviceKey: ServiceKey,
            eventId: Int,
        ): String = ProviderDataBridge.buildProgramKey(serviceKey, eventId)
