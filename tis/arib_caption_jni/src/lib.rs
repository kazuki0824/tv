...
use facts::{CaptionFactBatch, CaptionFactParser};
...
const CAPTION_FACT_REGISTRY_LOCK_NAME: &str = "caption_fact_registry";
const CAPTION_FACT_PARSER_LOCK_NAME: &str = "caption_fact_parser";
...
impl CaptionEngine {
    fn new(data_component_id: jint, superimpose: bool, language_id: jint) -> Option<Self> {
        ...
        debug_assert!(match engine.as_ref() {
            Some(value) =>
                !value.context.is_null() && !value.decoder.is_null() && !value.renderer.is_null(),
            None => true,
        });
        engine
    }
    ...
    fn decode_and_render(&mut self, pes: &[u8], pts_millis: jlong) -> Option<RenderedCaptionFrame> {
        ...
        let frame = unsafe {
            'decode: {
                ...
                let Some((caption_pts, caption_duration)) = validate_decoded_caption_timing(
                    caption.pts,
                    pts_millis,
        ...
        debug_assert!(match frame.as_ref() {
            Some(value) => value.images.len() <= MAX_IMAGES_PER_FRAME,
            None => true,
        });
        frame
    }
...
fn validate_decoded_caption_timing(
    ...
    if authoritative_pts == ARIBCC_PTS_NOPTS {
        if !allow_no_pts || (decoded_pts != ARIBCC_PTS_NOPTS && decoded_pts < 0) {
            return None;
        }
        return Some((decoded_pts, decoded_duration));
    }
...
fn encode_fact_batch(batch: CaptionFactBatch) -> Option<Vec<u8>> {
    if batch.disposition == facts::CaptionFactDisposition::None
        && batch.management.is_none()
        && batch.statement_time.is_none()
    {
        return None;
    }
    let mut flags = 0u8;
    if batch.management.is_some() {
        flags |= 0x01;
    }
    if batch.statement_time.is_some() {
        flags |= 0x02;
    }
    let mut packet = vec![2u8, flags, batch.disposition as u8];
    if let Some(statement) = batch.statement_time {
        packet.push(statement.tmd);
        packet.extend_from_slice(&statement.millis_of_day.to_le_bytes());
    }
    if let Some(management) = batch.management {
        packet.push(management.tmd);
        packet.push(u8::try_from(management.languages.len()).ok()?);
        for language in management.languages {
            packet.push(language.language_tag);
            packet.push(language.dmf);
            packet.push(language.automatic_presentation_on_reception as u8);
            packet.push(language.dc.unwrap_or(0xff));
            packet.push(language.format);
            packet.push(language.tcs);
            packet.push(language.rollup_mode);
            packet.extend_from_slice(&language.iso639);
        }
    }
    Some(packet)
}
...
#[derive(Default)]
struct FactParserRegistry {
    next_handle: jlong,
    parsers: BTreeMap<jlong, Arc<Mutex<CaptionFactParser>>>,
}

impl FactParserRegistry {
    fn insert(&mut self, parser: CaptionFactParser) -> jlong {
        self.next_handle = self.next_handle.checked_add(1).unwrap_or(1).max(1);
        let handle = self.next_handle;
        self.parsers.insert(handle, Arc::new(Mutex::new(parser)));
        handle
    }

    fn get(&self, handle: jlong) -> Option<Arc<Mutex<CaptionFactParser>>> {
        self.parsers.get(&handle).cloned()
    }

    fn remove(&mut self, handle: jlong) {
        self.parsers.remove(&handle);
    }
}

fn fact_registry() -> &'static Mutex<FactParserRegistry> {
    static REGISTRY: OnceLock<Mutex<FactParserRegistry>> = OnceLock::new();
    REGISTRY.get_or_init(|| Mutex::new(FactParserRegistry::default()))
}

fn fact_parser(handle: jlong) -> Option<Arc<Mutex<CaptionFactParser>>> {
    if handle == 0 || !caption_module_is_healthy() {
        return None;
    }
    match fact_registry().lock() {
        Ok(guard) => guard.get(handle),
        Err(_) => {
            record_caption_mutex_poison(CAPTION_FACT_REGISTRY_LOCK_NAME);
            None
        }
    }
}
...
#[no_mangle]
pub extern "system" fn Java_com_maleicacid_tvinput_aribsi_NativeAribCaptionFactParser_nativeCreateFactParser(
    _env: JNIEnv<'_>,
    _this: JObject<'_>,
    ...
    match fact_registry().lock() {
        Ok(mut guard) => guard.insert(CaptionFactParser::new(superimpose != 0)),
        Err(_) => {
            record_caption_mutex_poison(CAPTION_FACT_REGISTRY_LOCK_NAME);
            0
        }
...
#[no_mangle]
pub extern "system" fn Java_com_maleicacid_tvinput_aribsi_NativeAribCaptionFactParser_nativeIngestFactParser(
    env: JNIEnv<'_>,
    _this: JObject<'_>,
    handle: jlong,
    pes_payload: JByteArray<'_>,
) -> jbyteArray {
    let Ok(bytes) = env.convert_byte_array(pes_payload) else {
        return ptr::null_mut();
    };
    let Some(parser) = fact_parser(handle) else {
        return ptr::null_mut();
    };
    let batch = match parser.lock() {
        Ok(mut guard) => guard.ingest(&bytes),
        Err(_) => {
            record_caption_mutex_poison(CAPTION_FACT_PARSER_LOCK_NAME);
            return ptr::null_mut();
        }
    };
    let Some(packet) = encode_fact_batch(batch) else {
        return ptr::null_mut();
    };
...
#[no_mangle]
pub extern "system" fn Java_com_maleicacid_tvinput_aribsi_NativeAribCaptionFactParser_nativeResetFactParser(
    _env: JNIEnv<'_>,
    _this: JObject<'_>,
    handle: jlong,
) {
    ...
    match parser.lock() {
        ...
        Err(_) => record_caption_mutex_poison(CAPTION_FACT_PARSER_LOCK_NAME),
    };
}

#[no_mangle]
pub extern "system" fn Java_com_maleicacid_tvinput_aribsi_NativeAribCaptionFactParser_nativeReleaseFactParser(
    _env: JNIEnv<'_>,
    _this: JObject<'_>,
    handle: jlong,
) {
    match fact_registry().lock() {
        ...
        Err(_) => record_caption_mutex_poison(CAPTION_FACT_REGISTRY_LOCK_NAME),
    }
}
...
pub extern "system" fn Java_com_maleicacid_tvinput_aribsi_NativeAribCaptionRenderer_nativeFlush(
    ...
    let _ = with_engine(handle, |engine| engine.flush());
}
...
mod tests {
    ...
    fn no_pts_is_reserved_for_asynchronous_superimpose() {
        ...
        assert_eq!(
            validate_decoded_caption_timing(12_345, ARIBCC_PTS_NOPTS, 10, true),
            Some((12_345, 10)),
        );
        assert_eq!(
