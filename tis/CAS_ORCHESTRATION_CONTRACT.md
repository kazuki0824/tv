...
途中のcleanup失敗で後続cleanupを中止しない。解放に失敗したcontext、PID/key link、Descrambler、Session、pluginの所有と成功済み解放の状態を保持し、次回cleanupで未解放の資源だけを再試行する。metadata更新は退役資源の解放完了まで新しいroutingを公開しない。TISはvendor-private key registryへ直接Revokeを送らない。MediaCas session closeに伴うprovider-side revokeはCAS HAL/key provisioning側の責務とする。
...
`close()` は同executor上でcurrent contextをrouting mapから除外し、上記teardown orderingに従うcleanup、plugin/descrambler解放、`CLOSED`診断確定までを直列化した後にexecutorをshutdownする。cleanupが失敗した場合は所有とexecutorを保持して`close()`を再試行可能にし、全解放が成功した場合だけshutdownする。`close()` は冪等であり、shutdown済みexecutorへ再度workを投入しない。
...
- contextはcurrent Tuner/demux generationに属する。Descrambler factoryはcurrent-generationのCAS更新transaction内でcontextごとに実行し、resource lossとの間に未所有のbridgeを残さない。
