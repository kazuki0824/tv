...
class ChannelNumberingPolicyTest {
    @Test fun terrestrialUsesRemoteKeyAndStableBranch() {
        val candidate = ScanCandidate(ChannelRecord.DELIVERY_SYSTEM_ISDB_T, 473_142_857L, displayChannel = "13", physicalChannel = 13)
    ...
    @Test fun satelliteUsesBandAndServiceIdWithoutCsStreamSelector() {
        val candidate = ScanCandidate(ChannelRecord.DELIVERY_SYSTEM_ISDB_S, 1_613_000_000L, displayChannel = "CS1", satelliteBand = "110CS")
    ...
    @Test fun earthPt1BsRejectsRelativeSelector() {
        val failed = runCatching {
            ScanCandidate(ChannelRecord.DELIVERY_SYSTEM_ISDB_S, 1_318_000_000L, streamSelector = StreamSelector.relative(1), displayChannel = "BS15/1", satelliteBand = "BS", backendHint = "earth_pt1")
        }.isFailure
        check(failed)
    }
    ...
    @Test fun cs110RejectsStreamSelector() {
        val failed = runCatching {
            ScanCandidate(ChannelRecord.DELIVERY_SYSTEM_ISDB_S, 1_613_000_000L, streamSelector = StreamSelector.tsid(16400), displayChannel = "CS1", satelliteBand = "110CS")
        }.isFailure
        check(failed)
    }

    @Test fun px4BsAllowsRelativeSelector() {
        val candidate = ScanCandidate(ChannelRecord.DELIVERY_SYSTEM_ISDB_S, 1_318_000_000L, streamSelector = StreamSelector.relative(1), displayChannel = "BS15/1", satelliteBand = "BS", backendHint = "px4")
