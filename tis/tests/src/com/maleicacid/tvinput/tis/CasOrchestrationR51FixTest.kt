...
class CasOrchestrationR51FixTest {
    @Test fun catOnlyDoesNotRequireCasButIsRetainedForEmmFilter() {
        val catOnly = listOf(CaMetadata(null, 0x0005, ecmPid = null, emmPid = 0x0010, elementaryPid = null, source = CaMetadataSource.CAT))
        val mapped = PmtCatCaMetadataMapper().expandProgramLevelToElementaryStreams(catOnly, services = emptyList())
        check(mapped.single().emmPid == 0x0010)
    ...
    @Test fun pmtOrEsCaRequiresCas() {
        ...
        val program = CaMetadata(key, 0x0005, ecmPid = 0x1fff, emmPid = null, elementaryPid = null, source = CaMetadataSource.PROGRAM)
        val es = CaMetadata(key, 0x0005, ecmPid = 0x1ffe, emmPid = null, elementaryPid = 0x0100, source = CaMetadataSource.ELEMENTARY_STREAM)
