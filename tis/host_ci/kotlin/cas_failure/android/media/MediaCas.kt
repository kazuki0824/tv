...
class MediaCas(caSystemId: Int) : AutoCloseable {
    init { Faults.creates++ }
    fun setPrivateData(data: ByteArray) = Unit
    fun processEmm(data: ByteArray, offset: Int, length: Int) = Unit
    ...
    class Session : AutoCloseable {
        fun setPrivateData(data: ByteArray) = Unit
        fun processEcm(data: ByteArray, offset: Int, length: Int) = Unit
        override fun close() {
    ...
    object Faults {
        ...
        fun reset() {
            creates = 0; pluginCloses = 0; sessionCloses = 0
            openFailure = false; pluginFailure = false; sessionFailure = false
        }
    }
}
